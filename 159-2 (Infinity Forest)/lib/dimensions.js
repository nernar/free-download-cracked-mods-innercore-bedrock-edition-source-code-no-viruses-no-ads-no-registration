LIBRARY({name: "dimensions", version: 1, shared: true, api: "CoreEngine"});
var __global__ = this;
...
var DEBUG_COLOR_PALETTE = [android.graphics.Color.WHITE, android.graphics.Color.RED, android.graphics.Color.GREEN, android.graphics.Color.BLUE, android.graphics.Color.YELLOW, android.graphics.Color.CYAN, android.graphics.Color.MAGENTA, android.graphics.Color.rgb(255, 153, 0)];
...
var WrappedDimensionRegistry = {dimensionById: {}, registerDimension: function (dim) {
...
}, lastLoadedDimension: null, dimensionLoaded: function (id) {
...
var Parser = {assertParameterType: function (param, expected, name) {
    ...
    if (typeof (idData) == "number") {
        ...
        if (typeof (idData.id) == "number") {
            pair = [idData.id, idData.data];
        } else {
            if (typeof (idData[0]) == "number") {
                pair = idData;
            } else {
                return null;
            }
    ...
    if (descr.octaves) {
        ...
        if (Array.isArray(descr.octaves)) {
            ...
            if (descr.octaves.count > 0 && descr.octaves.scale) {
...
function Dimension(descr) {
    ...
    this.setupGeneration = function (description) {
        ...
        if (description.decoration) {
            if (Parser.assertParameterType(description.decoration.ee, "number", "decoration.biome")) {
    ...
    this._prepareTeleporter = function (teleporter, handler, gui) {
        var additionalData = {dimension: this, teleporter: teleporter};
        var container = new UI.Container();
        teleporter.setCallbacks({onStart: function (tp) {
...
var PortalRegistry = {PORTAL_BLOCK_TYPE: Block.createSpecialType({base: 90, lightlevel: 15, rendertype: 0, renderlayer: 1, explosionres: 9999}, "custom-portal"), portalBlocks: {}, lastTickPortal: null, getPortalForBlock: function (id) {
...
}, RENDER_TYPE_FULL: "full", RENDER_TYPE_VERTICAL_PLANE: "v-plane", RENDER_TYPE_HORIZONTAL_PLANE: "h-plane", RENDER_TYPE_UNIVERSAL_PLANE: "u-plane", setupPortalBlock: function (id, texture, portalRenderParams) {
    ...
    if (portalRenderParams.type == this.RENDER_TYPE_FULL) {
        ...
        if (portalRenderParams.type == this.RENDER_TYPE_HORIZONTAL_PLANE) {
            render.addEntry(BlockRenderer.createTexturedBox(0, 0.375, 0, 1, 0.625, 1, texture));
        } else {
            if (portalRenderParams.type == this.RENDER_TYPE_VERTICAL_PLANE) {
                var xAxis = BlockRenderer.createTexturedBox(0, 0, 0.375, 1, 1, 0.625, texture);
                var zAxis = BlockRenderer.createTexturedBox(0.375, 0, 0, 0.625, 1, 1, texture);
                ...
                if (portalRenderParams.type == this.RENDER_TYPE_UNIVERSAL_PLANE) {
                    var xAxis = BlockRenderer.createTexturedBox(0, 0, 0.375, 1, 1, 0.625, texture);
                    var yAxis = BlockRenderer.createTexturedBox(0, 0.375, 0, 1, 0.625, 1, texture);
                    var zAxis = BlockRenderer.createTexturedBox(0.375, 0, 0, 0.625, 1, 1, texture);
                    render.addEntry(xAxis).setCondition(ICRender.NOT(ICRender.OR(ICRender.BLOCK(0, 0, 1, group, false), ICRender.BLOCK(0, 0, -1, group, false))));
    ...
    Block.createBlock(id, [{name: "tile.portal." + id, texture: texture, inCreative: debugEnabled}], this.PORTAL_BLOCK_TYPE);
...
function Portal(dimension) {
    ...
    this.startTransfer = function () {
        ...
        this.transferUpdatable = {portal: this, progress: 0, finish: function () {
            this.remove = true;
...
function DimensionTransfer(dimension) {
    ...
    this.idle = true;
    var self = this;
    this.handler = {transfer: this, dimension: this.dimension, portalBlockId: -1, frameBlockId: -1, portalSearchRadius: 5, start: function () {
        self.idle = false;
    }, progress: function () {
        ...
        if (this.portalBlockId > 0) {
            ...
            if (portalPos) {
                ...
                pos.x = Math.floor(correctedPos.x) + 0.5;
                pos.y = correctedPos.y;
                pos.z = Math.floor(correctedPos.z) + 0.5;
                self.idle = true;
                return true;
            }
        }
    ...
    this.gui = {transfer: this, dimension: this.dimension, screen: new UI.Window({location: {fullscreen: true}, drawing: [], elements: {text: {type: "text", x: 500, y: UI.getScreenHeight() / 2 - 35, z: 1, text: "Teleporting...", font: {color: android.graphics.Color.WHITE, size: 22, shadow: 0, align: 1}}, background: {type: "custom", x: 0, y: 0, z: -1, progress: 0, custom: {texture: "", resizeToScreen: false, scaleColor: android.graphics.Color.GREEN, scaleBgColor: android.graphics.Color.argb(128, 0, 40, 0), scaleAtBottom: false}, onSetup: function (element) {
...
function PortalShape() {
    ...
    this.frameIds = {};
    this.buildIds = {portal: -1, frame: -1};
    this.orientation = {vertical: true, horizontal: true};
    ...
    this._buildRect = function (pos, raw) {
        if (raw.minX != -1 && raw.minY != -1 && raw.maxX != -1 && raw.maxY != -1) {
            var rect = {minX: -raw.minX, maxX: raw.maxX + 1, minY: -raw.minY, maxY: raw.maxY + 1, plane: raw.plane, width: function () {
            ...
            switch (rect.plane) {
              case "oX":
                rect.convert = function (x, y, z) {
                    return {x: this.offset.x + (z || 0), y: this.offset.y + y, z: this.offset.z + x};
                };
                break;
              case "oY":
                rect.convert = function (x, y, z) {
                    return {x: this.offset.x + x, y: this.offset.y + (z || 0), z: this.offset.z + y};
                };
                break;
              case "oZ":
                rect.convert = function (x, y, z) {
                    return {x: this.offset.x + x, y: this.offset.y + y, z: this.offset.z + (z || 0)};
                };
                break;
              default:
                return;
    ...
    this._getPossibleRects = function (x, y, z) {
        var distances = {left: this._searchForFrameTile(x, y, z, -1, 0, 0), right: this._searchForFrameTile(x, y, z, 1, 0, 0), down: this._searchForFrameTile(x, y, z, 0, -1, 0), up: this._searchForFrameTile(x, y, z, 0, 1, 0), back: this._searchForFrameTile(x, y, z, 0, 0, -1), forward: this._searchForFrameTile(x, y, z, 0, 0, 1)};
        var rawRects = [];
        if (this.orientation.horizontal) {
            rawRects.push({minX: distances.left, maxX: distances.right, minY: distances.back, maxY: distances.forward, plane: "oY"});
        }
        if (this.orientation.vertical) {
            rawRects.push({minX: distances.back, maxX: distances.forward, minY: distances.down, maxY: distances.up, plane: "oX"});
            rawRects.push({minX: distances.left, maxX: distances.right, minY: distances.down, maxY: distances.up, plane: "oZ"});
        }
        var rects = [];
    ...
    this.getBuilder = function () {
        ...
        return function (pos) {
            ...
            var rect = self._buildRect({x: x, y: y, z: z}, {minX: 0, minY: 0, maxX: self.minWidth + 1, maxY: self.minHeight + 1, plane: self.orientation.vertical ? "oX" : "oY"});
            self.buildPortal(rect, true);
        };
    };
}
var DimensionHelper = {searchForPortal: function (pos, blockId, options) {
    if (!options) {
        options = {};
    }
    if (!options.radius) {
    ...
    if (options.movePlayer) {
        Player.setPosition(posx + 0.5, posy + 2.63, posz + 0.5);
    }
    return {x: posx + 0.5, y: posy + 2.63, z: posz + 0.5};
    ...
    Updatable.addUpdatable({_checkPortal: function (x, y, z) {
        var count = 0;
...
function PortalOverlayWindow(content) {
    this.alpha = 1;
    var overlay = this;
    this.window = new UI.Window({location: {fullscreen: true}, drawing: [{type: "color", color: 0}], elements: {"overlay": {type: "custom", custom: {alpha: 0}, onSetup: function (element) {
