Saver.addSavesScope("SatchelScope", function read(scope) {
    SatchelRegistry.save = scope;
}, function save() {
    return SatchelRegistry.save;
});
const SatchelRegistry = {save: {uniqueID: 0, containers: {}}, prototypes: {}, guis: {}, onBackpackUse: function (item) {
    ...
    if (!this.guis[arg.slots]) {
        let obj = new UI.StandartWindow({standart: {header: {text: {text: Translation.translate("Backpack")}}, inventory: {standart: true}, background: {standart: true}, minHeight: 90 + (arg.slots / 10 * 61) + 70}, drawing: [], elements: {}});
        let xp = 345;
        let yp = 60;
