function initializeRecipeGuideFor(object, name, unique) {
    ...
    for (let i in object.recipes) {
        ...
        if (!page[currentSection]) {
            page[currentSection] = {controller: PageControllers.TE_TO_RECIPE_PAGE, title: name + " Recipes(Page " + (count / 3) + ")", recipes: []};
        }
        page[currentSection].recipes.push({input: {id: input.id, data: input.data < 0 || !input.data ? 0 : input.data, count: 1}, output: {id: result.id, data: result.data < 0 || !result.data ? 0 : result.data, count: result.count || 1}, dop: {id: dop.id, data: dop.data, count: dop.count, chance: dop.chance}});
        count++;
        if (count % 3 === 0) {
            ...
            currentSection = "left";
            currentPage = newPage;
            pages[newPage] = {preLink: oldPage};
        }
    }
    GuideAPI.registerGuide(unique, {pages: pages});
}
function initializeMagmaCrucibleRecipesGuide() {
    ...
    for (let i in MagmaCrucibleRecipes.recipes) {
        ...
        let page = pages[currentPage];
        if (!page[currentSection]) {
            page[currentSection] = {controller: PageControllers.MAGMA_CRUCIBLE_RECIPE_PAGE, title: "Magma Crucible Recipes(Page " + (count / 6) + ")", recipes: []};
        }
    ...
    GuideAPI.registerGuide("guideMagmaCrucibleRecipes", {pages: pages});
}
