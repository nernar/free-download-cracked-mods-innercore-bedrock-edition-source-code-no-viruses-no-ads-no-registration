...
function bitmap(b){
  var byte=android.util.Base64.decode(b,0)
  return android.graphics.BitmapFactory.decodeByteArray(byte,0,byte.length)
}
function bitmap2(b){
  return new android.graphics.drawable.BitmapDrawable(bitmap(b))
}
...
//McMath.lookDirection(lookAngle.yaw, lookAngle.pitch);
...
//发射的实体
var entity = Entity.spawn(子弹.x+力量.x,子弹.y+力量.y,子弹.z+力量.z,params.stid/*实体 烈焰人火球 上面有实体id表↑*/);
Entity.setVelocity(entity,力度*力量.x,力度*力量.y,力度*力量.z);
...
var Bows = {
	...
	addUI() {
		newThread(function () {
准窗=new android.widget.PopupWindow(ctx);
准线=new android.widget.LinearLayout(ctx);
准线.setPadding(0,0,0,0);
准线.setOrientation(0);
准线.setBackgroundColor(android.graphics .Color.argb(20,0,0,0));
准线.setGravity(17);
准线.setImageBitmap(ImageHelper.bmpToImg(Bows.aim));
		

准窗.setContentView(准线);
准窗.setWidth(适应(640));
准窗.setHeight(适应(360));
准窗.setTouchable(false)
准窗.setBackgroundDrawable(new android.graphics.drawable.ColorDrawable(0));
//准窗.showAtLocation(ctx.getWindow().getDecorView(), android.view.Gravity.CENTER,适应(0),适应(0))
...
layoutMain.setPadding(0,0,0,0);
			...
			var windowAim = new android.widget.PopupWindow(ctx);//layoutMain, LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT);
			windowAim.setTouchable(false);
windowAim.setWidth(适应(640));
windowAim.setHeight(适应(360));
windowAim.setContentView(layoutMain);
	...
	updateAim() {
		...
		//		Bows.windowAim.showAtLocation(ctx.getWindow().getDecorView(), android.view.Gravity.CENTER,适应(0),适应(0))
			}else {
