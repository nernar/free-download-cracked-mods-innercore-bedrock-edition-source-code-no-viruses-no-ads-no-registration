const AchievementAPI = {groups: {}, groupNames: null, groupsAmount: 0, completed: {}, data: {}, containerOverlay: new UI.Container(), container: new UI.Container(), parentContainer: new UI.Container(), currentIndex: 0, init: function () {
    this.groupsShowUI = new UI.Window({location: {x: 1000 / 2 - 80, y: 5, width: 45, height: 45}, drawing: [{type: "color", color: Color.argb(0, 0, 0, 0)}], elements: {"btn": {type: "button", x: 0, y: 0, bitmap: "btn_achievements", scale: 60, clicker: {onClick: function () {
    ...
    this.windowParent = new UI.Window({location: {x: 244, y: (UI.getScreenHeight() - 370) / 2, width: 512, height: 370}, drawing: [{type: "color", color: Color.rgb(198, 198, 198)}, {type: "frame", x: 0, y: 0, width: 1000, height: 725, bitmap: "achievements_frame", scale: 5}, {type: "text", text: Translation.translate("achievementApi.achievements"), x: 80, y: 65, font: {size: 35, color: Color.DKGRAY}}], elements: {"slotGroupIcon": {type: "slot", x: 75, y: 605, size: 100, visual: true, bitmap: "_default_slot_empty", isTransparentBackground: true}, "textGroupName": {type: "text", x: 180, y: 630, text: "", font: {size: 40, color: Color.DKGRAY}}, "btnClose": {type: "button", x: 910, y: 15, bitmap: "achievements_btn_close", bitmap2: "achievements_btn_close_hover", scale: 5, clicker: {onClick: function () {
        AchievementAPI.windowParent.close();
        AchievementAPI.windowArea.close();
    }}}, "textPageIndex": {type: "text", x: 730, y: 630, text: "", font: {size: 40, color: Color.DKGRAY}}, "btnNext": {type: "button", x: 860, y: 620, bitmap: "btn_achievements_next", bitmap2: "btn_achievements_next_hover", scale: 3, clicker: {onClick: function () {
        AchievementAPI.currentIndex++;
        AchievementAPI.openAchievementsWindow();
    }}}, "btnPrevious": {type: "button", x: 640, y: 620, bitmap: "btn_achievements_previous", bitmap2: "btn_achievements_previous_hover", scale: 3, clicker: {onClick: function () {
        AchievementAPI.currentIndex--;
        AchievementAPI.openAchievementsWindow();
    }}}}});
    ...
    if (AchievementAPI.isCompleted(group.unique, achievement.unique)) {
        ...
        if (AchievementAPI.isUnlocked_(group, achievement)) {
            type = "unlocked";
        } else {
            type = "locked";
        }
    ...
    for (let index in group.list) {
        ...
        elements[index] = {type: "slot", x: x, y: y, size: size, visual: true, bitmap: AchievementAPI.getAchievementTexture(group, achievement), isTransparentBackground: true, clicker: {onClick: function () {
    ...
    elements["lines"] = {type: "custom", z: -1, custom: {}, onSetup: function () {
        ...
        if (!this.path) {
            ...
            for (let index in group.list) {
                ...
                let parent = achievement.parent;
                let parentItem;
                if (!parent || parent.groupUnique !== group.unique || (!AchievementAPI.isCompleted(group.unique, parent.unique) && achievement.strongDependence)) {
                    continue;
                }
    ...
    drawing.push({type: "custom", func: function (canvas) {
        this.onDraw(canvas);
    ...
    if (this.currentIndex < 0) {
        ...
        if (this.currentIndex >= this.groupsAmount) {
            this.currentIndex = 0;
        }
    }
    ...
    if (contentExist) {
        ...
        elements["nothing"] = {type: "text", x: 0, y: 200, text: translated, font: {size: 40, color: Color.WHITE}};
    }
    if (this.windowArea) {
        this.windowArea.close();
    }
    this.windowArea = new UI.Window({location: {x: 284, y: (UI.getScreenHeight() - 370) / 2 + 50, width: 432, height: 260, scrollX: width, scrollY: height}, drawing: drawing, elements: elements});
    ...
    if (progressMax) {
        ...
        if (++progress.progress < progressMax) {
            ...
            this.removeDataFor(groupUnique, unique);
        }
    }
    if (!achievement.notCompletePopup) {
        ...
        AchievementPopup.show({title: title, color: color, description: this.getLocalized(achievement, "name"), item: {id: item.id || 1, data: item.data || 0, count: 1}});
    }
