...
var GUI = {
	...
	createDrawable:function(settings, def){
		...
		if(settings.bitmap && settings.bitmap.ninePatch){
			...
			if(!(settings.bitmap.ninePatch && typeof settings.bitmap.ninePatch != "string" && settings.bitmap.ninePatch.length != null)) throw new GUI.DrawableException(settings, "settings.bitmap.ninePatch must be an array");
	...
	Container:function(){
		...
		this.refreshSlots = function(z){
			...
			for(var i in slots){
					...
					if(storage.slots[i]){
						...
						slots[i].setSlot(0,0,0);
					}
					
					slots[i].refreshSlotInThread();
			}
		...
		this.save = function(){
			...
			for(var i in storage.slots){
				_storage.slots[i] = {
					id:storage.slots[i].id,
					data:storage.slots[i].data,
					count:storage.slots[i].count
				};
		...
		this.read = function(_storage){
			...
			if(_storage.slots){
				for(var i in _storage.slots){
					storage.slots[i] = {
						id:_storage.slots[i].id,
						data:_storage.slots[i].data,
						count:_storage.slots[i].count,
					};
	...
	IWindow:function(){
		...
		this.addElement = function(name, element){
			...
			var x = 0;
			if(this.inventory)
				x = this.inventory.getWidth() + GUI.getUnitsInPixels(30) || 0;
	...
	TabsWindow:function(settings){
		...
		this.onCreate = function(){
			...
			exitButton.setOnClick(
				settings.onClickExitButton || function(container){container.close();}
			);
			params = new RelativeLayout.LayoutParams(
				GUI.getUnitsInPixels(60),
				GUI.getUnitsInPixels(60)
			)
	...
	Slot:function(settings){
		...
		this.isValid = settings.isValid || function(){return true};
		...
		this.setSlot = function(id, count, data, extra){
			...
			this.onChange(this, {id:id, count:count, data:data, extra:extra}, oldItem);
		...
		this.addItem = function(id, count, data, extra){
			...
			var container = this.getWindow().getContainer();
			if(this.isValid(id, count, data, extra, this, container) !== true) return false;
			
			return this.getWindow().getContainer().setSlot(this.name, id, this.count + count, this.data, extra);
		}
