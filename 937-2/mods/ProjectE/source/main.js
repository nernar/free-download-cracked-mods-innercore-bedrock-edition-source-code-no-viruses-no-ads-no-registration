...
//create Reider ___ size - 16
var table_model = (function (obj, texture_default, data_default) {
    obj = obj || {};
    var texture = texture_default || 1, data = data_default || 0;
    var model = new RenderUtil.Model();
...
//create Reider ___ size - 16
var pedestal = (function (obj, texture_default, data_default) {
    obj = obj || {};
    var texture = texture_default || 1, data = data_default || 0;
    var model = new RenderUtil.Model();
...
Saver.addSavesScope("ProjectE", function read(scope) {
...
var color = "§" + __config__.get("color_emc");
var Core = {
    emc: {},
    players: {},
    setEmc: function (player, emc) {
        this.players[player] = emc;
    },
    getEmc: function (player) {
        return this.players[player];
    },
    addEmc: function (player, emc) {
        this.setEmc(player, this.getEmc(player) + emc);
    },
    setItemEmc: function (id, data, emc) {
        if (!id)
            return;
        data = (data || 0) == -1 ? 0 : (data || 0);
        this.emc[id + ":" + data] = emc;
    },
    deleteItemEmc: function (id, data) {
        delete this.emc[id + ":" + data];
        //ItemName.setName(id, "", true);
    },
    getItemEmc: function (id, data) {
        data = (data || 0) == -1 ? 0 : (data || 0);
        var storage = this.stars[id];
        if (!storage) {
    ...
    recipes: {},
    addRecipe: function (id, count, data, items) {
        this.recipes[id] = this.recipes[id] || {};
        this.recipes[id][data] = [count, items];
    },
    deleteRecipe: function (id, data) {
        this.recipes[id] = this.recipes[id] || {};
    ...
    updateItemEmc: function (id, count, data, items, op) {
        op = op || 0;
        op++;
        if (op >= 50)
            return false;
        ...
        for (var i in items) {
            var item = items[i];
            var emc = Core.getItemEmc(item.id, item.data);
            if (!emc) {
                var recipe = this.getRecipe(item.id, item.data);
        ...
        var item_emc = Core.getItemEmc(id, data);
        if (!item_emc) {
    ...
    updateRecipesEmc: function () {
        var keys = Object.keys(this.recipes);
        for (var i in keys) {
            var datas = Object.keys(this.recipes[keys[i]]);
            for (var ii in datas) {
                var recipe = this.getRecipe(keys[i], datas[ii]);
                this.updateItemEmc(parseInt(keys[i]), recipe[0], parseInt(datas[ii]), recipe[1]);
            }
        }
    },
    isItemEmc: function (id, data) {
        return !!this.getItemEmc(id, data);
    },
    stars: {},
    registerStar: function (id, storage) {
        ...
        Item.registerNameOverrideFunction(id, function (item, name) {
            var emc_default = Core.getItemEmc(item.id, storage) || 0;
            var emc = Core.getItemEmc(item.id, item.data) || 0;
            return name + "\n§7EMC: " + (emc) + "/" + (storage + emc_default);
        });
    },
    getStorageStar: function (id) {
        return this.stars[id];
    },
    updateStarSlot: function (self, name, value) {
        value = value || 1;
        var slot = self.container.getSlot(name);
        var storage = this.getStorageStar(slot.id);
        if (!!storage) {
            var a = Math.min(self.data.emc, value);
            if (slot.data - a >= 0)
    ...
    loadFromFile: function (path) {
        var arr = FileTools.ReadJSON(path);
        for (var i in arr)
            this.setItemEmc(eval(arr[i].id), arr[i].data, arr[i].emc);
    },
    _loadFromFile: function (path, type) {
        var arr = FileTools.ReadText(path);
        arr = arr.split("\n");
        var _loop_1 = function (i) {
            var data = arr[i].split("===");
            var block = data[0].split(":");
            var id = (function () {
            ...
            this_1.setItemEmc(id, parseInt(block[1]), parseInt(data[1]));
        };
        var this_1 = this;
        for (var i in arr) {
            _loop_1(i);
    ...
    getTextByArr: function (prefix, begin, end) {
        var arr = [];
    ...
    collectors: {},
    setResultCollector: function (id, data, r_id, r_data, emc) {
    ...
    getResultCollector: function (id, data) {
        return this.collectors[id + ":" + data];
    },
    collectorBlocks: {},
    isCollector: function (id) {
        return !!this.collectorBlocks[id];
    },
    transfers: {},
    setTransfer: function (id, bool) {
        this.transfers[id] = bool;
    },
    getTransfers: function (self, transfers) {
        var result = [];
        var arr = [
            { x: -1, y: 0, z: 0 },
            { x: 1, y: 0, z: 0 },
        ...
        for (var i in arr) {
            ...
            var id = self.blockSource.getBlockId(pos.x, pos.y, pos.z);
            if (!!transfers[id])
                result.push(pos);
        }
        return result;
    },
    transfer: function (self, transfers) {
        transfers = transfers || this.transfers;
        var arr = this.getTransfers(self, transfers);
        for (var i in arr) {
            var tile = TileEntity.getTileEntity(arr[i].x, arr[i].y, arr[i].z, self.blockSource);
            if (tile && tile.data && tile.data.active) {
                var value = Math.min(self.data.emc / arr.length, self.data.emc_out);
    ...
    inputSlotsEmc: function (self, slots, black) {
        black = black || [];
        for (var i in slots) {
            var slot = self.container.getSlot(slots[i]);
            if (black.indexOf(slot.id) != -1)
                continue;
            var emc = Core.getItemEmc(slot.id, slot.data);
            if (emc) {
                self.data.emc += emc;
    ...
    registerDefaultCollector: function (id, obj) {
        ...
        TileEntity.registerPrototype(id, {
            ...
            tick: function () {
                var sun = this.blockSource.getLightLevel(this.x, this.y + 1, this.z) / 15;
                ...
                var input = this.container.getSlot(obj.input);
                var output = this.container.getSlot(obj.output);
                var result = Core.getResultCollector(input.id, input.data);
    ...
    setDefaultRelay: function (id, obj) {
        obj.emc_out = obj.emc_out || 16;
        obj.emc_max = obj.emc_max || 10000;
        obj.emc_text = obj.emc_text || "emc_text";
        obj.emc_scale = obj.emc_scale || "emc_scale";
    ...
    condenser: {},
    isCondenser: function (id) {
        return this.condenser[id];
    },
    setDefaultCondenser: function (id, obj) {
        obj.slots = obj.slots || [];
        obj.emc_text = obj.emc_text || "emc_text";
        obj.emc_scale = obj.emc_scale || "emc_scale";
        ...
        TileEntity.registerPrototype(id, {
            ...
            add: function (item) {
                for (var i in obj.slots) {
                    var slot = this.container.getSlot(obj.slots[i]);
            ...
            tick: function () {
                ...
                this.container.setText(obj.emc_text, Math.floor(this.data.emc) + " EMC");
                var item = this.container.getSlot("item");
                var emc = Core.getItemEmc(item.id, item.data);
        ...
        var _slots = {};
        for (var i in obj.slots)
            _slots[obj.slots[i]] = { input: true, output: true };
...
Callback.addCallback("LevelLoaded", function () {
    ...
    var _loop_2 = function () {
        ...
        Core.addRecipe(result.id, result.count, result.data, (function () {
            var items = recipe.getSortedEntries();
            var arr = [];
            var obj = {};
    ...
    /*this.emc[id+":"+data] = emc;
            if(!this.stars[id]){
                //ItemName.setName(id, data, "\n"+color+" EMC "+ Core.emc[id+":"+data], true);
                ToolTip.addToolTip(id, 0, color+" EMC "+ Core.emc[id+":"+data]);
            }*/
    for (var key in Core.emc) {
        var datas = key.split(":");
        ToolTip.addToolTip(datas[0], datas[1], color + " EMC " + Core.emc[key]);
    }
}, -5);
...
Network.addServerPacket("ProjectE.transmutate", function (client, data) {
    ...
    var emc_p = Core.getEmc(player) || 0;
    var container = Table.getContainer(player);
    var slot = container.getSlot(data.slot);
    var emc = Core.getItemEmc(slot.id, slot.data);
    if (slot.id == 0 || !emc)
        return;
    var count = Math.floor(emc_p / emc) <= data.count ? Math.floor(emc_p / emc) : data.count;
    if (emc * count <= emc_p) {
...
Network.addServerPacket("ProjectE.addEmc", function (client, data) {
    ...
    var emc_p = Core.getEmc(player) || 0;
    var emc = Core.getItemEmc(slot.id, slot.data);
    var scrutinys = (Table.scrutinys[player] = (Table.scrutinys[player] || {}));
    if (!!emc || ItemID.tomeKnowledge == slot.id) {
        if (ItemID.tomeKnowledge == slot.id) {
            var keys = Object.keys(Core.emc);
            for (var i in keys)
                scrutinys[keys[i]] = 1;
        }
        else {
...
Network.addServerPacket("ProjectE.next", function (client, data) {
    var player = client.getPlayerUid();
    var scrutinys = (Table.scrutinys[player] = (Table.scrutinys[player] || {}));
    var count = Object.keys(scrutinys).length;
...
Network.addServerPacket("ProjectE.del", function (client, data) {
    ...
    var slot = container.getSlot(data.slot);
    var scrutinys = (Table.scrutinys[player] = (Table.scrutinys[player] || {}));
    delete scrutinys[slot.id + ":" + slot.data];
    Table.updatePage(Table.pages[player], data.slots, player);
});
var Table = {
    players: {},
    pages: {},
    scrutinys: {},
    getContainer: function (player, type) {
        var container = this.players[player];
        if (!container) {
            container = this.players[player] = new ItemContainer();
            container.setClientContainerTypeName(type);
            container.addServerCloseListener(function (cont, client) {
                Table.drop(client.getPlayerUid());
            });
        }
        return container;
    },
    open: function (player, type, slots) {
        var cont = this.getContainer(player, type);
        Table.pages[player] = 0;
    ...
    addEmc: function (slot, slots) {
        Network.sendToServer("ProjectE.addEmc", {
            slot: slot,
            slots: slots
        });
    },
    del: function (slot, slots) {
    ...
    updatePage: function (page, slots, player) {
        page = page || 0;
        var scrutinys = (Table.scrutinys[player] = (Table.scrutinys[player] || {}));
        var keys = Object.keys(scrutinys);
        var arr = [];
        for (var i in keys) {
            var data = keys[i].split(":");
            var emc = Core.getItemEmc(data[0], data[1]);
            var emc_p = Core.getEmc(player);
            if (emc <= emc_p && scrutinys[keys[i]])
                arr.push(keys[i]);
        }
        var container = this.getContainer(player);
    ...
    transmutate: function (slot, count, slots) {
    ...
    drop: function (player, pos) {
        pos = pos || Entity.getPosition(player);
        var container = this.getContainer(player);
        var region = BlockSource.getDefaultForActor(player);
        container.dropSlot(region, "unlearn", pos.x, pos.y, pos.z);
        container.dropSlot(region, "burn", pos.x, pos.y, pos.z);
    }
};
...
var table_elements = {
    "burn": { type: "slot", x: 582, y: 400, bitmap: "burn", onItemChanged: function () {
            Table.addEmc("burn", 12);
        } },
    "unlearn": { type: "slot", x: 518, y: 400, bitmap: "slotUnlearn", onItemChanged: function () {
...
var Pedestal = {
    funcs: {},
    register: function (id, func, active) {
...
TileEntity.registerPrototype(BlockID.pedestalMatter, {
    ...
    click: function (id, count, data, coords, player) {
        ...
        if (id == 0 && this.data.item.id != 0 && Entity.getSneaking(player)) {
            ...
            if (this.data.active)
                Pedestal.get(this.data.item.id).active(this);
        }
    ...
    spawnParticle: function () {
        ...
        group.add("flame", this.x + .2, this.y + .3, this.z + .2);
        group.add("flame", this.x + .8, this.y + .3, this.z + .8);
        group.add("flame", this.x + .8, this.y + .3, this.z + .2);
        group.add("flame", this.x + .2, this.y + .3, this.z + .8);
        group.add("flame", this.x + .5, this.y + .3, this.z + .2);
        group.add("flame", this.x + .2, this.y + .3, this.z + .5);
        group.add("flame", this.x + .5, this.y + .3, this.z + .8);
        group.add("flame", this.x + .8, this.y + .3, this.z + .5);
        group.send(this.blockSource);
    },
    tick: function () {
        if (this.data.active) {
            World.getThreadTime() % 4 == 0 && this.spawnParticle();
            var tick = Pedestal.get(this.data.item.id).tick;
            tick && tick(this);
        }
    }
});
...
Pedestal.register(ItemID.soulStone, function (tile) {
    var player = tile.data.player;
    if (World.getThreadTime() % 10 == 0 && Entity.getHealth(player) < 20) {
        var distance = Entity.getDistanceToCoords(player, { x: tile.x, y: tile.y, z: tile.z });
        if (distance <= 11)
...
var update_watchTime = 20;
var radius_watchTime = 4;
function update_tiles(tile) {
    tile.data.tiles = [];
    for (var x = -radius_watchTime; x < radius_watchTime; x++)
        for (var y = -radius_watchTime; y < radius_watchTime; y++)
            for (var z = -radius_watchTime; z < radius_watchTime; z++) {
                var block = tile.blockSource.getBlockId(tile.x + x, tile.y + y, tile.z + z);
                var _tile = TileEntity.getTileEntity(tile.x + x, tile.y + y, tile.z + z, tile.blockSource);
                block != BlockID.pedestalMatter && !!_tile && _tile.tick && tile.data.tiles.push([x, y, z]);
            }
}
Pedestal.register(ItemID.watchTime, function (tile) {
    if (tile.data.tiles) {
        if (World.getThreadTime() % update_watchTime != 0)
            return;
        try {
            for (var i in tile.data.tiles) {
                var pos = tile.data.tiles[i];
                var _tile = TileEntity.getTileEntity(tile.x + pos[0], tile.y + pos[1], tile.z + pos[2], tile.blockSource);
                for (var i_1 = 0; i_1 < update_watchTime; i_1++)
                    _tile.tick();
            }
        }
        catch (e) {
    ...
    for (var xx = -1; xx <= 1; xx++)
        for (var yy = -1; yy <= 1; yy++)
            for (var zz = -1; zz <= 1; zz++) {
                var val = ore_emc[region.getBlockId(c.x + xx, c.y + yy, c.z + zz)];
                if (val)
                    total += val;
            }
...
Callback.addCallback("ServerPlayerTick", function (player) {
    if (Entity.getArmorSlot(player, 0).id == ItemID.gemHelm &&
...
Item.registerUseFunction("dmHammer", function (coords, item, block, player) {
    ...
    if (!Entity.getSneaking(player))
        for (xx = -rr; xx <= rr; xx++)
            for (yy = -rr; yy <= rr; yy++)
                for (zz = -rr; zz <= rr; zz++) {
                    var block_1 = region.getBlock(x + xx, y + yy, z + zz);
                    if (!UNBREAKABLE[block_1.id]) {
                        var drop = getBlockDrop({ x: x + xx, y: y + yy, z: z + zz }, block_1.id, block_1.data, 5);
                        if (drop) {
                            for (i in drop) {
                                try {
...
Item.registerUseFunction("rmHammer", function (coords, item, block, player) {
    ...
    if (!Entity.getSneaking(player))
        for (xx = -rr; xx <= rr; xx++)
            for (yy = -rr; yy <= rr; yy++)
                for (zz = -rr; zz <= rr; zz++) {
                    var block_2 = region.getBlock(x + xx, y + yy, z + zz);
                    if (!UNBREAKABLE[block_2.id]) {
                        var drop = getBlockDrop({ x: x + xx, y: y + yy, z: z + zz }, block_2.id, block_2.data, 5);
                        if (drop) {
                            for (i in drop) {
                                try {
                                    new PlayerActor(player).addItemToInventory(drop[i][0], drop[i][1] || 1, drop[i][2] || 0, null, true);
    ...
    useItem: function (c, i, b, player) {
        if (b.id == 17 || b.id == 162) {
            ...
            for (x = -rad; x <= rad; x++)
                for (y = -2; y <= rad; y++)
                    for (z = -rad; z <= rad; z++) {
                        var block = region.getBlock(c.x + x, c.y + y, c.z + z);
                        if (block.id == b.id) {
    ...
    useItem: function (coords, item, block, player) {
        ...
        if (!Entity.getSneaking(player)) {
            ...
            for (xx = -rr; xx <= rr; xx++) {
                for (yy = -rr; yy <= rr; yy++) {
                    for (zz = -rr; zz <= rr; zz++) {
                        var block_3 = region.getBlock(x + xx, y + yy, z + zz);
                        if (!UNBREAKABLE[block_3.id]) {
                            var drop = getBlockDrop({ x: x + xx, y: y + yy, z: z + zz }, block_3.id, block_3.data, 5);
                            if (drop) {
                                for (i in drop) {
                                    try {
                                        new PlayerActor(player).addItemToInventory(drop[i][0], drop[i][1] || 1, drop[i][2] || 0, null, true);
...
function addPhilosophersStoneRecipe(item, input) {
    ...
    Recipes.addCraftToolRecipeItem({ id: item.id, count: item.count, data: item.data }, ingridients, ItemID.philosophersStone);
}
...
Callback.addCallback("PostEmcLoad", function () {
    ...
    Recipes.addCraftToolRecipeItem({ id: ItemID.fuelAlchemical, count: 1, data: 0 }, [
        { id: 263, data: 0 },
        { id: 263, data: 0 },
        { id: 263, data: 0 },
        { id: 263, data: 0 },
    ], ItemID.philosophersStone);
    Recipes.addCraftToolRecipeItem({ id: ItemID.fuelMobius, count: 1, data: 0 }, [
        { id: ItemID.fuelAlchemical, data: 0 },
        { id: ItemID.fuelAlchemical, data: 0 },
        { id: ItemID.fuelAlchemical, data: 0 },
        { id: ItemID.fuelAlchemical, data: 0 },
    ], ItemID.philosophersStone);
    Recipes.addCraftToolRecipeItem({ id: ItemID.fuelAstral, count: 1, data: 0 }, [
...
Callback.addCallback("PostLoaded", function () {
    ...
    Recipes.addShaped({ id: BlockID.energyCollector3, count: 1, data: 0 }, ["gmg",
        "gcg",
        "ggg"], ['g', 89, 0, 'm', ItemID.redMatter, 0, 'c', BlockID.energyCollector2, 0]);
    Recipes.addShaped({ id: BlockID.energyCollector2, count: 1, data: 0 }, ["gmg",
        "gcg",
        "ggg"], ['g', 89, 0, 'm', ItemID.darkMatter, 0, 'c', BlockID.energyCollector1, 0]);
    Recipes.addShaped({ id: BlockID.energyCollector1, count: 1, data: 0 }, ["glg",
        "gdg",
        "gfg"], ['g', 89, 0, 'd', 57, 0, 'f', 61, 0, 'l', 20, 0]);
    Recipes.addShaped({ id: ItemID.dmHelm, count: 1, data: 0 }, ["aaa",
        "a a",
        "   "], ["a", ItemID.darkMatter, 0]);
    Recipes.addShaped({ id: ItemID.dmChest, count: 1, data: 0 }, ["a a",
        "aaa",
        "aaa"], ["a", ItemID.darkMatter, 0]);
    Recipes.addShaped({ id: ItemID.dmLegg, count: 1, data: 0 }, ["aaa",
        "a a",
        "a a"], ["a", ItemID.darkMatter, 0]);
    Recipes.addShaped({ id: ItemID.dmBoots, count: 1, data: 0 }, ["a a",
        "a a",
        "   "], ["a", ItemID.darkMatter, 0]);
    Recipes.addShaped({ id: ItemID.dmPickaxe, count: 1, data: 0 }, ["aaa",
        " d ",
        " d "], ["a", ItemID.darkMatter, 0, "d", 264, 0]);
    Recipes.addShaped({ id: ItemID.dmAxe, count: 1, data: 0 }, ["aa",
        "ad",
        " d"], ["a", ItemID.darkMatter, 0, "d", 264, 0]);
    Recipes.addShaped({ id: ItemID.dmShovel, count: 1, data: 0 }, ["a",
        "d",
        "d"], ["a", ItemID.darkMatter, 0, "d", 264, 0]);
    Recipes.addShaped({ id: ItemID.dmSword, count: 1, data: 0 }, ["a",
        "a",
        "d"], ["a", ItemID.darkMatter, 0, "d", 264, 0]);
    Recipes.addShaped({ id: ItemID.rmHelm, count: 1, data: 0 }, ["aaa",
        "ada",
        "   "], ["a", ItemID.redMatter, 0, "d", ItemID.dmHelm, 0]);
    Recipes.addShaped({ id: ItemID.rmChest, count: 1, data: 0 }, ["ada",
        "aaa",
        "aaa"], ["a", ItemID.redMatter, 0, "d", ItemID.dmChest, 0]);
    Recipes.addShaped({ id: ItemID.rmLegg, count: 1, data: 0 }, ["aaa",
        "ada",
        "a a"], ["a", ItemID.redMatter, 0, "d", ItemID.dmLegg, 0]);
    Recipes.addShaped({ id: ItemID.rmBoots, count: 1, data: 0 }, ["ada",
        "a a",
        "   "], ["a", ItemID.redMatter, 0, "d", ItemID.dmBoots, 0]);
    Recipes.addShaped({ id: ItemID.rmPickaxe, count: 1, data: 0 }, ["aaa",
        " m ",
        " d "], ["a", ItemID.redMatter, 0, "d", ItemID.darkMatter, 0, "m", ItemID.dmPickaxe, 0]);
    Recipes.addShaped({ id: ItemID.rmAxe, count: 1, data: 0 }, ["aa",
        "am",
        " d"], ["a", ItemID.redMatter, 0, "d", ItemID.darkMatter, 0, "m", ItemID.dmAxe, 0]);
    Recipes.addShaped({ id: ItemID.rmShovel, count: 1, data: 0 }, ["a",
        "m",
        "d"], ["a", ItemID.redMatter, 0, "d", ItemID.darkMatter, 0, "m", ItemID.dmShovel, 0]);
    Recipes.addShaped({ id: ItemID.rmSword, count: 1, data: 0 }, ["a",
        "m",
        "d"], ["a", ItemID.redMatter, 0, "d", ItemID.darkMatter, 0, "m", ItemID.dmSword, 0]);
    Recipes.addShaped({ id: ItemID.rmKatar, count: 1, data: 0 }, ["sar",
        "rrr",
        "rrr"], ["s", ItemID.rmSword, 0, "a", ItemID.rmAxe, 0, "r", ItemID.redMatter, 0]);
    Recipes.addShaped({ id: ItemID.rmMorningStar, count: 1, data: 0 }, ["sar",
        "hrr",
        "rrr"], ["h", ItemID.rmHammer, 0, "s", ItemID.rmShovel, 0, "a", ItemID.rmPickaxe, 0, "r", ItemID.redMatter, 0]);
    Recipes.addShaped({
    ...
    Recipes.addShaped({ id: BlockID.energyCondenser1, count: 1, data: 0 }, ["odo",
        "dcd",
        "odo"], ["o", 49, 0, "d", 264, 0, "c", BlockID.alchChest, 0]);
    Recipes.addShaped({ id: BlockID.energyCondenser2, count: 1, data: 0 }, ["odo",
        "dcd",
        "odo"], ["o", BlockID.dmBlock, 0, "d", BlockID.rmBlock, 0, "c", BlockID.energyCondenser1, 0]);
    Recipes.addShaped({
    ...
    Recipes.addShaped({ id: BlockID.antimatterRelay2, count: 1, data: 0 }, ["odo",
        "ofo",
        "ooo"], ["o", 49, 0, "f", BlockID.antimatterRelay1, 0, "d", ItemID.darkMatter, 0]);
    Recipes.addShaped({ id: BlockID.antimatterRelay3, count: 1, data: 0 }, ["odo",
        "ofo",
        "ooo"], ["o", 49, 0, "f", BlockID.antimatterRelay2, 0, "d", ItemID.redMatter, 0]);
    Recipes.addShaped({ id: ItemID.philosophersStone, count: 1, data: 0 }, ["rgr",
        "gdg",
        "rgr"], ["r", 331, 0, "g", 348, 0, "d", 264, 0]);
    Recipes.addShaped({ id: ItemID.ironBand, count: 1, data: 0 }, ["iii",
        "ili",
        "iii"], ["l", 325, 10, "i", 265, 0]);
    Recipes.addShaped({ id: ItemID.darkMatter, count: 1, data: 0 }, ["aaa",
        "aba",
        "aaa"], ["a", ItemID.fuelAstral, 0, "b", 57, 0]);
    Recipes.addShaped({ id: ItemID.redMatter, count: 1, data: 0 }, ["aaa",
        "bbb",
        "aaa"], ["a", ItemID.fuelAstral, 0, "b", ItemID.darkMatter, 0]);
    Recipes.addShaped({ id: ItemID.covDust1, count: 40, data: 0 }, ["sii",
        "iii",
        "iii"], ["s", 263, 1, "i", 4, 0]);
    Recipes.addShaped({ id: ItemID.rodDivining1, count: 1, data: 0 }, ["iii",
        "isi",
        "iii"], ["s", 280, 0, "i", ItemID.covDust1, 0]);
    Recipes.addShaped({ id: ItemID.rodDivining2, count: 1, data: 0 }, ["iii",
        "isi",
        "iii"], ["s", ItemID.rodDivining1, 0, "i", ItemID.covDust2, 0]);
    Recipes.addShaped({ id: ItemID.rodDivining3, count: 1, data: 0 }, ["iii",
        "isi",
        "iii"], ["s", ItemID.rodDivining2, 0, "i", ItemID.covDust3, 0]);
    ...
    Recipes.addShaped({ id: BlockID.blockAlchemicalFuel, count: 1, data: 0 }, ["aaa",
        "aaa",
        "aaa"], ["a", ItemID.fuelAlchemical, 0]);
    Recipes.addShaped({ id: BlockID.blockMobiusFuel, count: 1, data: 0 }, ["aaa",
        "aaa",
        "aaa"], ["a", ItemID.fuelMobius, 0]);
    Recipes.addShaped({ id: BlockID.blockAstralFuel, count: 1, data: 0 }, ["aaa",
        "aaa",
        "aaa"], ["a", ItemID.fuelAstral, 0]);
    ...
    Recipes.addShaped({ id: BlockID.pedestalMatter, count: 1, data: 0 }, ["rir",
        "rir",
        "iii"], ["r", ItemID.redMatter, 0, "i", BlockID.dmBlock, 0]);
    Recipes.addShaped({ id: ItemID.soulStone, count: 1, data: 0 }, ["sss",
        "dbd",
        "sss"], ["b", 351, 4, "d", ItemID.redMatter, 0, "s", 348, 0]);
    Recipes.addShaped({ id: ItemID.watchTime, count: 1, data: 0 }, ["dsd",
        "gcg",
        "dsd"], ["c", 351, 4, "g", 89, 0, "c", 347, 0, "d", ItemID.darkMatter, 0, "s", 49, 0]);
...
var custom = FileTools.ReadJSON(__dir__ + "/custom.js");
Callback.addCallback("NativeCommand", function (str) {
    var arr = str.split(" ");
    if (arr[0] == "/emc") {
        var item_1 = Entity.getCarriedItem(Player.get());
        custom.push({
            id: (function () {
                var items = Object.keys(ItemID);
                for (var i in items)
                    if (ItemID[items[i]] == item_1.id)
                        return "ItemID." + items[i];
                items = Object.keys(BlockID);
                for (var i in items)
                    if (BlockID[items[i]] == Block.convertBlockToItemId(item_1.id))
        ...
        Core.setItemEmc(item_1.id, item_1.data, parseInt(arr[1]));
        Game.prevent();
        Game.message(":D");
    }
    else if (arr[0] == "/code") {
...
ModAPI.addAPICallback("RecipeViewer", function (api) {
    var Collector = (function (_super) {
        ...
        function Collector(name, icon, key, content) {
            var _this = _super.call(this, name, icon, content) || this;
            _this.ritualKey = key;
            return _this;
        }
        Collector.prototype.getAllList = function () {
            var recipes = Core.collectors;
            return (function () {
                var list = [];
                var keys = Object.keys(recipes);
                for (var i in keys) {
                    var item = keys[i].split(":");
                    var result = recipes[keys[i]];
                    list.push({
