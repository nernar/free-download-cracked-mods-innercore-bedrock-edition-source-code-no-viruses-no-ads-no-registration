var fogParticle = Particles.registerParticleType({texture: "fog", size: [10, 20], lifetime: [20, 20], render: 2});
var fog = false;
var FOG_DENCITY = __config__.getNumber("fog_density");
var Fog = {ticks: 0, tick: function () {
    this.ticks++;
    if (this.ticks % 200 == 0) {
...
if (__config__.getBool("weather")) {
    RandomEvents.registerTimedEvent({name: "toxic_fog", frequency: 0.0001, time: 1000, enable: function () {
    }, tick: function () {
        Fog.tick();
