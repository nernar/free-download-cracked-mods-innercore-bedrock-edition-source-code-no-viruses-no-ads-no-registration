...
function Building(filename) {
    ...
    this.size = {x: this.blocks[0].length, y: this.blocks.length, z: this.blocks[0][0].length};
...
var BuildingsSystem = {staticidsFile: FileTools.getFullPath("games/com.mojang/mods/.staticids"), checksumFile: FileTools.getFullPath("games/com.mojang/innercore/cache/checksum.txt"), cacheFile: FileTools.getFullPath("games/com.mojang/innercore/cache/apo.json"), build: function (building, x1, y1, z1, edit) {
    var randoms = [];
    for (var key in building.json.randomizer) {
        ...
        randoms.push({"block": {id: randomizer.block.id, data: randomizer.block.data || randomizer.block.meta || 0}, "variation": {id: variation.id, data: variation.data || variation.meta || 0}});
    }
    if (DEBUG) {
                    ...
                    if (container != null) {
                        for (var key in building.loot) {
                            ...
                            if (Math.random() < item.chance) {
                                ...
                                if (!item.id) {
                                    Game.message("Error adding item n\xb0" + key + " to generated chest");
                                } else {
...
if (DEBUG) {
    ...
    ModAPI.addAPICallback("WorldEdit", function (api) {
        ...
        WorldEditCommands["//save"] = {name: "//save", description: "Saves A.P.O. Craft building to file", args: "", func: function () {
    ...
    var BuildingEditor = {edit: function (x, y, z) {
        runAsUI(function () {
            var items = [];
            for (i in buildings) {
            ...
            dialog.setItems(items, function (d, item) {
                ...
                WorldEdit.selectPosition({x: x, y: y, z: z}, {x: x + BuildingEditor.current.size.x, y: y + BuildingEditor.current.size.y, z: z + BuildingEditor.current.size.z});
