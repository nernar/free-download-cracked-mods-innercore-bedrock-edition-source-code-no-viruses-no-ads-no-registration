...
Saver.addSavesScope("exoskeleton", function read(scope) {
    if (!scope.coords) {
        return;
    }
    Exoskeleton.coords = [];
...
Callback.addCallback("tick", function () {
    if (World.getThreadTime() % 5 === 0) {
        ...
        if (exo) {
            if (!(slots[0] == armorExo[0] && slots[1] == armorExo[1] && slots[2] == armorExo[2] && slots[3] == armorExo[3])) {
                for (var i = 0; i < 4; i++) {
        ...
        Exoskeleton.coords = Exoskeleton.coords.filter(function (obj) {
            ...
            if (player.x < exoskeleton.x + 1 && player.x > exoskeleton.x - 1 && player.y < exoskeleton.y + 1 && player.y > exoskeleton.y - 1 && player.z < exoskeleton.z + 1 && player.z > exoskeleton.z - 1) {
                near = true;
                if (empty) {
