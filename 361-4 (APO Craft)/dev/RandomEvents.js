var RandomEvents = {events: [], registerEvent: function (eventName, eventFrequency, eventCallback) {
    ...
    var x, y, z;
    if (Math.random() > 0.5) {
        x = coords.x + Math.random() * 16 + 6;
    } else {
        x = coords.x - Math.random() * 16 - 6;
    }
    if (Math.random() > 0.5) {
        y = coords.y + Math.random() * 16 + 6;
    } else {
        y = coords.y - Math.random() * 16 - 6;
    }
    if (Math.random() > 0.5) {
        z = coords.z + Math.random() * 16 + 6;
    } else {
        z = coords.z - Math.random() * 16 - 6;
    }
    return {"x": x, "y": y, "z": z};
