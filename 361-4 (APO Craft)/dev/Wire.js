...
Block.createBlock("hvTransformer", [{name: "High-Voltage Transformer", texture: [["std_bottom", 0], ["std_top", 0], ["hv_transformer", 0], ["hv_transformer", 0], ["hv_transformer", 0], ["hv_transformer", 0]], inCreative: true}]);
ICRender.getGroup("ic-transformer").add(BlockID.hvTransformer, -1);
TileEntity.registerPrototype(BlockID.hvTransformer, {defaultValues: {transmitter: false, energy: 0, consumers: [], nodes: []}, click: function (id, count, data) {
    if (id == BlockID.hvConnector) {
        return true;
    }
...
Block.createBlock("hvConnector", [{name: "High-Voltage Connector", texture: [["hv_connector", 0]], inCreative: true}]);
...
Callback.addCallback("DestroyBlock", function (coords, block, player) {
    if (block.id == BlockID.hvConnector) {
        ...
        WireSystem.wires = WireSystem.wires.filter(function (wire) {
            ...
            if ((wc1.x == coords.x && wc1.y == coords.y && wc1.z == coords.z) || (wc2.x == coords.x && wc2.y == coords.y && wc2.z == coords.z)) {
...
var WireSystem = {connector1: undefined, wires: [], setupWire: function (pos1, pos2) {
    ...
    let b1 = coords2[0];
    let b2 = coords2[1];
    return (a1.x == b1.x && a1.y == b1.y && a1.z == b1.z && a2.x == b2.x && a2.y == b2.y && a2.z == b2.z) || (a1.x == b2.x && a1.y == b2.y && a1.z == b2.z && a2.x == b1.x && a2.y == b1.y && a2.z == b1.z);
    ...
    render.setPart("wire", [{type: "box", coords: {x: 0, y: 0, z: 0}, size: {x: distance * 16, y: 1, z: 1}}], {});
    ...
    for (var i in WireSystem.wires) {
        ...
        if (wc1.x == coords.x && wc1.y == coords.y && wc1.z == coords.z) {
            ...
            if (wc2.x == coords.x && wc2.y == coords.y && wc2.z == coords.z) {
...
Saver.addSavesScope("wires", function read(scope) {
    WireSystem.wires = [];
