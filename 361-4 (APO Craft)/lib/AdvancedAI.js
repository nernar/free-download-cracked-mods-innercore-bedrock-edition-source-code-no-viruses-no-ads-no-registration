...
function findTarget(x, y, z, range) {
    ...
    if (Entity.isExist(target) && isLiving(target)) {
        return target;
    }
}
...
var AdvancedAI = {EnemyWatcher: new EntityAIWatcher({execute: function () {
...
}, params: {attackAI: "attack", followAI: "follow", find_delay: 20, priority_on_attack: 5, priority_on_idle: 0, feelingModifier: 10}}), Shooting: new EntityAIClass({execute: function () {
...
}, params: {ammo_type: Native.EntityType.FIREBALL, shoot_speed: 20, isQueue: false, queue_length: 3, projectile_speed: 0.2, queue_delay: 40}, shoot: function (attacker, ammo) {
...
}}), PhaseWatcher: new EntityAIWatcher({execute: function () {
    var i = 0;
...
}, params: {phases: []}}), PlayerWatcher: new EntityAIWatcher({execute: function () {
...
}, params: {ai: []}}), Summoning: new EntityAIClass({execute: function () {
...
}, params: {entity: Native.EntityType.ZOMBIE, spawn_area: 2, summon_delay: 30}}), Guarding: new EntityAIWatcher({execute: function () {
    if (World.getThreadTime() % this.params.find_delay == 0) {
        var coords = Entity.getPosition(this.entity);
        var attackAI = this.getAI(this.params.attackAI);
        var followAI = this.getAI(this.params.followAI);
...
}, params: {attackAI: "attack", followAI: "follow", find_delay: 50, priority_on_attack: 5, priority_on_idle: 0, feelingModifier: 10}}), Lifetimer: new EntityAIClass({getDefaultPriority: function () {
    return -1;
}, execute: function () {
