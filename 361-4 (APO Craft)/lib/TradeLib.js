LIBRARY({name: "TradeLib", version: 1, shared: true, api: "CoreEngine"});
...
function process() {
    ...
    if (source0.id == good.price[0].id && source0.data == good.price[0].data && source0.count >= good.price[0].count && source1.id == good.price[1].id && source1.data == good.price[1].data && source1.count >= good.price[1].count && ((result0.id == good.good.id && result0.data == good.good.data && result0.count + good.good.count <= 64) || result0.id == 0)) {
...
var window = new UI.StandartWindow({standart: {header: {text: {text: "Trading"}}, inventory: {standart: true}, background: {standart: true}}, drawing: [{type: "bitmap", x: 665, y: 100, bitmap: "arrow_right", scale: GUI_BAR_STANDART_SCALE}], elements: {"slotSource0": {type: "slot", x: 450, y: 200}, "iconSource0": {type: "slot", x: 450, y: 100, visual: true, bitmap: "empty", isTransparentBackground: true}, "slotSource1": {type: "slot", x: 520, y: 200}, "iconSource1": {type: "slot", x: 520, y: 100, visual: true, bitmap: "empty", isTransparentBackground: true}, "slotResult0": {type: "slot", x: 800, y: 200}, "iconResult0": {type: "slot", x: 800, y: 100, visual: true, bitmap: "empty", isTransparentBackground: true}, "buttonProcess": {type: "button", x: 665, y: 205, bitmap: "arrow_right", bitmap2: "arrow_right_down", scale: GUI_BAR_STANDART_SCALE, clicker: {onClick: process}}, "btnPrev": {type: "button", x: 390, y: 100, bitmap: "small_arrow_left", bitmap2: "small_arrow_left_down", scale: GUI_BAR_STANDART_SCALE, clicker: {onClick: previous}}, "btnNext": {type: "button", x: 900, y: 100, bitmap: "small_arrow_right", bitmap2: "small_arrow_right_down", scale: GUI_BAR_STANDART_SCALE, clicker: {onClick: next}}}});
var button = new UI.Window({location: {x: 450, y: 400, width: 100, height: 50}, elements: {"0": {type: "button", x: 0, y: 0, bitmap: "btnTrade", scale: 30, clicker: {onClick: function () {
...
TradeLib.registerTrader = function (nameId, goods, goodsChooser) {
    ...
    traders[nameId] = {"goods": goods, "goodsChooser": goodsChooser ? goodsChooser : function (goods, entity) {
        return goods;
    }};
};
