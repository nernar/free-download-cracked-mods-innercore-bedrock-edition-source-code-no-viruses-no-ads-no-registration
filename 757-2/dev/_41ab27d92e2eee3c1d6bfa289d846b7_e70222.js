const FetchTestsSequence = new LogotypeSequence({uncount: function (value) {
    if (value === undefined) {
        return 0;
    }
    let files = Files.listFileNames(Dirs.TESTING, true), scripts = Files.checkFormats(files, ".dns"), index = scripts.indexOf("attach.dns");
    if (index != -1) {
    ...
    let name = Files.getNameWithoutExtension(element), json = DebugEditor.fetchInformation(name);
    if (json !== null) {
        tryout(function () {
