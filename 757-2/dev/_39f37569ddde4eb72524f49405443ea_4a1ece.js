...
const addTextureMod = function (name) {
    return textures.push({name: name, items: new Array()}) - 1;
};
    ...
    handle(function () {
        ...
        if (mod.items.length > 0) {
            ...
            builder.setItems(converted, function (d, i) {
                tryout(function () {
                    let adapter = dialog.getListView().getAdapter(), texture = String(adapter.getItem(i)).split(", ");
            ...
            if (mod.items.length > 1) {
                ...
                edit.addTextChangedListener({onTextChanged: function (text) {
                    tryout(function () {
                        let adapter = dialog.getListView().getAdapter();
                        adapter.getFilter().filter(text, {onFilterComplete: function (count) {
                            tryout(function () {
                                let actor = count > 0 ? new FadeActor() : new SlideActor(Interface.Gravity.TOP);
                                actor.setInterpolator(new DecelerateInterpolator());
                                actor.addListener({onTransitionStart: function (transition) {
                                    tryout(function () {
                                        dialog.getListView().setVisibility(Interface.Visibility.GONE);
                                    });
                                }, onTransitionEnd: function (transition) {
                                    tryout(function () {
                                        dialog.getListView().setVisibility(count > 0 ? Interface.Visibility.VISIBLE : Interface.Visibility.GONE);
                                    });
                                }});
                                actor.setDuration(800);
                                actor.beginDelayedActor(layout);
                                wrong.setVisibility(count > 0 ? Interface.Visibility.GONE : Interface.Visibility.VISIBLE);
                            });
                        }});
                    });
                }});
...
const removeUnusedMappings = function () {
    let current = getCurrentMappings(), saved = getSavedMappings(), hasOne = false;
    for (let i = 0; i < saved.length; i++) {
...
const mapRenderBlock = function (worker) {
    return tryout(function () {
        ...
        let render = new ICRender.Model(), form = new ICRender.CollisionShape(), mapped = worker.Define.getMapped(), models = worker.Renderer.getModels(), collisions = worker.Collision.getModels(), shape = worker.Define.params.shape, hasRenderer = hasCollision = false;
        if (models.length > 0 && selectMode != 10) {
            for (let m = 0; m < models.length; m++) {
                let model = BlockRenderer.createModel(), boxes = models[m].getBoxes();
                for (let i = 0; i < boxes.length; i++) {
                    let box = boxes[i];
                    if (selectMode == 3 && drawSelection) {
                        let selected = BlockEditor.data.renderer;
                        model.addBox(box.x1, box.y1, box.z1, box.x2, box.y2, box.z2, i == selected ? "renderer_select" : "renderer_unselect", Number(transparentBoxes));
                    } else {
                        if (selectMode == 9) {
                            let selected = BlockEditor.data.rendererInst, innersection = models[m].checkBoxesInnersection(selected, i);
                            model.addBox(box.x1, box.y1, box.z1, box.x2, box.y2, box.z2, i == selected ? "renderer_select" : innersection ? "renderer_innersection" : "renderer_unselect", Number(transparentBoxes));
                        } else {
                            let texture = box.texture;
                            if (texture) {
                                if (texture.length > 1) {
                                    model.addBox(box.x1, box.y1, box.z1, box.x2, box.y2, box.z2, box.texture);
                                } else {
                                    model.addBox(box.x1, box.y1, box.z1, box.x2, box.y2, box.z2, box.texture[0][0], box.texture[0][1]);
                                }
                            } else {
                                model.addBox(box.x1, box.y1, box.z1, box.x2, box.y2, box.z2, "missing_block", 0);
                            }
                        }
                    }
                    hasRenderer = true;
        ...
        if (collisions.length > 0 && selectMode != 9) {
            for (let m = 0; m < collisions.length; m++) {
                let model = BlockRenderer.createModel(), collision = form.addEntry(), boxes = collisions[m].getBoxes();
                for (let i = 0; i < boxes.length; i++) {
                    let box = boxes[i];
                    if (selectMode == 11 && drawSelection) {
                        ...
                        if (selectMode == 10) {
                            let selected = BlockEditor.data.collisionInst, innersection = collisions[m].checkBoxesInnersection(selected, i);
                            model.addBox(box.x1, box.y1, box.z1, box.x2, box.y2, box.z2, i == selected ? "renderer_select" : innersection ? "renderer_innersection" : "renderer_unselect", Number(transparentBoxes));
                        } else {
                            if (BlockEditor.data.selected == 2) {
                                model.addBox(box.x1, box.y1, box.z1, box.x2, box.y2, box.z2, "renderer_shape", Number(transparentBoxes));
                            }
                        }
                    }
                    collision.addBox(box.x1, box.y1, box.z1, box.x2, box.y2, box.z2);
                    hasCollision = true;
                }
                render.addEntry(model);
            }
        ...
        if (shape && selectMode != 9 && selectMode != 10) {
            if (selectMode == 5) {
                render.addEntry(new BlockRenderer.Model(shape.x1, shape.y1, shape.z1, shape.x2, shape.y2, shape.z2, "renderer_shape", Number(transparentBoxes)));
            } else {
                if (!hasRenderer && (!hasCollision || BlockEditor.data.selected != 2)) {
                    render.addEntry(new BlockRenderer.Model(shape.x1, shape.y1, shape.z1, shape.x2, shape.y2, shape.z2, "renderer_shape", Number(transparentBoxes)));
                }
            }
        }
...
const convertJsonBlock = function (string, action) {
    ...
    let runned = compileData("(function() {\n" + ModelConverter + "\n" + "myFunction();\nreturn {\nvalue: document.getElementById(\"frm2\").value,\n" + "logged: console.logged.join(\"\\n\")\n};\n})()", "object", {document: {elements: {name: {value: string}, frm2: new Object()}, getElementById: function (id) {
        return this.elements[id];
    }}, console: {logged: new Array(), log: function (e) {
        this.logged.push(e.message);
    }}});
    if (runned instanceof Error) {
        reportError(runned);
    } else {
        if (runned && runned.value) {
            let compiled = compileScript(runned);
            if (compiled && compiled[0]) {
                action && action(compiled[0]);
            } else {
                confirm(translate("Compilation failed"), translate("Converter generated invalid script, they can't be runned.") + " " + translate("Retry compile with another converted model.") + " " + translate("Save exported result?"), function () {
            ...
            if (runned.logged) {
                confirm(translate("Script report"), translate("Script logging have several messages:") + "\n" + runned.logged);
            }
        }
    }
};
    ...
    handleThread(function () {
        ...
        source.forEach(function (element, index) {
            ...
            if (element.define !== undefined) {
                ...
                if (id !== undefined) {
                    if (project.define === undefined) {
                        project.define = new Object();
                    }
            ...
            if (element.renderer !== undefined) {
                ...
                if (model !== undefined) {
                    if (project.renderer === undefined) {
                        project.renderer = new Array();
                    }
            ...
            if (element.collision !== undefined) {
                ...
                if (shape !== undefined) {
                    if (project.collision === undefined) {
                        project.collision = new Array();
                    }
...
const BlockEditor = {data: new Object(), reset: function () {
    ...
    if (!saveCoords) {
        ...
        this.data.worker.Define.params.mapped = copyMappings();
    }
    this.unselect();
}, unselect: function () {
    ...
    this.data.hasRender = this.data.worker.Renderer.getModelCount() > 0 && this.data.worker.Renderer.getModel(0).getBoxCount() > 0;
    this.data.hasCollision = this.data.worker.Collision.getModelCount() > 0 && this.data.worker.Collision.getModel(0).getBoxCount() > 0;
    ...
    button.show();
    let sidebar = new SidebarWindow(), group = sidebar.addGroup("block");
    ...
    if (this.data.selected >= 0) {
        sidebar.select(this.data.selected);
    }
    ...
    category.addItem("blockRemove", translate("Unmap"), function () {
        ...
        if (!hasRemoveMessage) {
            let message = control.addMessage("blockSlice", translate("All mappings will be removed.") + " " + translate("Touch here to confirm."), function () {
    ...
    if (index == -1) {
        index = ProjectProvider.getCount();
    }
    ...
    let name = file.getName(), project = BlockEditor.data.worker.getProject();
    if (name.endsWith(".dnp")) {
        ...
        importProject(file.getPath(), function (result) {
            ...
            selectProjectData(result, function (selected) {
                ...
                mergeConvertedBlock(project, selected, function (output) {
                    ...
                    mapRenderBlock(BlockEditor.data.worker);
                    showHint(translate("Merged success") + " " + translate("as %ss", preround((Date.now() - active) / 1000, 1)));
                });
            }, "block");
        });
    } else {
        if (name.endsWith(".json")) {
            ...
            if (name.endsWith(".ndb")) {
                let active = Date.now();
                handleThread(function () {
                    let obj = compileData(Files.read(file)), result = convertNdb(obj);
                    mergeConvertedBlock(project, result, function (output) {
                        BlockEditor.data.worker.loadProject(output);
                        mapRenderBlock(BlockEditor.data.worker);
                        showHint(translate("Merged success") + " " + translate("as %ss", preround((Date.now() - active) / 1000, 1)));
    ...
    if (name.endsWith(".dnp")) {
        ...
        importProject(file.getPath(), function (result) {
            ...
            selectProjectData(result, function (selected) {
                ...
                showHint(translate("Loaded success") + " " + translate("as %ss", preround((Date.now() - active) / 1000, 1)));
            }, "block", true);
        });
    } else {
        if (name.endsWith(".json")) {
            ...
            convertJsonBlock(Files.read(file), function (result) {
                ...
                showHint(translate("Converted success") + " " + translate("as %ss", preround((Date.now() - active) / 1000, 1)));
            });
        } else {
            if (name.endsWith(".ndb")) {
                let active = Date.now();
                tryout(function () {
                    let current = BlockEditor.data.worker.getProject(), obj = compileData(Files.read(file)), result = convertNdb(obj);
                    BlockEditor.open(result);
                    showHint(translate("Imported success") + " " + translate("as %ss", preround((Date.now() - active) / 1000, 1)));
                });
            } else {
                if (name.endsWith(".js")) {
                    let active = Date.now();
    ...
    let name = (BlockEditor.data.name = i, file.getName()), project = BlockEditor.data.worker.getProject();
    if (name.endsWith(".dnp")) {
            ...
            tryout(function () {
                ...
                converter.executeAsync(function (link, result) {
                    if (link.hasResult()) {
                        ...
                        reportError(link.getLastException());
                    }
                });
            });
        }
...
}, Renderer: {hasSelection: function () {
    ...
    let selected = BlockEditor.data.renderer, box = BlockEditor.data.worker.Renderer.getModel(0).getBox(selected);
    ...
    popup.setOnClickListener(function (index) {
        let values = popup.getAllEditsValues(), side = compileData(values[0], "string").toLowerCase(), orientate = side == "z" ? 2 : side == "y" ? 1 : 0;
    ...
    let edit = popup.addEditElement(translate("Texture"), JSON.stringify(box.texture)).switchVisibility().setBackground("popup");
    let button = popup.addButtonElement(translate("Save"), function () {
        ...
        if (array.lineNumber !== undefined) {
            confirm(translate("Compilation failed"), translate("Looks like, you entered invalid array. Check it with following exception:") + "\n" + array.message);
        } else {
    ...
    confirm(translate("Deleting"), translate("Are you sure want to delete this box?"), function () {
...
}}, Collision: {hasSelection: function () {
    ...
    let selected = BlockEditor.data.collision, box = BlockEditor.data.worker.Collision.getModel(0).getBox(selected);
    let popup = new CoordsPopup();
    popup.setTitle(translate("Scretch"));
    let group = popup.addGroup("x");
    group.setOnChangeListener(function (index, value) {
    ...
    let params = BlockEditor.data.worker.Define.params, shape = params.shape = params.shape || {x1: 0, y1: 0, z1: 0, x2: 1, y2: 1, z2: 1};
    let popup = new CoordsPopup();
    ...
    popup.addButtonElement(translate("Save"), function () {
        let values = popup.getAllEditsValues(), result = compileData(values[0]);
    ...
    popup.addButtonElement(translate("Save"), function () {
        let values = popup.getAllEditsValues(), result = compileData(values[0]);
        if (result instanceof Error) {
            confirm(translate("Compilation failed"), translate("Looks like, you entered invalid object. Check it with following exception:") + "\n" + result.message + "\n\n" + translate("Force save special type?"), function () {
