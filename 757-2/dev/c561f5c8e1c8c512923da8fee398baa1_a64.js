...
SidebarWindow.prototype.addGroup = function (srcOrGroup, action) {
    ...
    let group = srcOrGroup instanceof SidebarWindow.Group ? srcOrGroup : new SidebarWindow.Group(srcOrGroup, action);
...
SidebarWindow.prototype.removeGroup = function (groupOrIndex) {
    ...
    let index = groupOrIndex instanceof SidebarWindow.Group ? this.indexOfGroup(groupOrIndex) : groupOrIndex;
    if (index < 0) {
        return false;
    }
...
SidebarWindow.prototype.reinflateLayout = function () {
    let sidebar = this, fragment = this.getFragment();
    if (fragment !== null) {
        if (this.isSelected()) {
            let set = new ActorSet(), slide = new SlideActor(Interface.Gravity.RIGHT), bounds = new BoundsActor();
            if (this.isTouchable()) {
                set.addListener({onTransitionStart: function (transition) {
                    tryout(function () {
...
SidebarWindow.Group.prototype.attachToWindow = function (window) {
    if (!window) {
        MCSystem.throwException("Window can't be illegal or undefined state!");
    }
    let attached = this.getWindow();
    if (attached != null) {
        MCSystem.throwException("You're must deattach sidebar group firstly!");
    }
...
SidebarWindow.Group.prototype.addItem = function (srcOrItem, action) {
    ...
    let item = srcOrItem instanceof SidebarWindow.Group.Item ? srcOrItem : new SidebarWindow.Group.Item(srcOrItem, action);
...
SidebarWindow.Group.prototype.removeItem = function (itemOrIndex) {
    ...
    let index = itemOrIndex instanceof SidebarWindow.Group.Item ? this.indexOfItem(itemOrIndex) : itemOrIndex;
    if (index < 0) {
        return false;
    }
...
SidebarWindow.Group.Item.prototype.attachToGroup = function (group) {
    if (!group) {
        MCSystem.throwException("Group can't be illegal or undefined state!");
    }
    let attached = this.getGroup();
    if (attached !== null) {
        MCSystem.throwException("You're must deattach sidebar item firstly!");
    }
    let window = group.getWindow();
    if (window !== null) {
        let actor = new BoundsActor();
