Callback.addCallback("CoreEngineLoaded", function (api) {
    ...
    tryout(function () {
        api.ModAPI.registerAPI("DevEditor", {createAndLock: function () {
            restart();
        ...
        if (ExecuteableSupport.isModuleMissed()) {
            ...
            UIEditor = importMod("UIEditor", function () {
                ...
                if (menu) {
                    let source = java.lang.String.valueOf(String(menu)), index = source.indexOf("{");
                    if (index > -1) {
                        let injectable = "if (DevEditor.isLocked()) return;\n" + "if (layout.getChildCount() == 1) DevEditor.createAndLock();", first = source.substring(0, index + 1), second = source.substring(index + 2, source.length() - 1);
            ...
            Setting = importMod("Setting", function () {
                ...
                rover = false;
                if (removeMenu) {
                    let source = java.lang.String.valueOf(String(removeMenu)), index = source.indexOf("{");
                    if (index > -1) {
                        let injectable = "if (!rover) DevEditor.createAndLock();", first = source.substring(0, index + 1), second = source.substring(index + 2, source.length() - 1);
    ...
    tryout(function () {
        ...
        if (loadSupportables) {
            if (supportSupportables) {
                ...
                showHint(translate("Supportables isn't supported and disabled"));
            }
        }
    });
});
