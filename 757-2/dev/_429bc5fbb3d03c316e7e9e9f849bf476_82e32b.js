const MenuWindow = function () {
    let set = new ActorSet(), slideIn = new SlideActor(), slideOut = new SlideActor();
...
MenuWindow.prototype.addElement = function (element) {
    this.indexOfElement(element) == -1 && this.getElements().push(element);
    return element;
};
...
MenuWindow.prototype.scrollToElement = function (elementOrIndex, duration) {
    let index = typeof elementOrIndex == "object" ? this.indexOfElement(elementOrIndex) : elementOrIndex;
...
MenuWindow.prototype.removeElement = function (elementOrIndex) {
    ...
    let set = new ActorSet(), fade = new FadeActor(), bounds = new BoundsActor();
...
MenuWindow.Header.prototype.reset = function () {
    ...
    let content = new android.widget.FrameLayout(context);
    let params = new android.widget.LinearLayout.LayoutParams(Interface.Display.MATCH, Interface.Display.WRAP);
    ...
    params = new android.widget.FrameLayout.LayoutParams(Interface.Display.MATCH, Interface.Display.MATCH);
    ...
    params = new android.widget.LinearLayout.LayoutParams(Interface.getY(320), Interface.getY(320));
    params.leftMargin = params.topMargin = params.rightMargin = params.bottomMargin = Interface.getY(12);
...
MenuWindow.Header.prototype.setWindow = function (window) {
    ...
    window.elements && window.elements.indexOf(this) == -1 && window.elements.push(this);
    let layout = window.views ? window.views.layout : null, content = this.getContent();
    if (!layout || !content) {
        return this;
    }
...
MenuWindow.Header.prototype.setupScroll = function () {
    ...
    if (!window || !window.views || !window.views.scroll) {
        return this;
    }
    window.views.scroll.setOnScrollChangeListener(function (view, x, y) {
        let content = scope.content.getY(), logo = scope.views.logo.getY(), offset = y - content - logo - Interface.getY(12), real = offset / scope.getMaxScroll() * 100;
...
MenuWindow.Header.prototype.setSlideProgress = function (procent) {
    ...
    let scope = this, content = this.getContent(), slide = this.views.slide, logo = this.views.logo;
    if (!content || !slide || !logo) {
        return this;
    }
    ...
    slide.setTranslationY(-slide.getHeight() + logo.getHeight() * 0.1 + (scope.offset || 0) - logo.getHeight() * procent);
    return this;
};
...
MenuWindow.prototype.addHeader = function (header) {
    let header = header instanceof MenuWindow.Header ? header : new MenuWindow.Header(this);
...
MenuWindow.ProjectHeader.prototype.reset = function () {
    ...
    params.topMargin = params.rightMargin = params.bottomMargin = Interface.getY(24);
...
MenuWindow.ProjectHeader.prototype.checkNothingNeedable = function () {
    let content = this.getContent(), views = this.views;
    if (!content || !views || !views.background) {
        return this;
    }
    let window = this.getWindow();
    if (window) {
        let set = new ActorSet(), bounds = new BoundsActor(), fade = new FadeActor();
    ...
    views.background.setVisibility(few ? Interface.Visibility.VISIBLE : Interface.Visibility.GONE);
    return this;
};
MenuWindow.ProjectHeader.prototype.addCategory = function (nameOrCategory, name) {
    let category = nameOrCategory instanceof MenuWindow.ProjectHeader.Category ? nameOrCategory : new MenuWindow.ProjectHeader.Category(this, nameOrCategory);
    name && nameOrCategory instanceof MenuWindow.ProjectHeader.Category && nameOrCategory.setTitle(name);
...
MenuWindow.ProjectHeader.prototype.removeCategory = function (categoryOrIndex) {
    let index = categoryOrIndex instanceof MenuWindow.ProjectHeader.Category ? this.indexOfCategory(categoryOrIndex) : categoryOrIndex;
...
MenuWindow.prototype.addProjectHeader = function (header) {
    let header = header instanceof MenuWindow.ProjectHeader ? header : new MenuWindow.ProjectHeader(this);
...
MenuWindow.ProjectHeader.Category.prototype.setParentHeader = function (header) {
    ...
    header.categories && header.categories.indexOf(this) == -1 && header.categories.push(this);
    let content = this.getContent();
    if (!content) {
        return this;
    }
    ...
    layout.addView(content);
    this.header = header;
    header.updateSlideProgress(), header.checkNothingNeedable();
    return this;
};
...
MenuWindow.ProjectHeader.Category.prototype.addItem = function (srcOrItem, titleOrSrc, descriptionOrTitle, actionOrDescription, action) {
    let item = srcOrItem instanceof MenuWindow.ProjectHeader.Category.Item ? srcOrItem : new MenuWindow.ProjectHeader.Category.Item(this, srcOrItem, titleOrSrc, descriptionOrTitle, actionOrDescription, action);
    titleOrSrc && srcOrItem instanceof MenuWindow.ProjectHeader.Category.Item && srcOrItem.setIcon(titleOrSrc);
    descriptionOrTitle && srcOrItem instanceof MenuWindow.ProjectHeader.Category.Item && srcOrItem.setTitle(descriptionOrTitle);
    actionOrDescription && srcOrItem instanceof MenuWindow.ProjectHeader.Category.Item && srcOrItem.setDescription(actionOrDescription);
    action && srcOrItem instanceof MenuWindow.ProjectHeader.Category.Item && srcOrItem.setOnClickListener(action);
...
MenuWindow.ProjectHeader.Category.prototype.removeItem = function (itemOrIndex) {
    let index = itemOrIndex instanceof MenuWindow.ProjectHeader.Category.Item ? this.indexOfItem(itemOrIndex) : itemOrIndex;
...
MenuWindow.ProjectHeader.Category.Item.prototype.reset = function () {
    ...
    params = new android.widget.LinearLayout.LayoutParams(Interface.getY(54), Interface.getY(54));
    params.leftMargin = Interface.getY(48);
...
MenuWindow.ProjectHeader.Category.Item.prototype.setParentCategory = function (category) {
    ...
    category.items && category.items.indexOf(this) == -1 && category.items.push(this);
...
MenuWindow.Category.prototype.addItem = function (srcOrItem, titleOrAction, action) {
    let item = srcOrItem instanceof MenuWindow.Category.Item ? srcOrItem : new MenuWindow.Category.Item(this, srcOrItem, titleOrAction, action);
    titleOrAction && srcOrItem instanceof MenuWindow.Category.Item && srcOrItem.setOnClickListener(titleOrAction);
    this.indexOfItem(item) == -1 && this.getItems().push(item);
    return item;
};
...
MenuWindow.Category.prototype.removeItem = function (itemOrIndex) {
    let index = itemOrIndex instanceof MenuWindow.Category.Item ? this.indexOfItem(itemOrIndex) : itemOrIndex;
    let item = this.getItemAt(index);
    let layout = this.views.layout, content = item.getContent();
    if (!layout || !content) {
        return this;
...
MenuWindow.prototype.addCategory = function (nameOrCategory, name) {
    let category = nameOrCategory instanceof MenuWindow.Category ? nameOrCategory : new MenuWindow.Category(this, nameOrCategory);
    name && nameOrCategory instanceof MenuWindow.Category && nameOrCategory.setTitle(name);
...
MenuWindow.Category.Item.prototype.reset = function () {
    ...
    this.content = content;
    views.icon = new android.widget.ImageView(context);
    params = new android.widget.LinearLayout.LayoutParams(Interface.getY(172), Interface.getY(172));
    content.addView(views.icon, params);
    views.title = new android.widget.TextView(context);
    typeface && views.title.setTypeface(typeface);
...
MenuWindow.Category.Item.prototype.setParentCategory = function (category) {
    ...
    category.items && category.items.indexOf(this) == -1 && category.items.push(this);
    let layout = category.views ? category.views.layout : null, content = this.getContent();
    if (!layout || !content) {
        return this;
    }
...
MenuWindow.Message.prototype.reset = function () {
    ...
    this.content = content;
    views.icon = new android.widget.ImageView(context);
    let params = new android.widget.LinearLayout.LayoutParams(Interface.getY(66), Interface.getY(66));
    params.leftMargin = params.topMargin = params.rightMargin = params.bottomMargin = Interface.getY(12);
    content.addView(views.icon, params);
...
MenuWindow.prototype.addMessage = function (srcOrMessage, messageOrSrc, actionOrMessage, action) {
    let message = srcOrMessage instanceof MenuWindow.Message ? srcOrMessage : new MenuWindow.Message(this, srcOrMessage, messageOrSrc, actionOrMessage);
    messageOrSrc && srcOrMessage instanceof MenuWindow.Message && srcOrMessage.setIcon(messageOrSrc);
    actionOrMessage && srcOrMessage instanceof MenuWindow.Message && srcOrMessage.setMessage(actionOrMessage);
    action && srcOrMessage instanceof MenuWindow.Message && srcOrMessage.setOnClickListener(action);
