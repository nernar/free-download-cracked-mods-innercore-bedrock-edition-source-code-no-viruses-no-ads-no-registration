const HintAlert = function () {
    ...
    let actor = new SlideActor(Interface.Gravity.BOTTOM), interpolator = new DecelerateInterpolator();
    actor.setInterpolator(interpolator);
    actor.setDuration(this.time / 6);
    this.setEnterActor(actor);
    actor = new ActorSet();
    let slide = new SlideActor(Interface.Gravity.BOTTOM), fade = new FadeActor(FadeActor.OUT);
...
HintAlert.prototype.attachMessage = function (hint, color, background) {
    if (this.canStackedMore()) {
        ...
        layout.setBackgroundDrawable(background !== undefined ? background instanceof java.lang.Object ? background : ImageFactory.getDrawable(background) : ImageFactory.getDrawable("popup"));
        layout.setOrientation(Interface.Orientate.VERTICAL);
        layout.setGravity(Interface.Gravity.CENTER);
        let content = this.getContent(), params = new android.widget.LinearLayout.LayoutParams(Interface.Display.WRAP, Interface.Display.WRAP);
        ...
        let bounds = new BoundsActor(), fade = new FadeActor();
...
HintAlert.prototype.forceAddMessage = function (hint, color, force) {
    if (!this.inConsoleMode() && this.findStackedHint(hint)) {
        ...
        if (this.canStackedMore()) {
            ...
            if ((!this.isPinned() && this.inConsoleMode()) || (!this.isPinned() && this.isStackable() && !this.alreadyHasHint(hint))) {
                this.addToStack(hint, color);
            } else {
                if (!this.isStackable() || (this.isPinned() && this.inConsoleMode())) {
...
const showHint = function (hint, color, reawait) {
    if (showHint.launchStacked !== undefined) {
        showHint.launchStacked.push({hint: hint, color: color, reawait: reawait});
        return;
    }
    handle(function () {
