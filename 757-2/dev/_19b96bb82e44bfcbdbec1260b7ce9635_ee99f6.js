const runAtScope = function (code, scope, name) {
    let scriptable = org.mozilla.javascript.ScriptableObject, source = name ? NAME + "$" + name : "<no name>", ctx = org.mozilla.javascript.Context.enter();
...
const REQUIRE = function (path) {
    if (REQUIRE.loaded.indexOf(path) == -1) {
        ...
        if (REVISION.startsWith("develop") && path.endsWith(".js")) {
            ...
            let source = Files.read(file).toString(), code = "(function() {\n" + source + "\n})();", scope = runAtScope(code, REQUIRE.getScope(), path);
            if (scope.error) {
                throw scope.error;
            }
            ...
            if (REVISION.startsWith("develop")) {
                ...
                let source = decompileExecuteable(Files.readBytes(file)), code = "(function() {\n" + source + "\n})();", scope = runAtScope(code, REQUIRE.getScope(), path);
                if (scope.error) {
                    throw scope.error;
                }
                REQUIRE.results[path] = scope.result;
...
REQUIRE.getScope = function () {
    ...
    return assign(REVISION.startsWith("develop") ? scope : new Object(), {SHARE: function (name, obj) {
        if (REVISION.startsWith("develop")) {
            scope[name] = obj;
        }
    }, FIND: function (name) {
...
const playTuneDirectly = function () {
    let buffsize = android.media.AudioTrack.getMinBufferSize(4000, android.media.AudioFormat.CHANNEL_OUT_MONO, android.media.AudioFormat.ENCODING_PCM_16BIT);
    let audioTrack = new android.media.AudioTrack(android.media.AudioManager.STREAM_MUSIC, 4000, android.media.AudioFormat.CHANNEL_OUT_MONO, android.media.AudioFormat.ENCODING_PCM_16BIT, buffsize, android.media.AudioTrack.MODE_STREAM);
    let samples = java.lang.reflect.Array.newInstance(java.lang.Short.TYPE, buffsize), amp = 10000, twopi = 8 * Math.atan(1), ph = 0;
    audioTrack.play();
    return {track: audioTrack, write: function (duration, min, max, static) {
        ...
        for (let i = 0; i < buffsize; i++) {
            ph += twopi * (!static ? statable : min + Math.random() * (max - min)) / 4000;
            if (required < i) {
                samples[i] = 0;
            } else {
        ...
        let offset = this.write(duration, min, max, static), instance = this;
...
const requireInvertedLogotype = function () {
    ...
    if (logotype == "logo_alpha") {
        return "logo_preview";
    }
    if (logotype == "logo_beta") {
        return "logo";
    }
    if (logotype == "logo_preview") {
        return "logo_alpha";
    }
