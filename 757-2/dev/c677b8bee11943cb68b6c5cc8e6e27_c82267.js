        ...
        obj.frames.forEach(function (element, index) {
            ...
            if (element.length > 6 && element[6]) {
                switch (element[6]) {
                  case "center":
                  case 1:
                    result.animation[0].frames[index].interpolator = Transition.Interpolator.ACCELERATE_DECELERATE;
                    break;
                  case "start":
                  case "left":
                  case 2:
                    result.animation[0].frames[index].interpolator = Transition.Interpolator.DECELERATE;
                    break;
                  case "end":
                  case "right":
                  case 3:
                    result.animation[0].frames[index].interpolator = Transition.Interpolator.ACCELERATE;
                    break;
                }
            }
        });
...
MathUtils.mathDivider = function (number) {
    ...
    if (number % 100000000 === 0) {
        return Math.round(number / 100000000) + "e8";
    }
    if (number % 10000000 === 0) {
        return Math.round(number / 10000000) + "e7";
    }
    if (number % 1000000 === 0) {
        return Math.round(number / 1000000) + "e6";
    }
    if (number % 100000 === 0) {
        return Math.round(number / 100000) + "e5";
    }
    if (number % 10000 === 0) {
        return Math.round(number / 10000) + "e4";
    }
    if (number % 1000 === 0) {
        return Math.round(number / 1000) + "e3";
    }
    if (number % 100 === 0) {
        return Math.round(number / 100) + "e2";
    }
    if (number % 1 === 0) {
        return String(number);
    }
