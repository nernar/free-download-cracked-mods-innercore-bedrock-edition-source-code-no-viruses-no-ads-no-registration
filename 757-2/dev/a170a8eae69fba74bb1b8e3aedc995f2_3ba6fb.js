const playTransition = function (worker, frame) {
    return tryout(function () {
        ...
        if (TransitionEditor.data.transition) {
            ...
            transition = TransitionEditor.data.transition = new Transition();
        }
        ...
        if (frame == undefined || frame == null) {
            ...
            transition.addFrame(offset.x, offset.y, offset.z, offset.yaw, offset.pitch, offset.duration, offset.interpolator);
        }
                    ...
                    for (let f = 0; f < animate.getFrameCount(); f++) {
                        ...
                        if (drawSelection && Popups.hasOpenedByName("frame_select")) {
                            if (TransitionEditor.data.frame == f) {
                                ...
                                Particles.addParticle(ParticleType.happyVillager, coords.x, coords.y, coords.z, 0, 0, 0, 0);
                            }
                        }
                        let previous = {x: 0, y: 0, z: 0, yaw: 0, pitch: 0};
                        let request = [frame.x / transitionSideDividers, frame.y / transitionSideDividers, frame.z / transitionSideDividers, frame.yaw / transitionSideDividers, frame.pitch / transitionSideDividers, transitionSideDividers, frame.duration / transitionSideDividers, frame.interpolator];
                        for (let i = 0; i < transitionSideDividers; i++) {
                            let point = Transition.compareVectorPoint(request, i);
                            Particles.addParticle(ParticleType.flame, coords.x + previous.x, coords.y + previous.y, coords.z + previous.z, (point.x + previous.x) / 20, (point.y + previous.y) / 20, (point.z + previous.z) / 20, 0);
                            previous.x += point.x;
                            previous.y += point.y;
                            previous.z += point.z;
        ...
        source.forEach(function (element, index) {
            ...
            if (element.animation !== undefined) {
                ...
                if (animate !== undefined) {
                    if (project.animation === undefined) {
                        project.animation = new Array();
                    }
...
let TransitionEditor = {data: new Object(), reset: function () {
    ...
    this.data.hasAnimate = this.data.worker.Animation.getAnimateCount() > 0 && this.data.worker.Animation.getAnimate(0).getFrameCount() > 0;
    ...
    button.show();
    let sidebar = new SidebarWindow(), group = sidebar.addGroup("transition");
    ...
    if (TransitionEditor.data.worker.Define.getEntity() != getPlayerEnt()) {
        ...
        category.addItem("transitionPlayer", translate("Player"), function () {
            if (!hasResetMessage) {
                let message = control.addMessage("menuBoardWarning", translate("Current entity will be lost.") + " " + translate("Touch here to confirm."), function () {
    ...
    let name = file.getName(), project = TransitionEditor.data.worker.getProject();
    if (name.endsWith(".dnp")) {
        ...
        if (name.endsWith(".nds")) {
            let active = Date.now();
            tryout(function () {
                let obj = compileData(Files.read(file)), result = convertNds(obj);
    ...
    if (name.endsWith(".dnp")) {
        ...
        if (name.endsWith(".nds")) {
            let active = Date.now();
            tryout(function () {
                let current = TransitionEditor.data.worker.getProject(), obj = compileData(Files.read(file)), result = convertNds(obj);
    ...
    let name = (TransitionEditor.data.name = i, file.getName()), project = TransitionEditor.data.worker.getProject();
    if (name.endsWith(".dnp")) {
    ...
    if (transition && transition.isStarted()) {
        ...
        playTransition(TransitionEditor.data.worker), TransitionEditor.create();
    }
}, move: function () {
    ...
    let transition = TransitionEditor.data.transition, selected = TransitionEditor.data.frame;
    if (transition && transition.isStarted()) {
        if (stopTransition(TransitionEditor.data.worker)) {
            showHint(translate("Transition stopped"));
        }
    } else {
        playTransition(TransitionEditor.data.worker, selected), TransitionEditor.create();
    }
}, move: function () {
    if (!TransitionEditor.Frame.hasSelection()) {
        showHint(translate("Nothing chosen"));
    ...
    let selected = TransitionEditor.data.frame, frame = TransitionEditor.data.worker.Animation.getAnimate(0).getFrame(selected);
    let popup = new CoordsPopup();
    popup.setTitle(translate("Move"));
    popup.setBaseMathes([1, 10, 100]);
    let group = popup.addGroup("x");
    ...
    confirm(translate("Deleting"), translate("Are you sure want to delete this frame?"), function () {
        ...
        if (TransitionEditor.data.worker.Animation.getAnimate(0).getFrameCount() == 0) {
            TransitionEditor.create();
        }
