const LaunchSequence = new LogotypeSequence({count: 3, create: function (value) {
    if (isFirstLaunch()) {
        let foreground = this.getForegroundIcon(), background = this.getBackgroundIcon();
    ...
    let sequence = this;
    confirm(translate(NAME) + " " + translate(VERSION), translate("Launch sequence interrupted or handled exception.") + " " + translate("Do you want to retry modification launch?"), function () {
        handle(function () {
            sequence.execute();
        }, sequence.getExpirationTime() * 2);
    });
    if (!isFirstLaunch()) {
        LogotypeSequence.prototype.create.call(this);
    }
...
const FetchAdditionalSequence = new SnackSequence({requiresProgress: false, count: 4, process: function (index) {
    if (index == 1) {
                            ...
                            for (let i = 0; i < resources.length; i++) {
                                ...
                                if (resource && resource.resourceType == "resource") {
                                    ...
                                    for (let t = 0; t < files.length; t++) {
                                        ...
                                        if (animation != -1) {
                                            name = name.substring(0, animation);
                                        }
