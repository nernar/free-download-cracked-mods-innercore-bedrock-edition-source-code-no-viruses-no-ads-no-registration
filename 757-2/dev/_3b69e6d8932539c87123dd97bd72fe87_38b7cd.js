const TypingTextView = function (scope, attrs, style) {
    ...
    let instance = new JavaAdapter(android.widget.TextView, {alreadyPostedSequenceTyping: false, typerHandler: new android.os.Handler(), typerRunnable: new java.lang.Runnable(function () {
        ...
        if (this.hasMoreTyperChar() == true) {
            let char = this.getCurrentTyperChar(), duration = instance.getCharDuration(previous, char);
            instance.typerHandler.postDelayed(instance.typerRunnable, duration);
            previous = char;
        } else {
            this.completeTyperSequence();
        }
        ...
        return previous !== undefined ? /[\.\,\-]/.test(previous) ? duration * punctuationMultiplier : /[\?\!\;\:]/.test(previous) ? duration * questionMultiplier : /\s/.test(previous) ? duration * spaceMultiplier : duration : duration;
        ...
        if (typeof sequence == "string") {
            ...
            this.typerHandler.removeCallbacks(this.typerRunnable);
            if (track !== undefined) {
                track.release(), delete track;
            }
            charIndex = Number(index) - 1;
            if (tone) {
                track = playTuneDirectly();
            }
            if (charIndex < 0) {
                delete previous;
            } else {
                previous = sequence.charAt(charIndex);
            }
