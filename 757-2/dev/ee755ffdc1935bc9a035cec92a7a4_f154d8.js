...
BlockConverter.prototype.getIdentifier = function (obj) {
    ...
    return obj.id || obj.define ? obj.define.id : this.getAndAttachWorker().Define.getIdentificator();
};
...
BlockConverter.prototype.buildRenderer = function (obj) {
    ...
    for (let i = 0; i < renderer.length; i++) {
        ...
        result.push("BlockRenderer.setStaticICRender(BlockID." + this.getIdentifier(obj) + ", " + (renderer.length == 1 ? "-1" : i) + ", " + this.resolvePrefix("renderer", renderer, i) + ");");
...
BlockConverter.prototype.buildCollision = function (obj) {
    ...
    for (let i = 0; i < collision.length; i++) {
        ...
        result.push("BlockRenderer.setCustomCollisionShape(BlockID." + this.getIdentifier(obj) + ", " + (collision.length == 1 ? "-1" : i) + ", " + this.resolvePrefix("collision", collision, i) + ");");
...
BlockConverter.prototype.buildBox = function (box) {
    if (box.texture) {
        if (box.texture.length > 1) {
            return MathUtils.mathDivider(box.x1) + ", " + MathUtils.mathDivider(box.y1) + ", " + MathUtils.mathDivider(box.z1) + ", " + MathUtils.mathDivider(box.x2) + ", " + MathUtils.mathDivider(box.y2) + ", " + MathUtils.mathDivider(box.z2) + ", " + stringifyObjectUnsafe(box.texture, true);
        }
        if (box.texture.length == 1) {
            return MathUtils.mathDivider(box.x1) + ", " + MathUtils.mathDivider(box.y1) + ", " + MathUtils.mathDivider(box.z1) + ", " + MathUtils.mathDivider(box.x2) + ", " + MathUtils.mathDivider(box.y2) + ", " + MathUtils.mathDivider(box.z2) + ", " + (typeof box.texture[0][0] != "number" ? "\"" + box.texture[0][0] + "\"" : box.texture[0][0]) + (box.texture[0][1] !== undefined ? ", " + box.texture[0][1] : new String());
        }
    }
    return MathUtils.mathDivider(box.x1) + ", " + MathUtils.mathDivider(box.y1) + ", " + MathUtils.mathDivider(box.z1) + ", " + MathUtils.mathDivider(box.x2) + ", " + MathUtils.mathDivider(box.y2) + ", " + MathUtils.mathDivider(box.z2);
};
