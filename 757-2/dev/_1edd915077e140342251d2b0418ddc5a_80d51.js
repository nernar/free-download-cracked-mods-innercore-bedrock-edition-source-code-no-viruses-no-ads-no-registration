...
SnackSequence.prototype.update = function (progress, index) {
    if (this.getMessage() !== null) {
        this.setHintAndBackground(this.getProgressionHint(progress, index), this.getProgressionBackground(progress, index));
    }
};
SnackSequence.prototype.cancel = function (error) {
    if (this.getMessage() !== null) {
        this.setHintAndBackground(this.getCancellationHint(error), this.getCancellationBackground(error));
    }
...
SnackSequence.prototype.complete = function (active) {
    if (this.getMessage() !== null) {
        this.setHintAndBackground(this.getCompletionHint(active), this.getCompletionBackground(active));
    } else {
...
StackedSnackSequence.prototype.getProgressionBackground = function (progress, index) {
    if (progress >= 100) {
        ...
        if (this.count !== undefined) {
            ...
            return this.getCreationBackground();
        }
    }
};
...
StackedSnackSequence.prototype.create = function (value, active) {
    if (this.isRequiredProgress()) {
        ...
        snack.attachMessage(this.getCreationHint(), this.getCreationColor(), this.getCreationBackground());
        snack.show();
        this.snack = snack;
        SnackSequence.addProcess(this);
    }
...
StackedSnackSequence.prototype.update = function (progress, index) {
    ...
    if (snack !== null) {
        ...
        for (let i = 0; i < stack.length; i++) {
            ...
            snack.attachMessage(this.getProgressionHint(element.message, progress, index), element.color, this.getProgressionBackground(progress, index));
            stack.splice(i, 1);
            i--;
        }
        snack.show();
...
StackedSnackSequence.prototype.change = function (message, color) {
    let stack = this.getStack();
    if (stack !== null) {
        stack.push({message: message, color: color});
        this.updated = true;
    }
};
StackedSnackSequence.prototype.cancel = function (error) {
    let snack = this.getSnackWindow();
    if (snack !== null) {
        snack.attachMessage(this.getCancellationHint(error), this.getCancellationColor(error), this.getCancellationBackground(error));
        snack.show();
    }
    this.handleCompletion();
    Sequence.prototype.cancel.apply(this, arguments);
...
StackedSnackSequence.prototype.complete = function (active) {
    ...
    if (snack !== null) {
        ...
        snack.attachMessage(this.getCompletionHint(active), this.getCompletionColor(active), this.getCompletionBackground(active));
        snack.show();
    }
    this.handleCompletion();
};
