const TransitionWorker = function (obj) {
    ...
    this.getProject = function () {
        return {type: "transition", define: this.Define.getParams(), animation: this.Animation.getParams()};
    };
    ...
    this.Define = {params: {entity: getPlayerEnt(), starting: new Object()}, getParams: function () {
        return this.params;
        ...
        let starting = this.getStarting(), coords = Entity.getPosition(this.getEntity()), angle = Entity.getRotation(this.getEntity());
    ...
    this.Animation = {animates: new Array(), createAnimate: function () {
        ...
        this.params = {frames: new Array()};
        this.addFrame = function () {
            let frames = this.getFrames();
            frames.push({x: 0, y: 0, z: 0, yaw: 0, pitch: 0, duration: 3});
        ...
        this.setupFrame = function (index) {
            let real = this.getFrameCoords(index), frame = this.getFrame(index), coords = Entity.getPosition(worker.Define.getEntity()), angle = Entity.getRotation(worker.Define.getEntity());
        ...
        this.asArray = function () {
            let frames = this.getFrames(), result = new Array();
            for (let i = 0; i < frames.length; i++) {
                let frame = frames[i], index = result.push([frame.x, frame.y, frame.z, frame.yaw, frame.pitch, frame.duration]) - 1;
