LIBRARY({name: "ShootLib", version: 1.5, api: "CoreEngine", dependencies: ["SoundAPI", "AdvancedEvents"]});
IMPORT("SoundAPI", "Sound");
var ShootLib = {ShotType: {SINGLE: 0, MULTIPLE: 1, NORMAL: 0, SHOTGUN: 1}, ButtonType: {CLICK: 1, TOUCH: 2}, MAX_DAMAGE: -1, GUN_BITMAP: -1, EXTRA_BITMAP: -2, init: function (settings) {
    if (!settings) {
        return false;
    }
    for (var i in settings) {
        switch (i) {
          ...
          case "aim":
            ...
            if (!bitmap.coords.x) {
                bitmap.coords.x = (i == "fire") ? 544 : 0;
            }
            if (!bitmap.coords.y) {
                bitmap.coords.y = 0;
            }
            if (!bitmap.coords.width) {
                bitmap.coords.width = 544;
            }
            if (!bitmap.coords.height) {
                bitmap.coords.height = 544;
            }
            if (!bitmap.size) {
                bitmap.size = {};
            }
            if (!bitmap.size.width) {
                bitmap.size.width = 90;
            }
            if (!bitmap.size.height) {
                bitmap.size.height = 90;
            }
          ...
          case "crosshair":
            ...
            if (!bitmap.coords.x) {
                bitmap.coords.x = 1088;
            }
            if (!bitmap.coords.y) {
                bitmap.coords.y = 0;
            }
            if (!bitmap.coords.width) {
                bitmap.coords.width = 64;
            }
            if (!bitmap.coords.height) {
                bitmap.coords.height = 64;
            }
            if (!bitmap.size) {
                bitmap.size = {};
            }
          ...
          case "crosshairGUI":
            ...
            if (!bitmap.name) {
                bitmap.name = ShootLib.GUN_BITMAP;
            }
            if (!bitmap.coords) {
                bitmap.coords = {};
            }
            ...
            if (!bitmap.coords.width) {
                bitmap.coords.width = 1024;
            }
            if (!bitmap.coords.height) {
                bitmap.coords.height = 1024;
            }
            if (!bitmap.size) {
                bitmap.size = {};
            }
            if (!bitmap.size.width) {
                bitmap.size.width = _shootlib.settings.crosshairGUI.size;
            }
            _shootlib.settings[i] = setting;
            break;
          case "reload":
    ...
    for (var i in _shootlib.guns) {
        if (typeof (item) == "number") {
            if (ItemID[_shootlib.guns[i].id] == item) {
                return _shootlib.guns[i];
            }
        } else {
            if (ItemID[_shootlib.guns[i].id] == item.id) {
                return _shootlib.guns[i];
            }
        }
    }
    ...
    for (var i in _shootlib.ammos) {
        if (typeof (item) == "number") {
            if (ItemID[_shootlib.ammos[i].id] == item) {
                return _shootlib.ammos[i];
            }
        } else {
            if (ItemID[_shootlib.ammos[i].id] == item.id) {
                return _shootlib.ammos[i];
            }
        }
    }
...
Entity.shot = function (entity) {
    ...
    if (gun.shotType == ShootLib.ShotType.NORMAL) {
        ...
        if (gun.shotType == ShootLib.ShotType.MULTIPLE) {
            shotShotgun(gun);
        }
    }
...
var _shootlib = {settings: {image_button: true, left_handed: false, fire: {text: {content: "FIRE", size: 18}, bitmap: {name: "ui", coords: {x: 544, y: 0, width: 544, height: 544}, size: {width: 90, height: 90}}}, aim: {text: {content: "AIM", size: 18}, bitmap: {name: "ui", coords: {x: 0, y: 0, width: 544, height: 544}, size: {width: 90, height: 90}}}, crosshair: {bitmap: {name: "ui", coords: {x: 1088, y: 0, width: 64, height: 64}, size: {width: 90, height: 90}}}, crosshairGUI: {bitmap: {name: -1, coords: {x: 0, y: 0, width: 1024, height: 1024}}}, reload: {text: {content: "8/8", size: 16}}}, guns: [], ammos: [], createGuns: function () {
    var guns = this.guns;
    for (var i = 0; i < guns.length; i++) {
        var gun = guns[i];
...
var GUI = {ctx: UI.getContext(), gui_dir: __dir__ + "gui/", run: function (f) {
...
var _width = metrics.widthPixels, _height = metrics.heightPixels;
if (_width > _height) {
    GUI.width = _width;
    GUI.height = _height;
} else {
...
var GUIMod = {fire: {button: null, popup: null, opened: false, open: function () {
    if (GUIMod.fire.opened == true) {
        return;
    }
    ...
    if (gun.buttonType == ShootLib.ButtonType.CLICK) {
        GUI.run(function () {
            GUIMod.fire.button.setOnClickListener(new android.view.View.OnClickListener() {onClick: function (b) {
                if (currentShotTicks <= 0 && !isReloading) {
                    shot(gun);
                }
            }});
            GUIMod.fire.button.setOnTouchListener(new android.view.View.OnTouchListener() {onTouch: function (a, c) {
                return false;
            }});
        });
    } else {
        if (gun.buttonType == ShootLib.ButtonType.TOUCH) {
            GUI.run(function () {
                GUIMod.fire.button.setOnTouchListener(new android.view.View.OnTouchListener() {onTouch: function (a, c) {
...
}}, aim: {button: null, popup: null, opened: false, open: function () {
    if (GUIMod.aim.opened == true) {
        return;
    }
    ...
    GUI.run(function () {
        GUIMod.aim.button.setOnClickListener(new android.view.View.OnClickListener() {onClick: function (b) {
            if (!isAim) {
...
}}, crosshair: {image: null, popup: null, opened: false, opened_gui: false, open: function () {
    if (GUIMod.crosshair.opened == true) {
        return;
    }
...
}}, reload: {text: null, popup: null, opened: false, open: function () {
    if (GUIMod.reload.opened == true) {
        return;
    }
    ...
    GUI.run(function () {
        GUIMod.reload.text.setOnClickListener(new android.view.View.OnClickListener() {onClick: function (b) {
            if (!isReloading) {
                reloadAmmo(gun);
            }
        }});
...
var ACCURACY = 0.55, isAim = false, isShooting = false, isReloading = false, selectGun = null, currentShotTicks = 0;
