LIBRARY({name: "BackpackAPI", version: 1, shared: true, api: "CoreEngine"});
Saver.addSavesScope("BackpacksScope", function read(scope) {
    ...
    return {nextUnique: BackpackRegistry.nextUnique, containers: BackpackRegistry.containers};
});
var BackpackRegistry = {nextUnique: 1, containers: {}, prototypes: {}, guis: {}, register: function (id, obj) {
    if (id <= 0) {
    ...
    if (!gui && !this.guis[slots]) {
        ...
        let gui = new UI.StandartWindow({standart: {header: {text: {text: "Backpack"}}, inventory: {standart: true}, background: {standart: true}, minHeight: 90 + (slots / 10 * 61) + 70}, drawing: [], elements: {}});
