LIBRARY({name: "MTL", version: 9, shared: true, api: "CoreEngine"});
IMPORT("SoundAPI");
var toolSound = new Sound();
var MTT = {sword: {isWeapon: true, enchantType: Native.EnchantType.weapon, damage: 4, blockTypes: ["fibre", "plant"], calcDestroyTime: function (item, coords, block, params, destroyTime, enchant) {
    if (block.id == 30) {
        return 0.08;
    }
    if (block.id == 35) {
    ...
    return destroyTime;
}}, shovel: {enchantType: Native.EnchantType.shovel, damage: 2, blockTypes: ["dirt"], useItem: function (coords, item, block) {
    if (block.id == 2 && coords.side == 1) {
        World.setBlock(coords.x, coords.y, coords.z, 198);
        toolSound.setSource("shovel.ogg");
        toolSound.setInPlayer();
        toolSound.play();
        ToolAPI.breakCarriedTool(1);
    }
}}, pickaxe: {enchantType: Native.EnchantType.pickaxe, damage: 2, blockTypes: ["stone"]}, axe: {enchantType: Native.EnchantType.axe, damage: 3, blockTypes: ["wood"]}, hoe: {useItem: function (coords, item, block) {
...
}}, fai: {useItem: function (coords, item, block) {
    if (World.getBlock(coords.x, coords.y + 1, coords.z).id == 0) {
        World.setBlock(coords.x, coords.y + 1, coords.z, 51, 0);
    }
    if (World.getBlock(coords.x + 1, coords.y + 1, coords.z).id == 0) {
        World.setBlock(coords.x + 1, coords.y + 1, coords.z, 51, 0);
    }
    if (World.getBlock(coords.x - 1, coords.y + 1, coords.z).id == 0) {
        World.setBlock(coords.x - 1, coords.y + 1, coords.z, 51, 0);
    }
    if (World.getBlock(coords.x, coords.y + 1, coords.z - 1).id == 0) {
        World.setBlock(coords.x, coords.y + 1, coords.z - 1, 51, 0);
    }
    if (World.getBlock(coords.x, coords.y + 1, coords.z + 1).id == 0) {
        World.setBlock(coords.x, coords.y + 1, coords.z + 1, 51, 0);
    }
    if (World.getBlock(coords.x + 1, coords.y + 1, coords.z + 1).id == 0) {
    ...
    toolSound.setSource("fai.ogg");
    toolSound.setInPlayer();
    toolSound.play();
    ToolAPI.breakCarriedTool(1);
}}, fav: {useItem: function (coords, item, block) {
    if (World.getBlock(coords.x, coords.y + 1, coords.z).id == 0) {
        World.setBlock(coords.x, coords.y + 1, coords.z, 11, 0);
    }
    toolSound.setSource("fav.ogg");
    toolSound.setInPlayer();
    toolSound.play();
    ToolAPI.breakCarriedTool(1);
}}};
...
ToolAPI.getEnchantExtraData = function (extra) {
    var enchant = {silk: false, fortune: 0, efficiency: 0, unbreaking: 0, experience: 0};
    if (!extra) {
...
Block.registerDropFunctionForID(13, function (coords, blockID, blockData, level, enchant) {
    if (Math.random() < [0.1, 0.14, 0.25, 1][enchant.fortune || 0]) {
        return [[318, 1, 0]];
    }
    return [[13, 1, 0]];
}, 0);
Block.registerDropFunctionForID(78, function (coords, blockID, blockData, level, enchant) {
    if (level > 0) {
        if (blockData == 7) {
            return [[332, 4, 0]];
        }
...
Block.registerDropFunctionForID(80, function (coords, blockID, blockData, level, enchant) {
    if (enchant.silk) {
        return [[80, 1, 0]];
    }
    return [[332, 1, 0], [332, 1, 0], [332, 1, 0], [332, 1, 0]];
}, 0);
Block.registerDropFunctionForID(110, function (coords, blockID, blockData, level, enchant) {
    if (enchant.silk) {
        return [[110, 1, 0]];
    }
    return [[3, 1, 0]];
}, 0);
Block.registerDropFunctionForID(198, function (coords, blockID, blockData, level, enchant) {
    if (enchant.silk) {
        return [[198, 1, 0]];
    }
    return [[3, 1, 0]];
}, 0);
Block.registerDropFunctionForID(243, function (coords, blockID, blockData, level, enchant) {
    if (enchant.silk) {
        return [[243, 1, 0]];
    }
    return [[3, 1, 0]];
