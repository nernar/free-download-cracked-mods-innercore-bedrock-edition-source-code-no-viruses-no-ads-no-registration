...
ListingPopup.prototype.addButtonElement = function (name, click) {
    ...
    if (name) {
        views.buttons[index].setText(name);
    }
...
ListingPopup.prototype.addEditElement = function (hint, value) {
    let scope = this, views = this.views, index = views.edits.length;
    views.edits[index] = new android.widget.EditText(context);
    views.edits[index].setInputType(android.text.InputType.TYPE_CLASS_TEXT | android.text.InputType.TYPE_TEXT_FLAG_MULTI_LINE | android.text.InputType.TYPE_TEXT_FLAG_NO_SUGGESTIONS);
    views.edits[index].setImeOptions(android.view.inputmethod.EditorInfo.IME_FLAG_NO_FULLSCREEN | android.view.inputmethod.EditorInfo.IME_FLAG_NO_ENTER_ACTION);
    ...
    if (value) {
        views.edits[index].setText(String(value));
    }
    if (hint) {
        views.edits[index].setHint(String(hint));
    }
...
ListingPopup.prototype.getButton = function (index) {
    ...
    return {view: button, setBackground: function (texture) {
        this.view.setBackgroundDrawable(ImageFactory.getDrawable(texture));
        return this;
    }, switchVisibility: function () {
        if (visible) {
            this.view.setVisibility(Interface.Visibility.GONE);
        } else {
            this.view.setVisibility(Interface.Visibility.VISIBLE);
        }
        visible = !visible;
        return this;
    }};
...
ListingPopup.prototype.getEdit = function (index) {
    ...
    return {getValue: function () {
        ...
        if (visible) {
            edit.setVisibility(Interface.Visibility.GONE);
        } else {
            edit.setVisibility(Interface.Visibility.VISIBLE);
        }
        visible = !visible;
        return this;
    }};
