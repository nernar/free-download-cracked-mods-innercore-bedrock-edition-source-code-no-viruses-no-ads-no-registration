const ProjectEditor = {data: new Object(), create: function () {
    selectMode = 0;
    ...
    prepareAdditionalInformation(3 + Number(isAnyCustomSupportableLoaded()) + Number(REVISION.indexOf("alpha") != -1), (3 + Number(isAnyCustomSupportableLoaded()) + Number(REVISION.indexOf("alpha") != -1)) * 2);
    ...
    if (ProjectProvider.isInitialized()) {
        ...
        if (project && project.getCount()) {
            let content = project.getAll(), blocks = project.getBlocks();
            if (blocks && blocks.length > 0) {
                let category = header.addCategory(translate("Blocks"));
                for (let i = 0; i < blocks.length; i++) {
                    let block = content[blocks[i]], models = block.renderer.length + block.collision.length;
                    category.addItem("block", block.define.id, translateCounter(models, "no models", "%s1 model", "%s" + (models % 10) + " models", "%s models", [models]));
                }
                ...
                category.setOnItemHoldListener(function (item, index) {
                    confirm(translate("Warning!"), translate("Selected worker will be removed, including all it's data.") + " " + translate("Do you want to continue?"), function () {
                        let position = blocks[index];
                        if (position >= 0 && position < content.length) {
                            content.splice(position, 1);
                            showHint(translate("Worker has been removed"));
                        } else {
                            showHint(translate("Something went wrong"));
                        }
                        ProjectEditor.menu();
                    });
                    return true;
            ...
            if (REVISION.indexOf("alpha") != -1) {
                ...
                if (entities && entities.length > 0) {
                    ...
                    for (let i = 0; i < entities.length; i++) {
                        ...
                        category.addItem("entity", entity.define.id, translateCounter(models, "no models / tree", "%s1 model / tree", "%s" + (models % 10) + " models / tree", "%s models / tree", [models]));
                    }
                    category.setOnItemClickListener(function (item, index) {
            ...
            if (transitions && transitions.length > 0) {
                ...
                for (let i = 0; i < transitions.length; i++) {
                    ...
                    category.addItem("transition", translate("Transition"), translateCounter(animates, "no animates", "%s1 animate", "%s" + (animates % 10) + " animates", "%s animates", [animates]) + " / " + translate("%s fps", transition.define.fps || 60));
                }
                category.setOnItemClickListener(function (item, index) {
    ...
    if (loadSupportables && supportSupportables && Setting) {
        category.addItem("world", translate("World"), function () {
            if (Level.isLoaded()) {
                ...
                showHint(translate("Supportable module can't be loaded at menu"));
            }
    ...
    category.addItem("menuProjectManage", translate("Reset"), function () {
        confirm(translate("Creating project"), translate("Current project will be erased, all unsaved data will be lost.") + " " + translate("Do you want to continue?"), function () {
    ...
    if (isAnyCustomSupportableLoaded()) {
        ...
        if (UIEditor) {
            category.addItem(UIEditor.icon, translate("UIEditor"), function () {
                isSupportEnv = true;
                ...
                if (!hintStackableDenied) {
                    ...
                    showHint(translate(UIEditor.modName) + " - " + translate(UIEditor.author));
                }
                control.dismiss();
            });
        }
        if (WorldEdit) {
            category.addItem(WorldEdit.icon, translate("WorldEdit"), function () {
                let result = WorldEdit(function () {
                    try {
                        let array = new Array();
                        for (let item in Commands) {
                            let command = Commands[item];
                            array.push(command.name + (command.args && command.args.length > 0 ? " " + command.args : new String()) + "\n" + Translation.translate(command.description));
                        }
                        return array.join("\n\n");
                    }
                    catch (e) {
                ...
                if (!hintStackableDenied) {
                    ...
                    showHint(translate(WorldEdit.modName) + " - " + translate(WorldEdit.author));
                }
            });
        }
        if (DumpCreator) {
            category.addItem(DumpCreator.icon, translate("Dumper"), function () {
                ...
                confirm(translate(DumpCreator.modName), translate(result ? "Dump will be saved into supportable directory. Do you want to overwrite it?" : Level.isLoaded() ? "Dump will be generated and saved into supportable directory. This will be take a few seconds. Continue?" : "Launch dump generation in menu may cause crash, you can also enter into world. Continue anyway?"), function () {
                    ...
                    if (evaluate != true) {
                        reportError(evaluate);
                    }
                });
            });
        }
        if (RunJSingame) {
            category.addItem(RunJSingame.icon, translate("Run JS"), function () {
                ...
                if (!hintStackableDenied) {
                    ...
                    showHint(translate(RunJSingame.modName) + " - " + translate(RunJSingame.author));
                }
            });
        }
        if (InstantRunner) {
            category.addItem(InstantRunner.icon, translate("IRunner"), function () {
                ...
                if (!hintStackableDenied) {
                    ...
                    showHint(translate(InstantRunner.modName) + " - " + translate(InstantRunner.author));
                }
            });
        }
        attachAdditionalInformation(control);
    ...
    if (name.endsWith(".dnp")) {
        ...
        if (name.endsWith(".json")) {
            ...
            if (name.endsWith(".ndb")) {
                let active = Date.now();
                tryout(function () {
                    let current = ProjectProvider.getProject(), obj = compileData(Files.read(file)), result = convertNdb(obj);
                    current.getAll().push(result);
                    ProjectEditor.menu();
                    showHint(translate("Imported success") + " " + translate("as %ss", preround((Date.now() - active) / 1000, 1)));
                });
                ...
                if (name.endsWith(".nds")) {
                    let active = Date.now();
                    tryout(function () {
                        let current = ProjectProvider.getProject(), obj = compileData(Files.read(file)), result = convertNds(obj);
                        current.getAll().push(result);
                        ProjectEditor.menu();
                        showHint(translate("Imported success") + " " + translate("as %ss", preround((Date.now() - active) / 1000, 1)));
                    });
    ...
    if (name.endsWith(".dnp")) {
        ...
        if (name.endsWith(".json")) {
            ...
            if (name.endsWith(".ndb")) {
                let active = Date.now();
                tryout(function () {
                    let current = ProjectProvider.create(), obj = compileData(Files.read(file));
    ...
    let name = (ProjectEditor.data.name = i, file.getName()), project = ProjectProvider.getProject().getAll();
    if (name.endsWith(".dnp")) {
