...
WindowProvider.openWindow = function (window) {
    ...
    if (!window.isFocusable()) {
        ...
        let popup = content ? new android.widget.PopupWindow(content, window.getWidth(), window.getHeight()) : new android.widget.PopupWindow(window.getWidth(), window.getHeight()), popupId = this.prepareIdentifier(window);
        ...
        popup.showAtLocation(Interface.getDecorView(), window.getGravity(), window.getX(), window.getY());
        return;
    }
...
WindowProvider.closeWindow = function (window) {
    ...
    if (!window.isFocusable()) {
        let popupId = window.popupId, popup = this.getByPopupId(popupId);
        if (popup === null) {
            return;
        }
        popup.dismiss();
...
WindowProvider.updateWindow = function (window) {
    ...
    if (!window.isFocusable()) {
        ...
        if (enter !== null) {
            this.setEnterActor(popupId, enter);
        }
        let exit = window.getExitActor();
        if (exit !== null) {
            this.setExitActor(popupId, exit);
        }
        popup.setContentView(window.getContent());
        popup.setTouchable(window.isTouchable());
        popup.setFocusable(window.isFocusable());
        popup.update(window.getX(), window.getY(), window.getWidth(), window.getHeight());
        return;
    }
    let flags = this.getFlagsForWindow(window);
...
UniqueHelper.prepareWindow = function (window) {
    if (this.isAttached(window)) {
        let opened = this.getWindow(window), updatable = opened.isUpdatable();
        if (opened == window) {
            return false;
        }
        if (updatable) {
