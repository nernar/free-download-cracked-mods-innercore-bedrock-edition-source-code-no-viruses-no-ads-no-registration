...
const stringifyObject = function (obj, identate, action, hint, complete) {
    ...
    const recursiveHint = function (update) {
        handle(function () {
            ...
            if (done) {
                update((complete || translate("Done")) + " " + translate("as %ss", preround((Date.now() - active) / 1000, 1)), 100);
                createProcess.complete();
            } else {
                recursiveHint(update);
            }
        }, 50);
    };
    if (showProcesses) {
    ...
    const recursiveIndexate = function (obj) {
        tryout(function () {
            if (done) {
                MCSystem.throwException("Was cancelled by timeout");
            }
            count++;
    ...
    handleThread(function () {
        tryout(function () {
            ...
            if (!done) {
                let result = stringifyObjectUnsafe(obj, identate, {onUpdate: function () {
                    if (done) {
                        MCSystem.throwException("Was cancelled by timeout");
                    }
                    if (safetyProcesses) {
            ...
            switch (typeof obj) {
              ...
              case "object":
                if (Array.isArray(obj)) {
                    ...
                    return (identate ? tabbed ? "{\n" + tabs : "{ " : "{") + joined + (identate ? tabbed ? tabs.replace("\t", new String()) + "\n}" : " }" : "}");
                }
              default:
...
const exportProject = function (object, isAutosave, path, action) {
    ...
    }, isAutosave ? translate("Autosaving") : translate("Exporting"), isAutosave ? translate("Autosaved") : translate("Exported"));
};
const compileData = function (text, type, additional) {
    if (type == "string") {
        text = "\"" + text + "\"";
    }
    let code = "(function() { return " + text + "; })();", scope = runAtScope(code, additional, "compile.js");
    return scope.error ? scope.error : !type ? scope.result : type == "string" ? String(scope.result) : type == "number" ? parseInt(scope.result) : type == "float" ? parseFloat(scope.result) : type == "object" ? scope.result : null;
};
const importProject = function (path, action) {
    readFile(path, true, function (bytes) {
        let result = decompileFromProduce(bytes), data = compileData(result, "object");
        if (data && !(data instanceof Error)) {
            handle(function () {
                action && (data.length !== undefined ? action(data) : action([data]));
            });
        } else {
            confirm(translate("Can't open file"), translate("Looks like, project is damaged. Check project and following exception information:") + "\n" + (data ? data.name + ": " + data.message : translate("Empty project")) + "\n\n" + translate("Do you want to retry?"), function () {
...
const importScript = function (path, action) {
    readFile(path, false, function (text) {
        ...
        if (result !== null) {
            handle(function () {
                action && (result.length !== undefined ? action(result) : action([result]));
            });
        }
    });
};
const compileScript = function (text) {
    ...
    if (scope.error) {
        ...
        if (scope.result && scope.result.error) {
...
const getScriptScope = function () {
    ...
    let Callback = {addCallback: function (name, func) {
    }};
    let BlockID = new Object();
    let IDRegistry = {fromString: function (id) {
        return BlockID[id] !== undefined ? BlockID[id] : -1;
    }, genBlockID: function (name) {
        let nid = BlockID[name] = __data__.length;
        __data__[nid] = {type: "block", define: {id: name}, renderer: new Array(), collision: new Array()};
        return nid;
    }};
    let Block = {createBlock: function (id, data, special) {
        ...
        __data__[id].collision.push({boxes: [{x1: x1, y1: y1, z1: z1, x2: x2, y2: y2, z2: z2}]});
    }, setBlockShape: function (id, pos1, pos2) {
        __data__[id].collision.push({boxes: [{x1: pos1.x, y1: pos1.y, z1: pos1.z, x2: pos2.x, y2: pos2.y, z2: pos2.z}]});
    }};
    let ICRender = {Model: function () {
        this.renderer = new Array();
        this.addEntry = function (model) {
            model && this.renderer.push({boxes: model.boxes});
        };
    }, CollisionShape: function () {
        this.shape = new Array();
        this.addEntry = function (model) {
            model && this.shape.push({boxes: model.boxes});
        };
    }};
    let BlockRenderer = {Model: function () {
        this.boxes = new Array();
        this.addBox = function (x1, y1, z1, x2, y2, z2, texture, data) {
            let index = this.boxes.push({x1: x1, y1: y1, z1: z1, x2: x2, y2: y2, z2: z2}) - 1;
            if (texture !== undefined) {
                this.boxes[index].texture = texture;
            }
            if (data !== undefined) {
                this.boxes[index].texture = [[texture, data]];
            }
        };
        ...
        if (!(id >= 0)) {
            __data__.push({type: "block", define: {id: "invalidIdentifier"}, renderer: render.renderer, collision: new Array()});
        } else {
            __data__[id].renderer = render.renderer;
        }
    }, setCustomCollisionShape: function (id, meta, collision) {
        if (!(id >= 0)) {
            __data__.push({type: "block", define: {id: "invalidIdentifier"}, renderer: new Array(), collision: collision.shape});
        } else {
            __data__[id].collision = collision.shape;
        }
    }, enableCoordMapping: function (id, meta, render) {
        this.setStaticICRender(id, meta, render);
    }};
    let Transition = function (obj) {
        this.__data__ = (__data__[__data__.length] = {type: "transition", define: {starting: new Object()}, animation: [{frames: new Array()}]});
        this.__compareVectorPoint = function (index, request) {
        };
        this.addFrame = function (x, y, z, yaw, pitch, duration, interpolator) {
            let index = this.__data__.animation[0].frames.push({x: x, y: y, z: z, yaw: yaw, pitch: pitch, duration: duration || 1 / (this.__data__.define.fps || 60)}) - 1;
        ...
        this.withFrom = function (x, y, z, yaw, pitch) {
            this.__data__.define.starting = {x: x, y: y, z: z, yaw: yaw, pitch: pitch};
        };
        ...
        this.withFrames = function (frames) {
            for (let i = 0; i < frames.length; i++) {
                this.addFrame(frames[i].x, frames[i].y, frames[i].z, frames[i].yaw, frames[i].pitch, frames[i].duration, frames[i].vector);
            }
        };
        if (obj) {
    ...
    Transition.Interpolator = {LINEAR: 0, ACCELERATE: 1, DECELERATE: 2, ACCELERATE_DECELERATE: 3};
    return {__data__: __data__, __stringify__: stringifyObjectUnsafe, Callback: Callback, BlockID: BlockID, IDRegistry: IDRegistry, Block: Block, ICRender: ICRender, BlockRenderer: BlockRenderer, Transition: Transition};
};
...
Project.prototype.getCurrentObject = function () {
    let id = this.getCurrentlyId(), obj = this.getAll()[id];
    if (obj) {
        return obj;
    }
    delete this.worker;
...
Project.prototype.updateCurrentWorker = function () {
    let id = this.getCurrentlyId(), worker = this.getCurrentWorker();
    if (!worker || id < 0) {
        return;
    }
...
Project.prototype.callAutosave = function () {
    ...
    tryout.call(this, function () {
        ...
        this.updateCurrentWorker();
        exportProject(autosaveProjectable ? this.getAll() : this.getCurrentObject(), true, Dirs.AUTOSAVE + this.getProjectTime() + ".dnp", function (result) {
...
Project.prototype.getProjectTime = function () {
    ...
    return monthToName(time.getMonth()) + " " + time.getDate() + ", " + time.getFullYear() + " " + (time.getHours() >= 10 ? time.getHours() : "0" + time.getHours()) + "-" + (time.getMinutes() >= 10 ? time.getMinutes() : "0" + time.getMinutes()) + "-" + (time.getSeconds() >= 10 ? time.getSeconds() : "0" + time.getSeconds());
};
