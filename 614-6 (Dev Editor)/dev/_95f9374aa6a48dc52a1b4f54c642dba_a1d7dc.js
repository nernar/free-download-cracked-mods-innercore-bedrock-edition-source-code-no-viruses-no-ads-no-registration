const ExecuteableSupport = {mods: new Object(), getClassLoader: function () {
    return context.getClassLoader();
}, updateClassReference: function (name) {
    return name.startsWith("zhekasmirnov.launcher.") ? "com.zhekasmirnov.innercore." + name.substring(22) : name;
}, newInstance: function (name, initialize) {
    return tryoutSafety.call(this, function () {
        return java.lang.Class.forName(isHorizon ? this.updateClassReference(name) : name, initialize || false, this.getClassLoader()).newInstance();
    }, null);
    ...
    if (!builder) {
        MCSystem.throwException("Submodule supportable " + name + " load cancelled");
    }
    let mod = builder.buildModForDir(dir);
    if (!mod) {
        MCSystem.throwException("Build mod directory " + name + " failed, api disabled");
    }
    mod.onImport();
    ...
    if (!mod) {
        MCSystem.throwException("Can't launch mod " + name);
    }
    ...
    return tryoutSafety(function () {
        ...
        Logger.Log("Failed to run exec source " + (source ? source.name : "unknown") + ", ignoring", "DEV-CORE");
    }, null);
    ...
    let mod = this.getSupportable(name);
    if (!mod) {
        MCSystem.throwException("Can't find mod " + name);
    }
...
const isNotSupported = function (obj) {
    if (obj.result == true) {
        ...
        if (obj.result == false) {
            ...
            if (obj.result.lineNumber !== undefined && REVISION.startsWith("develop")) {
