function selectBitmap(type) {
    Widgets.run(function () {
        ...
        for (let i in files) {
            ...
            item = item.replace(__dir__ + "gui/custom/", "");
            item = item.replace(".png", "");
            item = item + "";
            let text = Widgets.text(item, 16), linear = Widgets.linear([text], Widgets.orientate.horizontal, Widgets.gravity.left);
            linear.setOnClickListener(function () {
                ...
                if (c != null) {
                    ...
                    if (type == 1 && c.bitmap2) {
                        selectBitmap(2), alert("Select second bitmap");
                    }
                    if (type == 2 && c.bitmap2) {
                        c.bitmap2 = item;
                    }
        ...
        var dial = {title: "Select Bitmap " + type, view: scroll, buttons: {text: [null, null, "Cancel"], click: [null, null, function () {
        }]}};
        dialog(dial);
    });
}
