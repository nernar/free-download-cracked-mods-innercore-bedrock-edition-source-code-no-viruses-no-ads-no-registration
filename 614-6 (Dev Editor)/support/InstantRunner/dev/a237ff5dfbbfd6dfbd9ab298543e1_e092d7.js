...
var window = new UI.Window({location: {x: 950, y: 0, width: 60, height: 60}, elements: {"0": {type: "button", x: 0, y: 0, bitmap: "btnIR", scale: 60, clicker: {onClick: openAndroidUI}}}, drawing: [{type: "background", color: android.graphics.Color.TRANSPARENT}]});
...
function openAndroidUI() {
    try {
        ...
        for (var i in files) {
            ...
            btn.setOnClickListener(new OnClickListener({onClick: function () {
                dialog.dismiss();
                runScript(filename);
            }}));
            layout.addView(btn);
