const GuideAPI = {guides: {}, container: new UI.Container(), nextLink: null, preLink: null, openedGuide: null, registerGuide: function (unique, params) {
    ...
    function addLinks(elements) {
        if (GuideAPI.nextLink) {
            elements["nextLink"] = {type: "button", x: 910, y: UI.getScreenHeight() - 45, bitmap: (guide.textures && guide.textures.nextLink) || "next_page", scale: 3, clicker: {onClick: function () {
                GuideAPI.arrowClick(GuideAPI.nextLink);
            }}};
        }
        if (GuideAPI.preLink) {
            elements["preLink"] = {type: "button", x: 50, y: UI.getScreenHeight() - 45, bitmap: (guide.textures && guide.textures.preLink) || "pre_page", scale: 3, clicker: {onClick: function () {
    ...
    if (this.container.isOpened()) {
        ...
        if (page.right) {
            page.right.controller(page.right, content, this.container, "right");
        }
        if (page.left) {
            page.left.controller(page.left, content, this.container, "left");
        }
        addLinks(content);
        ...
        let elements = {"close": {type: "closeButton", x: 930, y: 10, bitmap: (guide.textures && guide.textures.close) || "btn_close", scale: 3}, "close3": {type: "button", x: 500, y: 10, bitmap: (guide.textures && guide.textures.close) || "btn_close", scale: 3}};
        let guideGUI = new UI.StandartWindow({standart: {background: {bitmap: (guide.textures && guide.textures.background) || "guide_background", color: android.graphics.Color.argb(256, 0, 0, 0)}}, drawing: [], elements: elements});
        if (guide.debug) {
            guideGUI.setDebugEnabled(true);
        }
        if (page.right) {
            page.right.controller(page.right, elements, this.container, "right");
        }
        if (page.left) {
            page.left.controller(page.left, elements, this.container, "left");
        }
        addLinks(elements);
...
const GuideHelper = {drawTextArray: function (array, x, y, globalSize, elements, section) {
    for (let i in array) {
        let line = array[i];
        ...
        if (line.link) {
            elements[section + "_" + i].clicker = {onClick: function () {
...
const PageControllers = {BASIC_PAGE: function (params, elements, container, section) {
    ...
    if (params.items) {
        ...
        for (let index in params.items) {
            ...
            if (item.link) {
                elements["grid_page_" + index + "_" + section].clicker = {onClick: function () {
...
ModAPI.registerAPI("GuideAPI", {GuideAPI: GuideAPI, GuideHelper: GuideHelper, PageControllers: PageControllers});
