const BlockWorker = function (obj) {
    ...
    this.getProject = function () {
        return {type: "block", define: this.Define.getParams(), renderer: this.Renderer.getParams(), collision: this.Collision.getParams()};
    };
    ...
    this.Define = {params: {id: "customBlock", data: "[{\n\tname: \"Custom Block\",\n\ttexture: [[\"stone\", 0]],\n\tinCreative: true\n}]", mapped: new Array()}, addMapping: function (x, y, z) {
        let mapped = this.getMapped(), index = this.getMapAtCoords(x, y, z);
        if (index == -1) {
        ...
        if (data.length == 0) {
            delete this.getParams().data;
        } else {
            this.getParams().data = data;
        }
    }, setSpecialType: function (type) {
        if (type.length == 0) {
            delete this.getParams().special;
        } else {
            this.getParams().special = type;
        }
    }};
    this.Renderer = {models: new Array(), createModel: function () {
        ...
        this.params = {boxes: new Array()};
        this.addBox = function (id, data) {
            let boxes = this.getBoxes();
            boxes.push({x1: 0, y1: 0, z1: 0, x2: 0.0625, y2: 0.0625, z2: 0.0625, texture: [[id, data]]});
        ...
        this.getBoxSize = function (index) {
            ...
            return {x: box.x2 - box.x1, y: box.y2 - box.y1, z: box.z2 - box.z1};
        };
    ...
    this.Collision = {models: new Array(), createModel: function () {
        let models = this.getModels();
        models.push(new this.Model());
        return models.length - 1;
    }, removeModel: function (index) {
        ...
        this.addBox = function () {
            let boxes = this.getBoxes();
            boxes.push({x1: 0, y1: 0, z1: 0, x2: 0.0625, y2: 0.0625, z2: 0.0625});
            return boxes.length - 1;
        };
        this.cloneBox = function (index) {
            let boxes = this.getBoxes();
