...
const Dirs = {EXTERNAL: android.os.Environment.getExternalStorageDirectory(), DATA: android.os.Environment.getDataDirectory() + "/data/" + (isHorizon ? context.getPackageName() : "com.zhekasmirnov.innercore") + "/", MOD: isHorizon ? __packdir__ + "innercore/mods/" : "/games/com.mojang/mods/", WORLD: isHorizon ? __packdir__ + "worlds/" : "/games/com.mojang/innercoreWorlds/", OPTION: isHorizon ? "/games/horizon/minecraftpe/options.txt" : "/games/com.mojang/minecraftpe/options.txt", RESOURCE: isHorizon ? __packdir__ + "assets/resource_packs/vanilla" : "/games/com.mojang/resource_packs/innercore-resources/", IMAGE: __dir__ + "gui/", ASSET: __dir__ + "assets/", EXPORT: __dir__ + "saves/", CACHE: __dir__ + "assets/cache/", AUTOSAVE: __dir__ + "saves/.autosave/", LOGGING: __dir__ + "saves/.logging/", SUPPORT: __dir__ + "support/", TESTING: __dir__ + ".script/", EVALUATE: __dir__ + ".eval/", BACKUP: __dir__ + ".backup/", TODO: __dir__ + ".todo/"};
tryout(function () {
...
const formatSize = function (size) {
    return size < 100 ? Number(size).toFixed(2) : size < 1000 ? Number(size).toFixed(1) : size < 1024 ? Number(size).toFixed() : "?";
};
const MediaTypes = {AUDIO: ["3gp", "mp4", "m4a", "aac", "ts", "flac", "gsm", "mid", "xmf", "mxmf", "rtttl", "rtx", "ota", "imy", "mp3", "mkv", "wav", "ogg"], VIDEO: ["3gp", "mp4", "ts", "webm", "mkv"], IMAGE: ["bmp", "gif", "jpg", "jpeg", "png", "webp", "heic", "heif", "ico"]};
...
Files.getExtensionType = function (file) {
    ...
    return (type == "zip" || type == "tar" || type == "rar" || type == "7z" || type == "mcpack" || type == "apk" || type == "aar" || type == "script" || type == "odex" || type == "vdex" || type == "zipp" || type == "rarr" || type == "gz" || type == "bz2" || type == "dex" || type == "jar" || type == "xz" || type == "icmod" || type == "mcworld") ? "archive" : (type == "json" || type == "config" || type == "info" || type == "proguard-rules.pro" || name == "manifest" || name == ".staticids" || type == ".profig.os" || name == "saves-info" || name == ".installation_info" || name == "cfg" || name == "info" || type == "gradle") ? "json" : (type == "txt" || type == "text" || type == "log" || type == "csv") ? "text" : (name == "order.txt" || name == ".includes" || type == "nproj" || type == "yml" || type == "md" || type == "xml" || type == "dat" || type == "dat_old" || type == "ldb") ? "order" : (type == "js" || type == "ts" || type == "cpp" || type == "css" || type == "bsh" || type == "h" || type == "java" || type == "html" || type == "cs") ? "script" : (type == "dnp" || type == "ndb" || type == "nds") ? "project" : (type == "ttf" || type == "otf" || type == "wotf") ? "font" : type ? MediaTypes.VIDEO.indexOf(type) != -1 ? "video" : MediaTypes.IMAGE.indexOf(type) != -1 ? "image" : MediaTypes.AUDIO.indexOf(type) != -1 ? "audio" : name == "icon" ? "image" : "unknown" : "none";
};
...
Files.prepareFormattedSize = function (size) {
    return size <= 0 ? translate("Empty") : size < 1024 ? translate("%s bytes", size) : size < 1024 * 1024 ? translate("%s KB", formatSize(size / 1024)) : size < 1024 * 1024 * 1024 ? translate("%s MB", formatSize(size / (1024 * 1024))) : size < 1024 * 1024 * 1024 * 1024 ? translate("%s GB", formatSize(size / (1024 * 1024 * 1024))) : translate("%s TB", formatSize(size / (1024 * 1024 * 1024 * 1024)));
};
Files.listFiles = function (path, explore) {
    let files = new Array(), file = new java.io.File(path);
    if (file.isFile()) {
        return [file];
    }
...
Files.listDirectories = function (path, explore) {
    let directories = new Array(), file = new java.io.File(path);
    if (file.isFile()) {
        return directories;
    }
    let list = file.listFiles() || new Array();
...
Files.listFileNames = function (path, explore, root) {
    ...
    if (file.isFile()) {
        return [String(path).replace(root, new String())];
    }
...
Files.readKey = function (file, separator) {
    separator = separator || "=";
    let text = this.read(file, true), obj = new Object();
    for (let i = 0; i < text.length; i++) {
        let source = text[i].split(separator);
        if (source.length == 2) {
            obj[source[0]] = source[1];
        }
    }
    return obj;
};
...
Files.read = function (file, massive) {
    ...
    let reader = java.io.BufferedReader(new java.io.FileReader(file)), result = new Array(), line;
...
Files.readLine = function (file, index) {
    ...
    let reader = java.io.BufferedReader(new java.io.FileReader(file)), count = -1, line;
...
Files.readLines = function (file, startInd, endInd) {
    ...
    let reader = java.io.BufferedReader(new java.io.FileReader(file)), result = new Array(), count = -1, line;
...
Files.copyRecursive = function (path, output, explore, includeDirectories) {
    let files = this.listFiles(path, explore), count = 0;
...
Files.compareRecursive = function (input, target, explore, simpleCompare, includeDirectories) {
    let left = this.listFileNames(input, explore), right = this.listFileNames(target, explore), changes = new Array();
    if (includeDirectories !== false) {
        let first = this.listDirectoryNames(input, explore), second = this.listDirectoryNames(target, explore);
...
Files.isIdentical = function (left, right, explore, simpleCompare, includeDirectories) {
    return this.isCompared(left, right, explore, simpleCompare, includeDirectories) && this.isCompared(right, left, explore, simpleCompare, includeDirectories);
};
...
Archives.unpack = function (file, path) {
    let zip = new java.util.zip.ZipFile(file), elements = zip.entries();
    new java.io.File(path).mkdirs();
    while (elements.hasMoreElements()) {
        let element = elements.nextElement(), source = zip.getInputStream(element), result = new java.io.File(path, element.getName()), bis = new java.io.BufferedInputStream(source);
        if (!element.isDirectory()) {
            result.getParentFile().mkdir();
            let bos = new java.io.BufferedOutputStream(new java.io.FileOutputStream(result)), buf = java.lang.reflect.Array.newInstance(java.lang.Byte.TYPE, 4096), line = 0;
...
Options.getValue = function (key) {
    let file = new java.io.File(Dirs.OPTION), reader = new java.io.BufferedReader(new java.io.InputStreamReader(new java.io.FileInputStream(file))), line = new String(), result = new String();
...
Options.setValue = function (name, key) {
    let file = new java.io.File(Dirs.OPTION), reader = new java.io.BufferedReader(new java.io.InputStreamReader(new java.io.FileInputStream(file))), result = new Array(), line;
    while ((line = reader.readLine()) != null) {
