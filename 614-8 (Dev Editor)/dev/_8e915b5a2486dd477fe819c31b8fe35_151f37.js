const LogViewer = {FREQUENCY: 250, show: function () {
    ...
    seek.setOnSeekBarChangeListener({onProgressChanged: function (view, progress) {
    ...
    handleThread(function () {
        let log = java.lang.Class.forName("zhekasmirnov.launcher.api.log.ICLog", true, context.getClass().getClassLoader()), filter = log.getMethod("getLogFilter").invoke(null);
        do {
            if (popup.isExpanded()) {
...
const ConsoleViewer = {show: function () {
    ...
    popup.addButtonElement(translate("Eval"), function () {
        ...
        if (String(values[0]).length > 0) {
            ...
            if (result.lineNumber !== undefined) {
                ...
                showHint(String(result), Interface.Color.LTGRAY);
            }
        }
    }).setBackground("popup");
...
const DebugEditor = {data: new Object(), create: function () {
    let button = new ControlButton();
    ...
    return tryout(function () {
        ...
        return null;
    }, {title: path, icon: "menuBoardWarning"});
...
const ModificationSource = {selector: function () {
    let control = new MenuWindow();
    control.setOnClickListener(function () {
        ProjectEditor.create();
    });
    ...
    let count = mod.getAllExecutables().size(), type = mod.getBuildType().toString();
    control.addMessage("worldSelectionLimit" + (count > 0 ? count > 5 ? count > 10 ? count > 15 ? "Infinity" : "Maximal" : "Normal" : "Minimal" : "Custom"), translate(mod.getName()) + "\n" + translateCounter(count, "no sources", "%s1 source", "%s" + (count % 10) + " sources", "%s sources", [count]) + " / " + translate(type), function () {
            ...
            confirm(translate("Compilation"), translate("Modification will be dexed and switched to release type.") + " " + translate("Do you want to continue?"), function () {
                handleThread(function () {
                    ...
                    handle(function () {
                        ...
                        if (!result.wasFailed) {
                            ModificationSource.rebuild(mod, "release");
                        }
                        confirm(translate(result.name) + " " + translate(result.version), (result.wasFailed ? translate("Something went wrong during compilation process.") + " " + translate("Checkout reports below to see more details.") : translate("Modification successfully compiled.") + " " + translate("You can switch build type in next window.")) + " " + translate("Founded sources count") + ": " + (result.buildConfig ? result.buildConfig.getAllSourcesToCompile().size() : 0) + ".\n" + translate("Do you want to review report?"), function () {
    ...
    if (type == "release") {
        ...
        if (type == "develop") {
            ...
            showHint(translate("Build type %s is unsupported", translate(type)));
        }
    }
}};
