...
const geometryToModel2 = function (obj) {
    let project = {texture: {width: obj.texturewidth, height: obj.textureheight}, elements: {body: {bones: new Array()}, head: {bones: new Array()}, leftArm: {bones: new Array()}, rightArm: {bones: new Array()}, leftLeg: {bones: new Array()}, rightLeg: {bones: new Array()}}};
    let parents = {hat: "head"};
    let bones = obj.bones;
    for (let item in bones) {
        ...
        bone.pivot && (part.offset = {x: bone.pivot[0], y: 24 - bone.pivot[1], z: bone.pivot[2]});
        bone.rotation && (part.rotation = {x: bone.rotation[0], y: bone.rotation[1], z: bone.rotation[2]});
        for (let n = 0; n < bone.cubes.length; n++) {
            ...
            part.bones.push({type: "box", coords: {x: box.origin[0], y: parent.indexOf("Leg") == -1 ? -box.origin[1] : box.origin[1], z: box.origin[2]}, size: {x: box.size[0], y: box.size[1], z: box.size[2]}, uv: {x: box.uv[0], y: box.uv[1]}});
        }
    }
    return project;
};
const EntityEditor = {data: new Object(), reset: function () {
    ...
    this.data.hasVisual = this.data.worker.Visual.getModelCount() > 0 && this.data.worker.Visual.getModel(0).getAssigmentSize() > 0;
    ...
    button.show();
    let sidebar = new SidebarWindow(), group = sidebar.addGroup("entity");
    ...
    let name = (EntityEditor.data.name = i, file.getName()), project = EntityEditor.data.worker.getProject();
    if (name.endsWith(".dnp")) {
    ...
    if (EntityEditor.data.hasVisual) {
        ...
        if (EntityEditor.Bone.hasSelection()) {
            popup.addButtonElement(translate("Copy current"), function () {
                let model = EntityEditor.data.worker.Visual.getModel(0), last = EntityEditor.data.group, index = EntityEditor.data.group = model.cloneAssigment(last);
    ...
    let selected = EntityEditor.data.group, model = EntityEditor.data.worker.Visual.getModel(0), name = model.getName(selected);
    let popup = new ListingPopup();
    ...
    let selected = EntityEditor.data.group, model = EntityEditor.data.worker.Visual.getModel(0), offset = model.getIndex(selected).getOffset();
    ...
    let selected = EntityEditor.data.group, model = EntityEditor.data.worker.Visual.getModel(0), rotate = model.getIndex(selected).getRotation();
    let popup = new CoordsPopup();
    ...
    confirm(translate("Deleting"), translate("Are you sure want to delete this bone?"), function () {
        let selected = EntityEditor.data.group, model = EntityEditor.data.worker.Visual.getModel(0);
        model.removeAssigment(selected);
        EntityEditor.data.group = selected > 0 ? selected - 1 : -1;
        if (EntityEditor.data.group >= 0) {
            EntityEditor.data.item = model.getIndex(EntityEditor.data.group).getBoxCount() > 0 ? 0 : -1;
        }
        if (model.getAssigmentSize() == 0 || EntityEditor.data.item == -1) {
            EntityEditor.create();
        }
    ...
    let model = EntityEditor.data.worker.Visual.getModel(0), selected = EntityEditor.data.item, group = EntityEditor.data.group;
    if (EntityEditor.Box.hasSelection()) {
        let popup = new ListingPopup();
        popup.setTitle(translate("Create"));
        popup.setOnSelectListener(function (index) {
    ...
    let selected = EntityEditor.data.group, item = EntityEditor.data.item, model = EntityEditor.data.worker.Visual.getModel(0), box = model.getIndex(selected).getBox(item), coords = box.getCoords();
    let popup = new CoordsPopup();
    ...
    let selected = EntityEditor.data.group, item = EntityEditor.data.item, model = EntityEditor.data.worker.Visual.getModel(0), box = model.getIndex(selected).getBox(item), vertex = box.getVertex();
    let popup = new CoordsPopup();
    ...
    confirm(translate("Deleting"), translate("Are you sure want to delete this box?"), function () {
        let selected = EntityEditor.data.group, item = EntityEditor.data.item, model = EntityEditor.data.worker.Visual.getModel(0);
        model.getIndex(selected).removeBox(item);
        EntityEditor.data.item = item > 0 ? item - 1 : -1;
        if (EntityEditor.data.item == -1) {
            EntityEditor.create();
        }
        Popups.closeAllByTag("box");
