...
CoordsPopup.prototype.addGroup = function (name) {
    let views = this.views, index = views.groups.length;
    ...
    if (name) {
        views.titles[index].setText(name);
    }
...
CoordsPopup.prototype.getGroup = function (position) {
    ...
    return this.groups[position] = {items: new Array(), addItem: function (current) {
        let elements = this, index = this.items.length, item = (this.items[index] = {views: new Object(), current: [current, 0], mathes: mathes || [16, 32, 64]});
        ...
        item.views.plus.setOnClickListener(function (view) {
            tryout(function () {
                ...
                if (elements.onChange) {
                    elements.onChange(index, item.current[0]);
                }
                elements.updateMather(index);
            });
        });
        ...
        item.views.mather.setText(current == 1 ? String(item.current[0]) : current > 0 ? preround(item.current[0] * current) + " : " + current : preround(item.current[0] / current) + " * " + (-current));
    }};
};
    ...
    for (let index in this.groups) {
        ...
        if (group.onLongChange) {
            for (let value in group.items) {
                ...
                if (group.onChange) {
                    group.onChange(value, item.current[0]);
                }
