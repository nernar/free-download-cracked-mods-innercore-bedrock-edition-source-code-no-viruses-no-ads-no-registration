...
ExplorerWindow.Approve.prototype.setWindow = function (window) {
    ...
    let layout = window.views ? window.views.layout : null, content = this.getContent(), scope = this;
    if (!layout || !content) {
        return this;
    }
...
ExplorerWindow.Approve.prototype.checkIfCanBeApproved = function () {
    ...
    content.setVisibility(window && window.isCanBeApproved() ? Interface.Visibility.VISIBLE : Interface.Visibility.GONE);
...
ExplorerWindow.prototype.addApprove = function (srcOrApprove, actionOrSrc, action) {
    let approve = srcOrApprove instanceof ExplorerWindow.Approve ? srcOrApprove : new ExplorerWindow.Approve(this, srcOrApprove, actionOrSrc);
    actionOrSrc && srcOrApprove instanceof ExplorerWindow.Approve && srcOrApprove.setIcon(actionOrSrc);
    action && srcOrApprove instanceof ExplorerWindow.Approve && srcOrApprove.setOnApproveListener(action);
...
ExplorerWindow.Path.prototype.setWindow = function (window) {
    ...
    window.elements && window.elements.indexOf(this) == -1 && window.elements.push(this);
    let layout = window.views ? window.views.layout : null, content = this.getContent(), files = window.views ? window.views.files : null, scope = this;
    if (!layout || !content || !files) {
        return this;
    }
    let actor = new BoundsActor();
    actor.setDuration(600);
...
ExplorerWindow.Path.prototype.addPathElement = function (view, extendsArrow) {
    ...
    extendsArrow = extendsArrow === undefined ? Boolean(this.lastPath) : extendsArrow;
    this.lastPath = view;
...
ExplorerWindow.Path.prototype.updatePath = function () {
    ...
    let current = window ? window.root : Dirs.EXTERNAL, pathFilter = path.replace(current, new String()), pathDivided = pathFilter.split("/");
    pathDivided.pop();
    this.addPathIcon("controlAdapterHome", current);
    for (let i = 0; i < pathDivided.length; i++) {
        current += pathDivided[i] + "/";
        if (pathDivided[i].length == 0) {
            continue;
        }
...
ExplorerWindow.prototype.addPath = function (actionOrPath, action) {
    let path = actionOrPath instanceof ExplorerWindow.Path ? actionOrPath : new ExplorerWindow.Path(this, actionOrPath);
    action && actionOrPath instanceof ExplorerWindow.Path && actionOrPath.setOnExploreListener(action);
...
ExplorerWindow.Rename.prototype.reset = function () {
    ...
    views.name.setInputType(android.text.InputType.TYPE_CLASS_TEXT | android.text.InputType.TYPE_TEXT_FLAG_NO_SUGGESTIONS);
...
ExplorerWindow.Rename.prototype.setWindow = function (window) {
    ...
    window.setOnSelectListener(function (window, file, item, previous) {
        if (previous == -1) {
            scope.setCurrentName(file.getName());
        }
    });
    this.window = window;
    return this;
...
ExplorerWindow.Rename.prototype.getCurrentName = function () {
    let name = String(this.views ? this.views.name ? this.views.name.getText().toString() : new String() : new String());
    if (name.length == 0) {
...
ExplorerWindow.Rename.prototype.setFormat = function (indexOrFormat) {
    let index = typeof indexOrFormat == "number" ? indexOrFormat : this.formats.indexOf(indexOrFormat);
...
ExplorerWindow.prototype.addRename = function (actionOrRename, action) {
    let rename = actionOrRename instanceof ExplorerWindow.Rename ? actionOrRename : new ExplorerWindow.Rename(this, actionOrRename);
    action && actionOrRename instanceof ExplorerWindow.Rename && actionOrRename.setOnApproveListener(action);
...
ExplorerAdapter.prototype = new JavaAdapter(android.widget.BaseAdapter, android.widget.Adapter, {array: new Array(), getCount: function () {
    ...
    tryout.call(this, function () {
        ...
        if (!object.isDirectory) {
            ...
            if (extension) {
                object.extension = extension.toUpperCase();
            }
        } else {
            object.name = item;
        }
        object.type = Files.getExtensionType(file);
        let date = new java.util.Date(file.lastModified()), format = new java.text.SimpleDateFormat();
        ...
        tryoutSafety.call(this, function () {
            if (object.isDirectory) {
                object.filesCount = file.list().length;
            } else {
    ...
    tryout.call(this, function () {
        convertView.setBackgroundDrawable(item.isApproved ? ImageFactory.getDrawable("popupSelectionSelected") : null);
        holder.name.setText(item.name);
        holder.bound.setText((item.extension ? item.extension : new String()) + (item.extension && item.size ? " / " : new String()) + (item.size ? item.size : new String()) + (item.isDirectory && item.filesCount !== undefined ? translateCounter(item.filesCount, "no files", "%s1 file", "%s" + (item.filesCount % 10) + " files", "%s files") : new String()));
        ...
        if (item.type == "image") {
            ...
            if (size !== null) {
                ...
                if (size[0] <= maximumAllowedBounds && size[1] <= maximumAllowedBounds) {
                    holder.icon.setImageDrawable(ImageFactory.getDrawable(ImageFactory.loadFromFile("cache", item.file) || "explorerFileCorrupted"));
                    return;
                }
            }
        } else {
        ...
        holder.icon.setImageDrawable(ImageFactory.getDrawable(item.isDirectory ? "explorerFolder" : item.type != "none" ? "explorerExtension" + item.type.charAt(0).toUpperCase() + item.type.substring(1) : "explorerFile"));
    });
    return convertView;
    ...
    if (position !== undefined) {
        ...
        if (this.choice == Interface.Choice.SINGLE) {
            ...
            if (selected != -1) {
                ...
                if (this.choice == Interface.Choice.NONE) {
