...
Block.createBlock("test", [{name: "Block", texture: [["stone", 0]]}]);
...
let __makeAndSaveDump__ = function () {
    ...
    const dumpIgnores = ["VanillaItemID.*", "VanillaBlockID.*", "VanillaTileID.*", "BlockID.*", "ItemID.*", "World.__inworld.*", "World.__inmenu.*", "ToolAPI.blockMaterials.*", "ToolAPI.toolMaterials.*", "ToolAPI.toolData.*", "ToolAPI.blockData.*", "LiquidRegistry.liquids", "LiquidRegistry.FullByEmpty.*", "LiquidRegistry.EmptyByFull.*", "Block.dropFunctions.*", "Block.idSource", "Item.idSource", "IDData.item", "IDData.block"];
    ...
    let dumpObj = function (obj, name) {
        ...
        for (let i in obj) {
            try {
                if (i == "class" || !name && i == "__makeAndSaveDump__") {
                    continue;
                }
                if (dumpIgnores.indexOf(name ? name + "." + i : i) != -1) {
                    continue;
                } else {
                    if (dumpIgnores.indexOf(name + ".*") != -1) {
                        continue;
                    }
                }
                let action = obj[i];
                ...
                if (previousType == "object" && !name && type == "function") {
                    ...
                    if (!previousType == "function" && !name && type == "object") {
                        arr.push("");
                        previousHasName = true;
                    }
                }
                ...
                switch (type) {
                  ...
                  case "object":
                    if (action == null) {
                        ...
                        if (action == scope) {
                            arr.push((name ? name + "." + i : "var " + i) + " = this;");
                        } else {
                            if (action.length + "" != "undefined") {
                                arr.push((name ? name + "." + i : "var " + i) + " = " + JSON.stringify(action) + ";");
                            } else {
                ...
                if (e.message.startsWith("Invalid JavaScript value of type")) {
                    ...
                    arr.push((name ? name + "." + i : "var " + i) + "; // " + e.message);
                }
            }
        }
        return arr.join("\n");
    ...
    let baseParams = {ent: Entity.getAll()[2], path: "test", name: "test", text: "some text", obj: {key: "value"}, file: new java.io.File(__dir__ + "test"), dir: __dir__ + "test.json", blockID: 1, rarityMultiplier: [[0, 1]], type: 10, clone: 0, skin: "zombie.png", modName: "Dump Creator", numericID: 1, x: 0, z: 0, y: 0, entity: Entity.getAll()[3], coords1: {x: 0, y: 0, z: 0}, coords2: {x: 2, y: 1, z: 0}, coords: {x: 0, y: 0, z: 0}, angle: {yaw: 0, pitch: 0}, pos1: {x: 0, y: 0, z: 0}, pos2: {x: 2, y: 1, z: 0}, ent1: Entity.getAll()[4], ent2: Entity.getAll()[5], maxRange: 10, handleNames: false, handleEnchant: false, slot: 0, projectile: 0, loadPart: 0, id: 1, data: 0, encode: false, namedID: "test", count: 1, func: new Function(), item: 280, enchant: 0, fullBlock: {id: 1, data: 0}, toolItem: {id: 280, data: 0}, coords: {x: 0, y: 1, z: 0}, ignoreNative: false, itemID: 280, key: "lava", width: 16, height: 16, liquid: "lava", gameMode: 1, fov: 70, age: 0, health: 10, message: "Hello, World!", texture: new Texture("images/mob/ce_default_entity_texture.png"), render: 3, effectId: 6, effectData: 1, effectTime: 5, tileEntity: TileEntity.getTileEntity(0, 0, 0), heal: 20, w: 0.8, h: 1.6};
    const callIgnores = [];
    let dumpReturn = function (obj, name, args, root) {
        for (let i = 0; i < callIgnores.length; i++) {
            if (callIgnores[i] == name) {
                return "/* ignored because crash */";
            }
        }
        ...
        if (name == "Player.setArmorSlot") {
            arr[1] = 298;
        }
        if (name == "Entity.remove") {
            arr[0] = Entity.getAll()[6];
        }
        if (name == "Block.getBlockAtlasTextureCoords") {
            arr[0] = "stone";
        }
        if (name == "alert") {
            arr[0] = "You're welcome!";
        }
        ...
        try {
            ...
            if (e.message.startsWith("Java class \"[Ljava.lang.reflect.Constructor;\"")) {
                ...
                if (e.message == "Cannot find default value for object.") {
...
Callback.addCallback(getCoreAPILevel() > 8 ? "LevelDisplayed" : "LevelLoaded", function () {
    if (!ModAPI.requireAPI("DevEditor")) {
        __makeAndSaveDump__();
    }
});
