...
Callback.addCallback("NativeCommand", function (str) {
    ...
    for (var i in Commands) {
        if (str.substring(0, Commands[i].length) == Commands[i]) {
            length = Commands[i].length;
        }
    }
...
const MainUI = {codeWindow: function (text) {
    ctx.runOnUiThread(new java.lang.Runnable({run: function () {
        try {
            ...
            new android.app.AlertDialog.Builder(ctx).setTitle(Translation.translate("enterCode1")).setView(editText).setPositiveButton(Translation.translate("runCode"), new android.content.DialogInterface.OnClickListener({onClick: function () {
