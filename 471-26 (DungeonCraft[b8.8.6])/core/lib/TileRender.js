...
LIBRARY({name: "TileRender", version: 21, shared: true, api: "CoreEngine"});
var TileRenderer;
(function (TileRenderer) {
    ...
    function setShapeWithRotation(id, data, boxes) {
        ...
        for (var i = 0; i < 4; i++) {
            ...
            try {
                ...
                try {
                    if (boxes_3_1 && !boxes_3_1.done && (_a = boxes_3.return)) {
                        _a.call(boxes_3);
                    }
                }
                finally {
    ...
    function setStandardModelWithRotation(id, data, texture, hasVertical) {
        var variations = [[texture[3], texture[2], texture[0], texture[1], texture[4], texture[5]], [texture[2], texture[3], texture[1], texture[0], texture[5], texture[4]], [texture[0], texture[1], texture[3], texture[2], texture[5], texture[4]], [texture[0], texture[1], texture[2], texture[3], texture[4], texture[5]], [texture[0], texture[1], texture[4], texture[5], texture[3], texture[2]], [texture[0], texture[1], texture[5], texture[4], texture[2], texture[3]]];
    ...
    function setupWireModel(id, data, width, groupName, preventSelfAdd) {
        ...
        width /= 2;
        var boxes = [{side: [1, 0, 0], box: [0.5 + width, 0.5 - width, 0.5 - width, 1, 0.5 + width, 0.5 + width]}, {side: [-1, 0, 0], box: [0, 0.5 - width, 0.5 - width, 0.5 - width, 0.5 + width, 0.5 + width]}, {side: [0, 1, 0], box: [0.5 - width, 0.5 + width, 0.5 - width, 0.5 + width, 1, 0.5 + width]}, {side: [0, -1, 0], box: [0.5 - width, 0, 0.5 - width, 0.5 + width, 0.5 - width, 0.5 + width]}, {side: [0, 0, 1], box: [0.5 - width, 0.5 - width, 0.5 + width, 0.5 + width, 0.5 + width, 1]}, {side: [0, 0, -1], box: [0.5 - width, 0.5 - width, 0, 0.5 + width, 0.5 + width, 0.5 - width]}];
        try {
            ...
            try {
                if (boxes_4_1 && !boxes_4_1.done && (_a = boxes_4.return)) {
                    _a.call(boxes_4);
                }
            }
            finally {
    ...
    function getCropModel(texture) {
        return createBlockModel(texture[0], texture[1], [[0.25, 0, 0, 0.25, 1, 1], [0.75, 0, 0, 0.75, 1, 1], [0, 0, 0.25, 1, 1, 0.25], [0, 0, 0.75, 1, 1, 0.75]]);
    }
    ...
    var __plantVertex = [[0.15, 0, 0.15, 1, 1], [0.85, 0, 0.85, 0, 1], [0.85, 1, 0.85, 0, 0], [0.15, 0, 0.15, 1, 1], [0.15, 1, 0.15, 1, 0], [0.85, 1, 0.85, 0, 0], [0.15, 0, 0.85, 1, 1], [0.85, 0, 0.15, 0, 1], [0.85, 1, 0.15, 0, 0], [0.15, 0, 0.85, 1, 1], [0.15, 1, 0.85, 1, 0], [0.85, 1, 0.15, 0, 0]];
