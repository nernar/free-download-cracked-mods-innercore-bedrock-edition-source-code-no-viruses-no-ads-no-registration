LIBRARY({name: "ParticlesCore", version: 3, shared: true, api: "CoreEngine"});
function uptPos(self, start, end, value) {
    self.setPos(self.coords.x, start + ((end - start) * value), self.coords.z);
    if (self.coords.y == end) {
        self.restart = !self.restart;
    }
}
let AnimationType = {VANILLA(obj) {
    obj = obj || {};
    let time = obj.time || 130;
    let pos = obj.pos || 0.05;
    let funcs = [function () {
...
Callback.addCallback("LevelDisplayed", function () {
    Network.addClientPacket("api.particle", function (data) {
        if (Entity.getDimension(Player.get()) != data.d) {
            return;
        }
    ...
    Network.addClientPacket("api.particle_array", function (packet) {
        if (Entity.getDimension(Player.get()) != packet.d) {
            return;
        }
...
let ParticlesStorage = {ids: {}, group: null, groups: {}, add(textId, id) {
    this.ids[textId] = id;
    if (this.group !== null) {
        this.groups[this.group][textId] = id;
    }
    return this;
}, get(textId) {
...
let ParticlesCore = {getVector(pos1, pos2) {
    return {x: pos2.x - pos1.x, y: pos2.y - pos1.y, z: pos2.z - pos1.z};
}, spawnParticle(region, type, x, y, z, vx, vy, vz) {
    Network.sendToAllClients("api.particle", {p: type, d: typeof region == "number" ? region : region.getDimension(), x: x, y: y, z: z, vx: vx, vy: vy, vz: vz});
}, spawnParticles(region, arr) {
    Network.sendToAllClients("api.particle_array", {d: typeof region == "number" ? region : region.getDimension(), arr: arr});
