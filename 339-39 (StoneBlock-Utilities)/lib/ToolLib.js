LIBRARY({name: "ToolLib", version: 24, shared: true, api: "CoreEngine"});
var ToolType = {sword: {__flag: "__sword", isWeapon: true, enchantType: Native.EnchantType.weapon, damage: 4, blockTypes: ["fibre", "plant"], calcDestroyTime: function (item, coords, block, params, destroyTime, enchant) {
    if (block.id == 30) {
        return 0.08;
    }
    if (block.id == 35) {
    ...
    return destroyTime;
}}, shovel: {__flag: "__shovel", enchantType: Native.EnchantType.shovel, damage: 2, blockTypes: ["dirt"], useItem: function (coords, item, block, player) {
    if (block.id == 2 && coords.side == 1) {
        var region = BlockSource.getDefaultForActor(player);
        region.setBlock(coords.x, coords.y, coords.z, 198, 0);
        World.playSound(coords.x + 0.5, coords.y + 1, coords.z + 0.5, "step.grass", 0.5, 0.8);
        ToolLib.breakCarriedTool(1, player);
    }
}}, pickaxe: {__flag: "__pickaxe", enchantType: Native.EnchantType.pickaxe, damage: 2, blockTypes: ["stone"]}, axe: {__flag: "__axe", enchantType: Native.EnchantType.axe, damage: 3, blockTypes: ["wood"], useItem: function (coords, item, block, player) {
    var region = BlockSource.getDefaultForActor(player);
    let strippedId;
    if (block.id == 17) {
        if (block.data == 0) {
            strippedId = VanillaTileID.stripped_oak_log;
        }
        if (block.data == 1) {
            strippedId = VanillaTileID.stripped_spruce_log;
        }
        if (block.data == 2) {
            strippedId = VanillaTileID.stripped_birch_log;
        }
        if (block.data == 3) {
            strippedId = VanillaTileID.stripped_jungle_log;
        }
        ...
        if (block.id == 162) {
            if (block.data == 0) {
                strippedId = VanillaTileID.stripped_acacia_log;
            } else {
                strippedId = VanillaTileID.stripped_dark_oak_log;
            }
            region.setBlock(coords.x, coords.y, coords.z, strippedId, 0);
            ToolLib.breakCarriedTool(1, player);
        }
    }
}}, hoe: {__flag: "__hoe", useItem: function (coords, item, block, player) {
...
}}, shears: {__flag: "__shears", blockTypes: ["plant", "fibre", "wool"], modifyEnchant(enchantData, item, coords, block) {
    if (block) {
        var material = ToolAPI.getBlockMaterialName(block.id);
        if (material == "fibre" || material == "plant") {
    ...
    if (block.id == 31 || block.id == 32 || block.id == 18 || block.id == 161) {
        ...
        region.spawnDroppedItem(coords.x + 0.5, coords.y + 0.5, coords.z + 0.5, block.id, 1, block.data);
    }
    return false;
}}};
...
var ToolLib = {setTool: function (id, toolMaterial, toolType, brokenId) {
    ...
    if (!enchant) {
        enchant = ToolAPI.getEnchantExtraData();
    }
    ...
    Block.registerPopResourcesFunction(id, function (coords, block, region) {
        if (Math.random() < 0.25) {
            ...
            for (let i in drop) {
                region.spawnDroppedItem(coords.x + 0.5, coords.y + 0.5, coords.z + 0.5, drop[i][0], drop[i][1], drop[i][2], drop[i][3] || null);
            }
        }
    });
}};
...
var MiningLevel = {STONE: 1, IRON: 2, DIAMOND: 3, OBSIDIAN: 4};
