LIBRARY({name: "Harvest_Core", version: 3, shared: true, api: "CoreEngine"});
var BLOCK_TYPE_CROP = Block.createSpecialType({base: 59, opaque: false, rendertype: 6, lightopacity: 0, destroytime: 0});
var BLOCK_TYPE_PLANT = Block.createSpecialType({base: 59, opaque: false, rendertype: 1, lightopacity: 0, destroytime: 0});
...
var Harvest = {toolList: {}, dropWithoutDirt: {}, dropWithoutLeaves: {}, grassDropsArray: [], addGrassDrop: function (item) {
    ...
    Entity.setVelocity(World.drop(coords.x, coords.y, coords.z, id, count, 0), Random.Float(-0.2, 0.2), Random.Float(-0.2, 0.2), Random.Float(-0.2, 0.2));
}, dropFruit: function (id, x, y, z) {
    Entity.setVelocity(World.drop(x, y, z, id, Random.Int(1, 3), 0), Random.Float(-0.2, 0.2), Random.Float(-0.2, 0.2), Random.Float(-0.2, 0.2));
}, dropPlant: function (id, x, y, z) {
    Entity.setVelocity(World.drop(x, y, z, id, 1, 0), Random.Float(-0.2, 0.2), Random.Float(-0.2, 0.2), Random.Float(-0.2, 0.2));
}};
var CropRegistry = {cropClasses: {}, registerWithID: function (blockID, name, texture, BLOCK_TYPE) {
    if (typeof texture == "string") {
        IDRegistry.genBlockID(blockID);
        Block.createBlock(blockID, [{name: name, texture: [["empty", 0], ["empty", 0], [texture, 0]], inCreative: false}, {name: name, texture: [["empty", 0], ["empty", 0], [texture, 1]], inCreative: false}, {name: name, texture: [["empty", 0], ["empty", 0], [texture, 2]], inCreative: false}, {name: name, texture: [["empty", 0], ["empty", 0], [texture, 3]], inCreative: false}], BLOCK_TYPE);
    } else {
        IDRegistry.genBlockID(blockID);
        Block.createBlock(blockID, texture.textures, BLOCK_TYPE);
    }
}, registerClass: function (name) {
    this.cropClasses[name] = {logic: null, functions: {derive: null, grow: null, collect: null}, config: {}, heirs: {}};
...
var TreeRegistry = {treeClasses: {}, registerClass: function (name) {
    this.treeClasses[name] = {proto: null, logic: {}, deriveFunc: null, deployFunc: function () {
    }, config: {}, heirs: {}};
}, deriveTreeAsClass: function (classs, tree) {
    var cl = this.treeClasses[classs];
    if (cl) {
        cl.heirs[tree.name] = {sapling: {item: tree.sapling.item, block: tree.sapling.block}, fruit: {item: tree.fruit.item, block: tree.fruit.block}};
        if (cl.deriveFunc) {
            cl.deriveFunc(cl);
        }
    }
    ...
    standartHarvestCraftTreePrototype.addStructure([{id: params.leaves.id, data: params.leaves.data, box: {pos1: {x: -2, y: 3, z: -2}, pos2: {x: 2, y: 4, z: 2}}}, {id: params.leaves.id, data: params.leaves.data, box: {pos1: {x: -1, y: 5, z: 0}, pos2: {x: 1, y: 6, z: 0}}}, {id: params.leaves.id, data: params.leaves.data, box: {pos1: {x: 0, y: 5, z: -1}, pos2: {x: 0, y: 6, z: 1}}}, {id: params.wood.id, data: params.wood.data, box: {pos1: {x: 0, y: 0, z: 0}, pos2: {x: 0, y: 5, z: 0}}}]);
    standartHarvestCraftTreePrototype.addFruitsArea([{box: {pos1: {x: -2, y: 2, z: -2}, pos2: {x: 2, y: 2, z: 2}}}]);
    return standartHarvestCraftTreePrototype;
}};
Callback.addCallback("DestroyBlock", function (coords, block, player) {
    var trueIDs = {31: true, 175: true};
    var trueMetaS = {1: true, 2: true, 10: true};
...
Callback.addCallback("ItemUse", function (coords, item, block) {
    var trueTool = {290: true, 291: true, 292: true, 293: true, 294: true};
