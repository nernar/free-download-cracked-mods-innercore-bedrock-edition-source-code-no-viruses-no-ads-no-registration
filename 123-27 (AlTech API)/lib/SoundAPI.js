...
function Custom_PlaySoundFile(sound_file) {
	try {
		var media = null;
		for(var i in Custom_SoundPlayers){
			var SP = Custom_SoundPlayers[i];
			if(i == sound_file && SP.isPlaying()) {
				media = SP;
				break;
			}
		}
		if(media == null) {
			media = new android.media.MediaPlayer();
			Custom_SoundPlayers[sound_file] = media
			//alert(SoundPlayers[sound_file])
		}
		media.reset();
		media.setDataSource(__dir__ + "/sounds/" + sound_file);
		media.prepare();
...
function Custom_StopSoundFile(sound_file) {
	try{ var SP = Custom_SoundPlayers[sound_file]
	//alert(sound_file + ", " + SP)
	if(SP && SP.isPlaying()){
         SP.stop()
         SP.release()
         delete Custom_SoundPlayers[sound_file]
     } }catch(e){}
}
...
Callback.addCallback("LevelLeft", function(){
	StopAllSoundPlayers();
	for(var keys in Custom_SoundPlayers){
		Custom_StopSoundFile(keys)
	}
});


registerAPIUnit("PlaySoundFile", PlaySoundFile);
registerAPIUnit("Custom_PlaySoundFile", Custom_PlaySoundFile);
registerAPIUnit("Custom_StopSoundFile", Custom_StopSoundFile);
