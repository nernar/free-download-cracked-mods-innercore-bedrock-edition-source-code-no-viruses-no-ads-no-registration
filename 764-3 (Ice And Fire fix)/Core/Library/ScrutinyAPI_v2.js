...
Callback.invokeCallback("Scrutiny_give", window, tab, name, player);
	...
	getGuiBook(name, player, content, group, container, id, obj){
		...
		group.addWindowInstance("book", new UI.Window({
			...
			drawing: [
				...
				{type: "bitmap", bitmap: "achievement.book_background", within: 1000, height: y_max},
			],
			elements: elements
		}))
	},
	...
	getGuiClient(window_name, player, container, id){
		...
		group.addWindowInstance("book", new UI.Window({
			...
			drawing: [
				{type: "color", color: android.graphics.Color.argb(0, 0, 0, 0)},
				{type: "bitmap", bitmap: "achievement.book_background", within: 1000, height: 500},
			],
			elements: {
				"close": {type: "button", x: 900, y: 0, bitmap: "classic_close_button", scale: 5, clicker: {
...
function AchievementAPI(){
	...
	this.getGui = function(title, description, item){
		...
		y_default = -600;
		let content = window.getContent();
		content.elements.background = {type: "image", bitmap: "achievement.achievement_background", x: 0, y:y_default, scale: 1.5}
...
Translation.addTranslation("message.scrutiny", {
	ru: "Изучено {name}"
})
Network.addClientPacket("aw.achievement.give", function(scrutiny){
	Achievement.give(scrutiny.name, TranslationLoad.get("message.scrutiny", [["name", scrutiny.name]]), scrutiny.icon)
});
let ScrutinyAPI = {
	save: true,
	register(name, obj){
