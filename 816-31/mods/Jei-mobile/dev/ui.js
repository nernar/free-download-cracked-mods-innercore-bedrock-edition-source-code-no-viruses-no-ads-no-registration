...
let Jei = {data: {}, window: null, list: 0, items: ItemList.get(), closeValue: false, putOpen(name, setting) {
    setting.name = name;
    ...
    for (let i = 0; i < slots; i++) {
        ...
        content.elements["slot_" + i] = {type: "slot", bitmap: "_default_slot_empty", x: x * size + padding_frame, y: y, size: size, visual: true, iconScale: setting.icon_scale, source: {id: item.id, data: item.data, count: 1}, clicker: {onClick() {
            isReplace = true;
    ...
    if (setting.isClose && this.closeValue) {
        ...
        group.addWindow("close", {location: {x: 1000 - 50, y: 100, width: 50, height: 50}, elements: {button: {type: "button", bitmap: "show_jei", x: 0, y: 0, scale: 1000 / 16, clicker: {onClick() {
    ...
    let location = new UI.WindowLocation({x: setting.x});
    let self = this;
    ...
    let time = getTime();
    let pos = null;
    let main = new UI.Window({location: location.asScriptable(), drawing: [{type: "color", color: android.graphics.Color.argb(0, 0, 0, 0)}], elements: {frame: {type: "frame", bitmap: "default_container_frame", scale: 2, width: 1000, height: height, y: 0, color: android.graphics.Color.argb(0.6, 0, 0, 0), onTouchEvent(self_, event) {
        if (!setting.isTouch) {
            return;
        }
        let newTime = getTime();
        if (pos === null || newTime - time > 300) {
            ...
            if (pos !== null && (event.x - pos < -400 || event.x - pos > 400)) {
                Jei.updateList(main, setting, height, self, text, event.x - pos < -400);
                time = 0;
                pos = null;
            }
        }
    }}, prev: {type: "button", bitmap: "prev_jei_button_0", bitmap2: "prev_jei_button_1", scale: setting.size_title / 20, x: padding_frame, y: padding_frame, clicker: {onClick() {
        Jei.updateList(main, setting, height, self, text, false);
    }}}, next: {type: "button", bitmap: "next_jei_button_0", bitmap2: "next_jei_button_1", scale: setting.size_title / 20, x: 1000 - setting.size_title - padding_frame, y: padding_frame, clicker: {onClick() {
        Jei.updateList(main, setting, height, self, text, true);
    }}}, text: {type: "text", text: text, font: {color: android.graphics.Color.WHITE, size: setting.size_title - padding_frame}, y: padding_frame, x: 500 - getTextWidth(text, setting.size_title) / 2}, search: {type: "frame", bitmap: "search_frame_jei", scale: 4, width: 1000 - padding_frame * 2, height: height - _height - 15 - 30, x: padding_frame, y: _height + 5, clicker: {onLongClick() {
        ...
        runOnUiThread(function () {
            ...
            new android.app.AlertDialog.Builder(UI.getContext()).setView(editText).setNegativeButton("exit", function () {
                self = Jei;
                ...
                Jei.setupSlots(main, setting, height, self);
                main.forceRefresh();
            }).setPositiveButton("ok", new android.content.DialogInterface.OnClickListener({onClick() {
    ...
    main.setEventListener({onOpen(main) {
        runOnUiThread(function () {
    ...
    if (setting.isClose) {
        let close = new UI.Window({location: {x: setting.x - 50, height: 50, width: 50}, elements: {button: {type: "button", bitmap: "unshow_jei", x: 0, y: 0, scale: 1000 / 16, clicker: {onClick() {
            Jei.closeValue = true;
            Jei.open(setting);
        }}}}});
...
Jei.putOpen("inventory_screen", {x: 700, line: 7, isClose: true});
Jei.putOpen("creative_inventory_screen", {x: 700, line: 7, isClose: true});
Jei.putOpen("inventory_screen_pocket", {x: 700, line: 7, isClose: true});
Jei.putOpen("survival_inventory_screen", {x: 700, line: 7, isClose: true});
Jei.putOpen("innercore_generic_crafting_screen", {x: 700, line: 7, isClose: true});
Jei.putOpen("furnace_screen", {x: 700, line: 7, isClose: true});
Jei.putOpen("blast_furnace_screen", {x: 700, line: 7, isClose: true});
Jei.putOpen("smoker_screen", {x: 700, line: 7, isClose: true});
Jei.putOpen("brewing_stand_screen", {x: 700, line: 7, isClose: true});
Jei.putOpen("stonecutter_screen", {x: 700, line: 7, isClose: true});
Jei.putOpen("trade_screen", {x: 700, line: 7, isClose: true});
