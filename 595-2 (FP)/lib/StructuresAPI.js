LIBRARY({name: "StructuresAPI", version: 6, shared: false, api: "CoreEngine"});
var StructuresDB = {structures: {}, dir: "structures", versionSaver: 2};
var Structure = function (name, alerted) {
    ...
    this.addChest = function (name, te) {
        ...
        for (let i = 0; i < size; i++) {
            ...
            if (slot && slot.id != 0 && slot.count > 0) {
                let item = {id: getSID(slot.id), data: slot.data, count: slot.count};
                chest.push([i, item]);
            }
        }
        chests[name] = chest;
    ...
    this.addTileEntity = function (name, te) {
        TileEntitys[name] = {data: te.data};
        let slots = te.container.slots;
        let _slots = {};
        for (let i in slots) {
            if (slots[i].id) {
                _slots[i] = {id: getSID(slots[i].id), count: slots[i].count, data: slots[i].data};
            }
        }
    ...
    this.save = function () {
        ...
        let saveObject = {version: StructuresDB.versionSaver, structure: structure};
    ...
    this.get = function (x, y, z, rotates, return_index) {
        if (rotates == undefined) {
            rotates = [Structure.ROTATE_NONE];
        }
        if (rotates === true) {
            rotates = Structure.ROTATE_Y;
        }
        ...
        for (var i = 0; i < rotates.length; i++) {
            ...
            for (var k = 0; k < structure.length; k++) {
                ...
                if (_block.id != id || _block.data != data) {
                    break;
                }
    ...
    function build(x, y, z, rotate, progressively, time) {
        for (var i = 0; i < structure.length; i++) {
            ...
            if (progressively) {
                java.lang.Thread.sleep(time || 100);
            }
        }
    }
    this.set = function (x, y, z, rotate, progressively, time) {
        if (rotate === undefined) {
            rotate = Structure.ROTATE_NONE;
        }
        if (progressively === undefined) {
            progressively = Structure.NOT_PROGRESSIVELY;
        }
        if (time === undefined) {
            time = 0;
        }
...
Structure.ROTATE_NONE = [1, 0, 0, 0, 1, 0, 0, 0, 1];
Structure.ROTATE_90Y = [0, 0, -1, 0, 1, 0, 1, 0, 0];
Structure.ROTATE_180Y = [-1, 0, 0, 0, 1, 0, 0, 0, -1];
Structure.ROTATE_270Y = [0, 0, 1, 0, 1, 0, -1, 0, 0];
Structure.ROTATE_90X = [1, 0, 0, 0, 0, -1, 0, 1, 0];
Structure.ROTATE_180X = [1, 0, 0, 0, -1, 0, 0, 0, -1];
Structure.ROTATE_270X = [1, 0, 0, 0, 0, 1, 0, -1, 0];
Structure.ROTATE_90Z = [0, -1, 0, 1, 0, 0, 0, 0, 1];
Structure.ROTATE_180Z = [-1, 0, 0, 0, -1, 0, 0, 0, 1];
Structure.ROTATE_270Z = [0, 1, 0, -1, 0, 0, 0, 0, 1];
Structure.ROTATE_RANDOM = [Structure.ROTATE_180X, Structure.ROTATE_180Y, Structure.ROTATE_180Z, Structure.ROTATE_270X, Structure.ROTATE_270Y, Structure.ROTATE_270Z, Structure.ROTATE_90X, Structure.ROTATE_90Y, Structure.ROTATE_90Z, Structure.ROTATE_NONE];
Structure.ROTATE_ALL = Structure.ROTATE_RANDOM;
Structure.ROTATE_Y = [Structure.ROTATE_NONE, Structure.ROTATE_90Y, Structure.ROTATE_180Y, Structure.ROTATE_270Y];
Structure.MIRROR_X = [-1, 0, 0, 0, 1, 0, 0, 0, 1];
Structure.MIRROR_Y = [1, 0, 0, 0, -1, 0, 0, 0, 1];
Structure.MIRROR_Z = [1, 0, 0, 0, 1, 0, 0, 0, -1];
...
if (JSON.parse(FileTools.ReadText(__dir__ + "build.config")).defaultConfig.buildType == "develop") {
    ModAPI.addAPICallback("WorldEdit", function (WorldEdit) {
        ...
        WorldEdit.addCommand({name: "/save", description: "Save structure.", args: "<file_name> [-a] [-x] [-y] [-z]", selectedArea: true, event: function (args) {
            ...
            for (x = pos.pos1.x; x <= pos.pos2.x; x++) {
                for (y = pos.pos1.y; y <= pos.pos2.y; y++) {
                    for (z = pos.pos1.z; z <= pos.pos2.z; z++) {
                        ...
                        if (args.indexOf("-a") == -1 && block.id == 0) {
                            continue;
                        }
...
Translation.addTranslation("Saved to %s", {ru: "\u0421\u043e\u0445\u0440\u0430\u043d\u0435\u043d\u043e \u0432 %s", en: "Saved to %s"});
Translation.addTranslation("Save structure.", {ru: "\u0421\u043e\u0445\u0440\u0430\u043d\u0438\u0442\u044c \u0441\u0442\u0440\u0443\u043a\u0442\u0443\u0440\u0443.", en: "Save structure."});
Translation.addTranslation("Structure \"%s\" not found.", {ru: "\u0421\u0442\u0440\u0443\u043a\u0442\u0443\u0440\u0430 \"%s\" \u043d\u0435 \u043d\u0430\u0439\u0434\u0435\u043d\u0430.", en: "Structure \"%s\" not found."});
Translation.addTranslation("Unknown version \"%s\".", {ru: "\u041d\u0435\u0438\u0437\u0432\u0435\u0441\u0442\u043d\u0430\u044f \u0432\u0435\u0440\u0441\u0438\u044f \"%s\".", en: "Unknown version \"%s\"."});
Translation.addTranslation("The zero point of the structure is set.", {ru: "\u041d\u0443\u043b\u0435\u0432\u0430\u044f \u0442\u043e\u0447\u043a\u0430 \u0441\u0442\u0440\u0443\u043a\u0442\u0443\u0440\u044b \u0443\u0441\u0442\u0430\u043d\u043e\u0432\u043b\u0435\u043d\u0430.", en: "The zero point of the structure is set."});
