LIBRARY({name: "ToolLib", version: 20, shared: true, api: "CoreEngine"});
var ToolType = {sword: {isWeapon: true, enchantType: Native.EnchantType.weapon, damage: 4, blockTypes: ["fibre", "plant"], calcDestroyTime: function (item, coords, block, params, destroyTime, enchant) {
    ...
    if (block.id == 2 && coords.side == 1) {
        ...
        World.playSoundAt(coords.x + 0.5, coords.y + 1, coords.z + 0.5, "step.grass", 0.5, 0.8);
    ...
    if ((block.id == 2 || block.id == 3) && coords.side == 1) {
        ...
        World.playSoundAt(coords.x + 0.5, coords.y + 1, coords.z + 0.5, "step.gravel", 1, 0.8);
        ToolLib.breakCarriedTool(1);
    }
}}};
