...
for(i = arthropod.length; i--;){
	...
	Entity.enemy[Native.EntityType[arthropod[i]]] = true;
}

for(i = undead.length; i--;){
	Entity.undead[Native.EntityType[undead[i]]] = true;
	Entity.enemy[Native.EntityType[undead[i]]] = true;
}

for(i = other.length; i--;){
	Entity.enemy[Native.EntityType[other[i]]] = true;
}
...
Entity.isEnemy = function(ent){
	const type = this.getType(ent);
	return this.enemy[type];
};

Entity.isBoss = function(ent){
	const type = this.getType(ent);
