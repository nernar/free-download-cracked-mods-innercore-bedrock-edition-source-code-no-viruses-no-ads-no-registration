LIBRARY({name: "BookHelper", version: 1, shared: true, api: "CoreEngine"});
...
function Book(identifier) {
    ...
    this.open = function (player, page) {
        ...
        if (client) {
            client.send("book_helper.open", {identifier: identifier, page: page});
        }
        return this;
    };
...
let LOCATION = new UI.WindowLocation({x: 100, width: 800});
...
function Page() {
    ...
    pages[true] = [];
    pages[false] = [];
    let link = {next: undefined, pre: undefined};
    ...
    this.open = function (book, ui_) {
        let content = {location: LOCATION.asScriptable(), drawing: [{type: "color", color: android.graphics.Color.argb(0, 0, 0, 0)}, {type: "bitmap", bitmap: "book_background", width: 1000, height: HEIGHT}], elements: {"close": {type: "close_button", x: 900, y: 0, bitmap: "classic_close_button", scale: 5}}};
        updateUi(book, content, true, this);
        updateUi(book, content, false, this);
        let self = this;
        if (link.next) {
            content.elements["next_button"] = {type: "button", x: 910, y: HEIGHT - 45, bitmap: "next_page", scale: 3, clicker: {onClick() {
                self.close(book);
                book.openClient(link.next);
            }}};
        }
        if (link.pre) {
            content.elements["pre_button"] = {type: "button", x: 50, y: HEIGHT - 45, bitmap: "pre_page", scale: 3, clicker: {onClick() {
                self.close(book);
...
function TextTypeElement(text, obj) {
    ...
    this.build = function (book, page, i, isLeft, data) {
        ...
        let result = [{type: "text", text: text_.result, x: data.x, y: data.y, multiline: true, font: {color: color, size: relSize * size, underline: obj.underline, cursive: obj.cursive, bold: obj.bold, alignment: obj.alignment}, clicker: {onClick() {
            if (obj.link) {
                page.close(book);
...
function SlotsTypeElement(slots) {
    ...
    this.build = function (book, page, i, isLeft, data) {
        ...
        for (let i in slots) {
            ...
            arr.push({type: "slot", bitmap: slot.texture, x: x, y: y, size: size, visual: true, source: slot.item, clicker: {onClick() {
                if (slot.link) {
                    page.close(book);
...
EXPORT("BookElements", {Style: {Text: Text, Slot: Slot}, Text: TextTypeElement, Slot: SlotsTypeElement});
