LIBRARY({name: "RenderUtil", version: 4, shared: true, api: "CoreEngine"});
let RenderAPI = {models: {}, Model() {
    let boxes = {};
    this.addBoxByBlock = function (name, x1, y1, z1, x2, y2, z2, id, data) {
        boxes[name || Object.keys(boxes).length] = {x1: x1, y1: y1, z1: z1, x2: x2 || x1, y2: y2 || y1, z2: z2 || z1, id: id || 1, data: data || 0};
        return this;
    };
    ...
    this.play = function (x, y, z, infinite) {
        let _this = this;
        handler.start();
        Updatable.addUpdatable({tick: 0, update() {
    ...
    this.setHandler = function (obj) {
        handler = {start: obj.start || function () {
    ...
    this.getModels = function (tick) {
        ...
        for (let i in arr) {
            let t = arr[i];
            if (tick < t && tick >= t_p) {
                return {pre: t_p == 0 ? model : obj[t_p], post: obj[t] || obj[t_p], time: t - t_p};
            } else {
                t_p = t;
            }
        }
        return {pre: obj[arr[arr.length - 1]], post: obj[arr[arr.length - 1]], time: 1};
    };
