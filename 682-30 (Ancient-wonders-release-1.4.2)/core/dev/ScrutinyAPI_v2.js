Saver.addSavesScope("Save.lib.ScrutinyAPI", function read(scope) {
    if (ScrutinyAPI.save) {
        ScrutinyAPI_V2.scrutiny = scope.save || {};
    }
}, function save() {
    return {save: ScrutinyAPI_V2.scrutiny};
});
...
let ScrutinyWindow = {left: sh * 0.3, right: sh * 0.3, top: sh * 0.05, bottom: sh * 0.1, size_y: sh - (this.top + this.bottom), height: 540};
let ScrutinyAPI_V2 = {windows: {}, scrutiny: {}, players: {}, register(name, obj) {
    obj = obj || {};
    this.windows[name] = {tabs: {}, scale: obj.scale || 3, frame: obj.frame || "default_frame_bg_dark", default_tab: obj.default_tab || "", default_bitmap: obj.default_bitmap || "classic_frame_bg_light_border", default_bitmap_click: obj.default_bitmap_click || "workbench_frame3"};
}, setTab(window, tab, obj) {
    obj.title = obj.title || "", obj.id = obj.id || 0;
    obj.icon = obj.icon || 0;
    obj.width = obj.width || 0;
    obj.height = obj.height || 0;
    obj.isVisual = obj.isVisual || function () {
        return true;
    };
    obj.scrutinys = {};
    obj.name = tab;
    ...
    if (!this.scrutiny[window][tab]) {
        this.scrutiny[window][tab] = {player: {}};
    }
    ...
    Network.sendToAllClients("SC.API.give", {value: ScrutinyAPI_V2.scrutiny, server: parseInt("" + Player.get())});
    return true;
}, getStr(str, count) {
    let chars = str.split("");
    let a = 1;
    ...
    return string;
}, elements: {}, registerDrawingElement(name, func) {
    ...
    let elements = {"close": {type: "button", x: 900, y: 0, bitmap: "classic_close_button", scale: 5, clicker: {onClick() {
    ...
    group.addWindowInstance("book", new UI.Window({location: {padding: {bottom: ScrutinyWindow.bottom - 10, top: ScrutinyWindow.top - 10, right: ScrutinyWindow.right - 10, left: ScrutinyWindow.left - 10}, scrollX: 100, forceScrollY: true, forceScrollX: true}, drawing: [{type: "color", color: android.graphics.Color.argb(0, 0, 0, 0)}, {type: "bitmap", bitmap: "book_background", within: 1000, height: y_max}], elements: elements}));
}, client_open_tab: "", updateIcon(obj) {
    ...
    for (let i in scrutinys) {
        ...
        content.elements["scrutiny_" + scrutiny.scrutiny + tab.name] = {type: "slot", x: scrutiny.x, y: scrutiny.y, visual: true, bitmap: this.isScrutiny(player, name, tab.name, scrutiny.scrutiny) ? scrutiny.bitmap_click : scrutiny.bitmap, size: scrutiny.size, clicker: {onClick() {
        ...
        if (icon.ids) {
            ...
            let i = 0;
            setTimeout(ScrutinyAPI_V2.updateIcon, icon.time || 10, {tab: tab, icon: icon, name_slot: name_slot, i: i, container: container});
        } else {
    ...
    let window = new UI.Window({location: {padding: {bottom: ScrutinyWindow.bottom, top: ScrutinyWindow.top, right: ScrutinyWindow.right, left: ScrutinyWindow.left}}, drawing: [{type: "color", color: android.graphics.Color.argb(0, 0, 0, 0)}], elements: {"close": {type: "close_button", x: 850, y: -25, bitmap: ScrutinyAPI_V2.windows[window_name].close_bitmap || "classic_close_button", scale: 5}}});
    let tabs = Object.keys(this.windows[window_name].tabs);
    let window_tab = new UI.Window({location: {padding: {bottom: ScrutinyWindow.bottom + 20, top: ScrutinyWindow.top + 30, right: ScrutinyWindow.right + 70, left: ScrutinyWindow.left + 70}, scrollY: 100, scrollX: 100, forceScrollY: true, forceScrollX: true}, drawing: [{type: "color", color: android.graphics.Color.argb(0, 0, 0, 0)}], elements: {}});
    ...
    group.addWindowInstance("book", new UI.Window({location: {padding: {bottom: 10000, top: 1000, right: 1000, left: 1000}, scrollY: 100, scrollX: 100, forceScrollY: true, forceScrollX: true}, drawing: [{type: "color", color: android.graphics.Color.argb(0, 0, 0, 0)}, {type: "bitmap", bitmap: "book_background", within: 1000, height: 500}], elements: {"close": {type: "button", x: 900, y: 0, bitmap: "classic_close_button", scale: 5, clicker: {onClick(position, container, tileEntity, window) {
...
ScrutinyAPI_V2.registerDrawingElement("text", function (obj, x, y) {
    ...
    return {y: 10 + ((obj.size || 25) * Math.ceil(text.split("").length / (obj.chars || Math.floor(310 / (obj.size / 2))))), elem: [{type: "text", text: ScrutinyAPI_V2.getStr(text, obj.chars || Math.floor(310 / (obj.size / 2))) || "", size: obj.size || 25, x: x, y: y, multiline: true}]};
});
ScrutinyAPI_V2.registerDrawingElement("slots", function (obj, x, y, container, i, pos) {
    ...
    return {y: obj.size || 30, elem: slots};
});
...
function createAnimation(_duration, _updateFunc) {
    ...
    if (_updateFunc) {
        animation.addUpdateListener({onAnimationUpdate(updatedAnim) {
            _updateFunc(updatedAnim.getAnimatedValue(), updatedAnim);
        }});
    }
    JAVA_HANDLER_THREAD.post({run() {
        animation.start();
    }});
    return animation;
}
function AchievementAPI() {
    ...
    let window = new UI.Window({location: {x: 650, width: 350, y: 0, height: 150}, drawing: [{type: "color", color: android.graphics.Color.argb(0, 0, 0, 0)}], elements: {}});
...
let ScrutinyAPI = {save: true, register(name, obj) {
    ...
    if (value) {
        ...
        let client = Network.getClientForPlayer(player);
        if (client) {
            client.send("aw.achievement.give", {window: window, tab: tab, name: name});
        }
    }
    return value;
}, addScrutiny(window, tab, name, obj) {
    ScrutinyAPI_V2.setScrutiny(window, tab, name, {name: obj.name, x: obj.x, y: obj.y, cellX: obj.cellX, cellY: obj.cellY, size: obj.size, icon: obj.item, lines: obj.line, line_color: obj.line_color, isDone: (function (arr2) {
        let arr = [];
        for (let i in arr2) {
        ...
        return arr;
    })(obj.isDone || obj.isVisual), isVisual: (function (arr2) {
        let arr = [];
        for (let i in arr2) {
            if (typeof (arr2[i]) == "string") {
                arr.push([tab, arr2[i]]);
        ...
        return arr;
    })(obj.isVisual), book_post: obj.bookPost, book_pre: obj.bookPre});
}, open(player, name) {
