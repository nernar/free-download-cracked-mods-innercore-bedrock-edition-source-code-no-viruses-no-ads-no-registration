let OffhandModule = WRAP_NATIVE("OffhandModule");
...
function addGlowingItemID(id, level, bool) {
    ...
    glowingItemID[id] = level;
    OffhandModule.allowVanillaItem(id);
    if (bool) {
        glowingDroppedItemID[id] = level;
    }
}
...
Callback.addCallback("ServerPlayerTick", function (player) {
    if (World.getThreadTime() % UPDATE_TIME == 0) {
        ...
        for (let i in ents) {
            ...
            let offHand = Entity.getOffhandItem(ent);
            if (glowingItemID[item.id]) {
                lightLevel += glowingItemID[item.id];
            }
            if (glowingItemID[offHand.id]) {
                lightLevel += glowingItemID[offHand.id];
            }
