LIBRARY({name: "WheatAPI", version: 1.5, shared: false, api: "CoreEngine"});
var canTileBeReplaced = ModAPI.requireGlobal("canTileBeReplaced");
Block.createSpecialType({base: 59, opaque: false, lightopacity: 0, destroytime: 0}, "crop");
var DataRegistry = {drop: {grass: []}, renderer: {}, crop: {id: {}, data: {}}, tooltip: {}};
...
var RendererRegistry = {getRenderModel: function (id, data) {
...
var CropRegistry = {addGrassDrop: function (id, count, data) {
    data = data || 0;
    ...
    if (!this.isCrop(ItemID[uid])) {
        if (data) {
            ...
            data = {farmland: {id: 60, data: -1}, growTime: 3600, growSpeed: 1, growLight: 8, drop: []};
        }
        ...
        Callback.addCallback("PreLoaded", function () {
            ...
            if (tile) {
                ...
                if (prototype.defaultValues) {
                    ...
                    prototype.defaultValues = {grow_speed: speed, grow_time: time, grow_light: light, grow_progress: 0, grow_count: texture.crop.length - 1};
                }
                ...
                prototype.tick = function () {
                    ...
                    if (this.data.grow_progress > this.data.grow_time) {
                        this.data.grow_progress = this.data.grow_time;
                    }
                };
                ...
                TileEntity.registerPrototype(BlockID[uid], {defaultValues: {grow_speed: speed, grow_time: time, grow_light: light, grow_progress: 0, grow_count: texture.crop.length - 1}, CropRender: function () {
                    if (this.data.grow_progress >= this.data.grow_time) {
                        RendererRegistry.mapAtCoords(this.x, this.y, this.z, BlockID[uid], this.data.grow_count);
                    } else {
                        RendererRegistry.mapAtCoords(this.x, this.y, this.z, BlockID[uid], Math.round(this.data.grow_progress / this.data.grow_time * (this.data.grow_count - 1)));
...
Block.registerDropFunctionForID(175, function (coords, id, data, level, enchant) {
    var grass = DataRegistry.drop.grass;
    if (data == 2) {
        if (enchant.silk) {
            return [[id, 1, data]];
...
var UIRegistry = {getScreenWidth: function () {
    ...
    var drawing = [{type: "background", color: android.graphics.Color.TRANSPARENT}, {type: "frame", x: 0, y: 0, width: 1000, height: 750, bitmap: state.background, scale: 6}];
    for (let i in state.drawing) {
        drawing.push(state.drawing[i]);
    }
    ...
    for (let i in state.elements) {
        element[i] = state.elements[i];
    }
    for (let i = 9; i <= 44; i++) {
        element["invSlot" + i] = {type: "invSlot", x: 163 + (i % 9) * 75, y: 345 + (i / 9 | 0) * 75, size: 80, index: i};
    }
    var ui = UI.Window({location: {padding: {left: width / 10, right: width / 10, bottom: (height - 0.6 * width) / 2, top: 0}}, params: {slot: "classic_slot", inv_slot: "classic_slot"}, drawing: drawing, elements: element});
    ui.setInventoryNeeded(true);
    ui.setBlockingBackground(true);
    return ui;
}};
var GameRegistry = {getSetting: function (data) {
...
var PlayerRegistry = {getInventoryItemCount: function (id, data) {
    var count = 0;
...
var ItemRegistry = {getTooltip: function (id) {
...
EXPORT("wheat", {ui: UIRegistry, game: GameRegistry, item: ItemRegistry, crop: CropRegistry, player: PlayerRegistry, requireGlobal: function (command) {
    return eval(command);
}});
