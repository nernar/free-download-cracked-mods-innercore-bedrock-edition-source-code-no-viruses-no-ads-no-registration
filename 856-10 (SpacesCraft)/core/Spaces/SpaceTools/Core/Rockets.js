...
if(block.id == BlockID.rocket_1_tier){region.setBlock(coords.x, coords.y, coords.z, VanillaBlockID.air); region.setBlock(coords.x, coords.y, coords.z, BlockID.Padding1lvl);
					SPC_b1.close();
					SPC_b2.close();
					SPC_b3.close();
...
var region = BlockSource.getDefaultForActor(player);
   ...
   if(region.getBlockId(place.x,place.y-1,place.z)==BlockID.Padding1lvl){ 
        region.setBlock(place.x, place.y-1, place.z, BlockID.rocket_1_tier);  
         Player.setCarriedItem(player, item.id, item.count - 1, item.data);
...
Rocktmesh.importFromFile(__dir__+"/models/rocket_tier3.obj","obj",{translation: [-0.5,-0.5,0.0]}); 
