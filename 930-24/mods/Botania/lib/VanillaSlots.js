LIBRARY({name: "VanillaSlots", version: 4, shared: true, api: "CoreEngine"});
...
if (!Object.assign) {
    Object.defineProperty(Object, "assign", {enumerable: false, configurable: true, writable: true, value: function (target, firstSource) {
        "use strict";
...
function defaultChestData() {
    return {container: null, currentWindow: null, start: false, item: {maxCount: 0, count: 0}, barData: {name: "", x: 0, y: 0}, pre_selectedSlot: null, selectedSlot: null, selectedSlotType: null, lastClickTime: 0, anim: {pos1: {slotSize: 0, window: null, x: 0, y: 0}, pos2: {x: 0, y: 0}}};
}
var chestData = defaultChestData();
var defaultFunctions = {SlotToSlot: function (container, eventData, connectedClient) {
    ...
    if ((slot2.id != slot1.id || slot2.data != slot1.data || (slot2.extra != slot1.extra && ((!slot2.extra || slot2.extra.getAllCustomData()) != (!slot1.extra || slot1.extra.getAllCustomData())))) && slot2.id != 0) {
        ...
        if (transferPolicy1 && transferPolicy1.transfer(container, eventData.slot1, slot1.id, slot1.count, slot1.data, slot1.extra, connectedClient.getPlayerUid()) != slot1.count) {
            return;
        }
        if (transferPolicy3 && transferPolicy3.transfer(container, eventData.slot2, slot2.id, slot2.count, slot2.data, slot2.extra, connectedClient.getPlayerUid()) != slot2.count) {
            return;
        }
        if (transferPolicy2 && transferPolicy2.transfer(container, eventData.slot2, slot1.id, slot1.count, slot1.data, slot1.extra, connectedClient.getPlayerUid()) != slot1.count) {
            return;
        }
        if (transferPolicy4 && transferPolicy4.transfer(container, eventData.slot1, slot2.id, slot2.count, slot2.data, slot2.extra, connectedClient.getPlayerUid()) != slot2.count) {
            return;
        }
    ...
    if (transferPolicy1) {
        _count = (transferCount = transferPolicy1.transfer(container, eventData.slot1, slot1.id, _count, slot1.data, slot1.extra, connectedClient.getPlayerUid())) != undefined && transferCount != null ? transferCount : _count;
    }
    if (_count <= 0) {
        return;
    }
    if (transferPolicy2) {
        _count = (transferCount = transferPolicy2.transfer(container, eventData.slot2, slot1.id, _count, slot1.data, slot1.extra, connectedClient.getPlayerUid())) != undefined && transferCount != null ? transferCount : _count;
    }
    if (_count <= 0) {
        return;
    }
    ...
    if ((slot2.id != slot1.id || slot2.data != slot1.data || (slot2.extra != slot1.extra && ((!slot2.extra || slot2.extra.getAllCustomData()) != (!slot1.extra || slot1.extra.getAllCustomData())))) && slot2.id != 0) {
        ...
        if (transferPolicy2 && transferPolicy2.transfer(container, eventData.slot1, slot2.id, slot2.count, slot2.data, slot2.extra, connectedClient.getPlayerUid()) != slot2.count) {
            return;
        }
        player.setInventorySlot(eventData.slot2, slot1.id, slot1.count, slot1.data, slot1.extra);
        container.setSlot(eventData.slot1, slot2.id, slot2.count, slot2.data, slot2.extra);
...
function registerForWindow(_window, _container, _options) {
    ...
    function startAnim(window, slot, size, size1) {
        ...
        content.elements[slot_id] = {type: "slot", x: pos1.x - size / 2, y: pos1.y - size / 2, size: (size || 70) + _options.bonusSize, source: {id: slot.id, count: 1, data: slot.data, extra: slot.extra || null}, bitmap: "_default_slot_empty", visual: true};
        window.forceRefresh();
    ...
    mainWindow.setEventListener({onClose: function () {
        if (!groupWindow) {
            overlayWindow.close();
        }
    }, onOpen: function (window) {
        var elements = window.getElements();
        if (elements.get("selection1___")) {
            elements.get("selection1___").setPosition(-1000, -1000);
        }
        if (elements.get("scale1___")) {
            elements.get("scale1___").setPosition(-1000, -1000);
        }
    ...
    function slotOnTouchEvent(element, event) {
        ...
        if (event.type == "CLICK" && chestData.selectedSlot != null) {
            if (chestData.selectedSlot == slot_id) {
                ...
                if (java.lang.System.currentTimeMillis() - chestData.lastClickTime <= 500) {
                    ...
                    if (item.count < maxStack) {
                        ...
                        chestData.anim.pos2 = {window: element.window, x: element.window.location.windowToGlobal(this.x + slotSize / 2) + element.window.location.x, y: element.window.location.windowToGlobal((this.y + slotSize / 2) - scrollY / element.window.getScale()) + element.window.location.y};
                        if (currentSlotType) {
                            for (var i in slots) {
                                if (i[0] == "$") {
                                    continue;
                                ...
                                var item2 = slots[i];
                                if (!item2 || i == slot_id || item2.id != item.id || item2.data != item.data || (item2.extra != item.extra && ((!item2.extra || item2.extra.getAllCustomData()) != (!item.extra || item.extra.getAllCustomData())))) {
                                    continue;
                                }
                                ...
                                chestData.anim.pos1 = {window: thiswindow, slotSize: thiswindowElements[i].size || 60, x: thiswindow.location.windowToGlobal(thiswindowElements[i].x + thiswindowElements[i].size / 2) + thiswindow.location.x, y: thiswindow.location.windowToGlobal((thiswindowElements[i].y + thiswindowElements[i].size / 2) - _scrollY / thiswindow.getScale()) + thiswindow.location.y};
                            ...
                            if (needCount > 0) {
                                for (var i = 0; i <= 35; i++) {
                                    ...
                                    if (!item2 || item2.id != item.id || item2.data != item.data || (item2.extra != item.extra && ((!item2.extra || item2.extra.getAllCustomData()) != (!item.extra || item.extra.getAllCustomData())))) {
                                        continue;
                                    }
                                    var _count = Math.min(item2.count, needCount);
                                    needCount -= _count;
                                    ...
                                    var _scrollY = getScrollY(thiswindow);
                                    var thiswindowElements = thiswindow.getContent().elements;
                                    chestData.anim.pos1 = {window: thiswindow, slotSize: thiswindowElements[slotNameByIndex[i]].size || 60, x: thiswindow.location.windowToGlobal(thiswindowElements[slotNameByIndex[i]].x + thiswindowElements[slotNameByIndex[i]].size / 2) + thiswindow.location.x, y: thiswindow.location.windowToGlobal(((thiswindowElements[slotNameByIndex[i]].y + thiswindowElements[slotNameByIndex[i]].size / 2) * thiswindow.getScale() - _scrollY) / thiswindow.getScale()) + thiswindow.location.y};
                ...
                if (elements___2.containsKey("scale1___")) {
                    elements___2.get("scale1___").setPosition(-300, -300);
                }
                if (elements___.containsKey("scale1___")) {
                    elements___.get("scale1___").setPosition(-300, -300);
                }
                chestData.container.setScale("scale1___", 0);
                if (elements___2.containsKey("selection1___")) {
                    elements___2.get("selection1___").setPosition(-1000, -1000);
                }
                if (elements___.containsKey("selection1___")) {
                    elements___.get("selection1___").setPosition(-1000, -1000);
                }
                return;
            }
            ...
            var scrollY = getScrollY(element.window);
            chestData.anim.pos2 = {x: element.window.location.windowToGlobal(this.x + slotSize / 2) + element.window.location.x, y: element.window.location.windowToGlobal((this.y + slotSize / 2) - scrollY / element.window.getScale()) + element.window.location.y};
            ...
            var _pos2 = {window: chestData.anim.pos2, x: chestData.anim.pos2.x, y: chestData.anim.pos2.y};
            ...
            if (item.id != 0 && (item2.id != item.id || item2.count > Item.getMaxStack(item.id) - item.count)) {
                startAnim(overlayWindow, item, element.window.location.windowToGlobal(slotSize), chestData.anim.pos2.window.location.windowToGlobal(chestData.anim.pos2.slotSize || 251));
            }
            if (elements___2.containsKey("scale1___")) {
                elements___2.get("scale1___").setPosition(-300, -300);
            }
        ...
        if (event_type == "DOWN") {
            if (item.id == 0 || chestData.selectedSlot != null) {
                return;
            }
            chestData.item = {maxCount: item.count, count: 1};
            chestData.pre_selectedSlot = slot_id;
            chestData.start = World.getThreadTime() + 10;
            chestData.tickStarted = false;
            chestData.barData = {name: "scale1___", x: this.x + (slotSize - _elements_["scale1___"].scale * 15) / 2, y: this.y};
            ...
            chestData.anim.pos1 = {window: element.window, slotSize: slotSize, pre_x: this.x, pre_y: this.y};
        }
        if (event_type == "UP") {
            if (chestData.selectedSlot != null || !chestData.start) {
                return;
            }
            ...
            if (!chestData.tickStarted) {
                chestData.item.count = chestData.item.maxCount;
            }
        }
    }
    var slotNameByIndex = {};
    ...
    for (var k in allWindows) {
        ...
        if (overlayWindow && overlayWindow == ___window) {
            continue;
        }
        if (mainWindow != ___window) {
            ___window.setEventListener({onClose: function () {
            }, onOpen: function (window) {
                var elements = window.getElements();
                if (elements.get("selection1___")) {
                    elements.get("selection1___").setPosition(-1000, -1000);
        ...
        windowContent.elements["scale1___"] = {type: "scale", x: -300, y: -300, z: 100001, direction: 0, bitmap: "___transferBarFull", background: "___transferBarEmpty", value: 0, scale: 60 / 108 * 100 / 15};
        windowContent.elements["selection1___"] = {type: "image", x: -1000, y: -1000, z: 100000, bitmap: "style:selection", width: 100, height: 100};
        for (var i in windowContent.elements) {
            if ((windowContent.elements[i].type.indexOf("slot") == -1 && windowContent.elements[i].type.indexOf("Slot") == -1) || windowContent.elements[i].clicker || windowContent.elements[i].visual || windowContent.elements[i].onTouchEvent) {
                continue;
            }
            if (windowContent.elements[i].type == "invSlot") {
                slotNameByIndex[windowContent.elements[i].index] = i;
                windowContent.elements["_CLICKFRAME_" + (++uniqueId_)] = {type: "frame", x: windowContent.elements[i].x, y: windowContent.elements[i].y, z: -100, width: windowContent.elements[i].size || 60, height: windowContent.elements[i].size || 60, bitmap: "_default_slot_empty", slot_id: windowContent.elements[i].index || 0, slot_size: windowContent.elements[i].size || 60, slot_textid: i, scale: 1, onTouchEvent: slotOnTouchEvent};
                continue;
            }
    ...
    if (_container) {
        registerServerEventsForContainer(_container);
    }
}
function registerForTile(id, window) {
    Callback.addCallback("PreLoaded", function () {
        ...
        if (!Prototype) {
            throw "Tile entity with " + id + " id not registered";
        }
        var window = window || Prototype.getScreenByName("main");
        if (!window) {
            throw "Cannot be get window from tileentity(" + id + ") please pass it as the second parameter";
        }
        ...
        Prototype.init = function () {
            ...
            if (this.__init_______________) {
                this.__init_______________();
            }
        };
