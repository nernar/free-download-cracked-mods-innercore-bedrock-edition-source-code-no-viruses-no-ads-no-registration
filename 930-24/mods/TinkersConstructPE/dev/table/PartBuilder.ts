...
const PartBuilderWindow = new class extends CraftingWindow {
    ...
    override onUpdate(container: ItemContainer): void {
        ...
        if(slotPattern.id === ItemID.tcon_pattern_blank && patternData){
            ...
            for(let key in Material){
                ...
                if(item && item.id === slotMaterial.id && (item.data === -1 || item.data === slotMaterial.data)){
                    ...
                    textTitle = Material[key].getLocalizedName();
                    textStats = translate("Head") + "\n" +
                                translate("Durability: ") + statsHead.durability + "\n" +
                                translate("Mining Tier: ") + MiningLvName[statsHead.level] + "\n" +
                                translate("Mining Speed: ") + statsHead.speed + "\n" +
                                translate("Melee Damage: ") + statsHead.attack + "\n" +
                                "\n" +
                                translate("Handle") + "\n" +
                                translate("Multiplier: ") + statsHandle.modifier + "\n" +
                                translate("Durability: ") + statsHandle.durability + "\n" +
                                "\n" +
                                translate("Extra") + "\n" +
                                translate("Durability: ") + statsExtra.durability;
                    if(!Material[key].isMetal){
                        textCost = translate("Material value: %s", slotMaterial.count + " / " + patternData.cost);
        ...
        container.setText("textTitle", textTitle || translate("Part Builder"));
        container.setText("textStats", textStats || addLineBreaks(18, translate("Here you can craft tool parts to fulfill your tinkering fantasies.")) + "\n\n" + addLineBreaks(18, translate("To craft a part simply put a blank pattern into the left slot and select the part you want. The remaining slot holds the material you want to craft your part out of.")));
        container.sendChanges();
