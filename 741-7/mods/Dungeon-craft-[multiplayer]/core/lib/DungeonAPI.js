LIBRARY({name: "DungeonAPI", version: 9, api: "CoreEngine"});
function random(max) {
    return Math.floor(Math.random() * max);
}
let StructureDir = "structure";
var DungeonArr = {registerStructure: function (name) {
    return {name: name, structure: [], addBlock: function (identifier) {
        ...
        for (let i in arr) {
            ...
            let id = "0.0.0.0.0";
            if (arr2[3].id) {
                id = Dungeon.generateionIdentifier({id: arr2[3].id, data: arr2[3].data, x: arr2[0], y: arr2[1], z: arr2[2]});
            } else {
                id = Dungeon.generateionIdentifier({id: arr2[3], data: 0, x: arr2[0], y: arr2[1], z: arr2[2]});
            }
            arr3.push(id);
        }
        this.structure = arr3;
...
var Dungeon = {getArrayBlock: function (name, parameter, value) {
    let arr = [];
    ...
    for (let i in arr) {
        ...
        switch (rotation || 0) {
          case 0:
            var coords = {x: arr3[2], y: arr3[3], z: arr3[4]};
            break;
          case 1:
            var coords = {x: arr3[4], y: arr3[3], z: arr3[2]};
            break;
          case 2:
            var coords = {x: -arr3[2], y: arr3[3], z: arr3[4]};
            break;
    ...
    let structure = {version: 3, structure: arr3};
    ...
    for (let i in arr) {
        ...
        let id = "0.0.0.0.0";
        if (arr2[3].id) {
            id = this.generateionIdentifier({id: arr2[3].id, data: arr2[3].data, x: arr2[0], y: arr2[1], z: arr2[2]});
        } else {
            id = this.generateionIdentifier({id: arr2[3], data: 0, x: arr2[0], y: arr2[1], z: arr2[2]});
        }
        arr3.push(id);
    }
    ...
    let ide = identifier;
    let arr3 = ide.split(".");
    let identifie = {id: this.getBlockID(arr3[0]), data: parseInt(arr3[1]), x: parseInt(arr3[2]), y: parseInt(arr3[3]), z: parseInt(arr3[4])};
    return identifie;
...
function DungeonAPI(path) {
    let code = {isSetBlock: function (x, y, z, id, data, identifier, packet, dimension) {
        return true;
...
let ItemGenerateAPI = {deb: false, setDir: function (path) {
    dir = path;
...
function ItemGenerate() {
    this.generateion = [];
    this.Prototype = {isGenerate: function (slot, x, y, z, random, id, data, count, packet, dimension) {
        return true;
    ...
    this.fillChest = function (x, y, z, dimension, packet) {
        ...
        if (container) {
            ...
            for (i in this.generateion) {
                let item = {id: this.generateion[i].id, data: this.generateion[i].data, count: this.generateion[i].count};
            ...
            for (i in arr) {
                ...
                if (ch[1]) {
                    ...
                    if (ch[0] == 1) {
                        ...
                        if (ch[0].split("").length <= 1) {
                            ch = ch[0] + "0";
                        }
                    }
                }
                recipeList.push({max: item.count.max, min: item.count.min, chance: ch + "%", input: [], output: [{id: item.id, data: item.data}]});
            }
            RV.registerRecipeType(type, {title: title, contents: {icon: 54, params: {slot: "_default_slot_light"}, drawing: [{type: "bitmap", x: 100, y: 100, scale: 0.5, bitmap: "chest_rv"}], elements: {output0: {x: 300, y: 150, size: 120}, textMax: {type: "text", x: 490, y: 110, font: {size: 40}}, textMin: {type: "text", x: 490, y: 160, font: {size: 40}}, textChance: {type: "text", x: 490, y: 210, font: {size: 40}}}}, recipeList: recipeList, onOpen: function (elements, data) {
...
var File = {update1: function (file, value) {
