Callback.addCallback("GenerateCustomDimensionChunk", function (chunkX, chunkZ, random, dimensionId) {
    if (dimensionId == rai1.id) {
        if (Math.random() <= 0.6) {
            ...
            if (World.getBlock(coords.x, coords.y, coords.z) == BlockID.stone2) {
...
Recipes.addShaped({id: ItemID.angelAmulet, data: Item.getMaxDamage(ItemID.angelAmulet), count: 1}, [" x ", "xox", " x "], ["x", ItemID.clitok, 0, "o", ItemID.angelHeart, 0]);
...
function itemInHotbar(id, player) {
    ...
    for (let i = 0; i < 8; i++) {
        ...
        if (slot.id == id) {
            return {slot: i, id: slot.id, count: slot.count, data: slot.data, extra: slot.extra};
        }
    }
    return {};
}
...
Callback.addCallback("ItemUse", function (coords, item, block, isExternal, player) {
    if (item.id == ItemID.Gem) {
        ...
        if (b.getBlock(coords.x, coords.y, coords.z).id == BlockID.block1) {
            ...
            setTimeout(function () {
                Entity.setPosition(player, coords.x, coords.y, coords.z);
            }, 200);
        }
    }
});
