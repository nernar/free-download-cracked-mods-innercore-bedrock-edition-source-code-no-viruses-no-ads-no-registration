...
(function (Agriculture) {
    var CropCardManager = /** @class */ (function () {
        ...
        CropCardManager.getIndexByCropCardID = function (id) {
            for (var i_2 in this.cropCards) {
                if (this.cropCards[i_2].getID() == id) {
                    return +i_2;
                }
            }
            return null;
    ...
    var CropEatingplant = /** @class */ (function (_super) {
        ...
        CropEatingplant.prototype.hasMetalArmor = function (player) {
            for (var i_3 = 0; i_3 < 4; i_3++) {
                var armorSlot = new PlayerEntity(player).getArmor(i_3);
...
var IC2Config;
(function (IC2Config) {
    IC2Config.debugMode = getBool("debug_mode");
    ...
    IC2Config.wireDamageEnabled = getBool("wire_damage_enabled");
...
(function (CableRegistry) {
    ...
    function createBlock(stringID, properties, blockType) {
        ...
        for (var i_5 = 0; i_5 < 16; i_5++) {
            variations.push({ name: properties.name, texture: [[properties.texture, i_5]] });
        }
    ...
    function registerCable(stringID, maxVoltage, maxInsulationLevel) {
        if (maxInsulationLevel) {
            var _loop_1 = function (index) {
                var blockID = BlockID[stringID + index];
                ...
                var itemID = ItemID[stringID + index];
...
(function (EntityHelper) {
    ...
    function dealDamageFromCables(entity, x, y, z) {
            ...
            for (var xx = x - 1; xx <= x + 1; xx++)
                for (var zz = z - 1; zz <= z + 1; zz++) {
                    ...
                    if (cableData && cableData.insulation < cableData.maxInsulation) {
                        var node = EnergyNet.getNodeOnCoords(region, xx, yy, zz);
                        if (node && node.baseEnergy == "Eu" && node.energyPower > insulationMaxVolt[cableData.insulation]) {
                            var damage = Math.ceil(node.energyPower / 32);
    ...
    EntityHelper.dealDamageFromCables = dealDamageFromCables;
    Callback.addCallback("tick", function () {
        if (World.getThreadTime() % 20 == 0) {
            var entities = Entity.getAll();
            for (var i_7 in entities) {
                var ent = entities[i_7];
...
(function (LaserShotProvider) {
    ...
    function updateAll() {
        for (var i_9 = 0; i_9 < laserShots.length; i_9++) {
            var laser = laserShots[i_9];
...
(function (ItemName) {
    ...
    function showBlockStorage(item, name, tier, capacity) {
        var tierText = "§7" + Translation.translate("tooltip.power_tier").replace("%s", tier.toString());
        var energy = 0;
        if (item.extra) {
    ...
    function displayEnergy(energy, debug) {
        if (debug === void 0) { debug = IC2Config.debugMode; }
        if (!debug) {
            if (energy >= 1e6) {
...
var Machine;
(function (Machine) {
    Machine.ClientSide = BlockEngine.Decorators.ClientSide;
    Machine.NetworkEvent = BlockEngine.Decorators.NetworkEvent;
    Machine.ContainerEvent = BlockEngine.Decorators.ContainerEvent;
    var MachineBase = /** @class */ (function (_super) {
        ...
        MachineBase.prototype.isWrenchable = function () {
            return false;
        };
...
(function (RadiationAPI) {
    ...
    function resetRadiation(playerUid) {
        RadiationAPI.effectDuration[playerUid] = 0;
        Entity.clearEffect(playerUid, PotionEffect.fatal_poison);
    }
    RadiationAPI.resetRadiation = resetRadiation;
    ...
    function hasHazmatSuit(playerUid) {
        for (var i_10 = 0; i_10 < 4; i_10++) {
            var itemID = Entity.getArmorSlot(playerUid, i_10).id;
    ...
    Callback.addCallback("tick", function () {
        if (World.getThreadTime() % 20 == 0) {
            for (var i_11 = 0; i_11 < RadiationAPI.sources.length; i_11++) {
                var source = RadiationAPI.sources[i_11];
                ...
                source.timer--;
                if (source.timer <= 0) {
                    RadiationAPI.sources.splice(i_11--, 1);
                }
            }
        }
    });
    Callback.addCallback("ServerPlayerTick", function (playerUid) {
        if (World.getThreadTime() % 20 == 0) {
            if (!hasHazmatSuit(playerUid)) {
                ...
                for (var i_12 = 9; i_12 < 45; i_12++) {
                    var itemID = player.getInventorySlot(i_12).id;
...
(function (ICTool) {
    ...
    function useWrench(item, damage, player) {
        var wrench = getWrenchData(item.id);
        return wrench === null || wrench === void 0 ? void 0 : wrench.useItem(item, damage, player);
    }
    ...
    function addRecipe(result, data, tool) {
        ...
        Recipes.addShapeless(result, data, function (api, field, result) {
            for (var i_13 = 0; i_13 < field.length; i_13++) {
                if (field[i_13].id == tool) {
                    field[i_13].data++;
                    if (field[i_13].data >= Item.getMaxDamage(tool)) {
                        field[i_13].id = field[i_13].count = field[i_13].data = 0;
                    }
                }
                else {
                    api.decreaseFieldSlot(i_13);
                }
            }
        });
    }
    ...
    function dischargeItem(item, consume, player) {
        ...
        var itemChargeLevel = ChargeItemRegistry.getItemData(item.id).tier;
    ...
    Network.addServerPacket("icpe.demontageMachine", function (client, data) {
        ...
        if (MachineRegistry.isMachine(blockID)) {
            ...
            if (ICTool.isUseableWrench(item, 10)) {
                ...
                if (tileEntity) {
                    var chance_1 = ICTool.getWrenchData(item.id).dropChance;
                    var dropID = Math.random() < chance_1 ? tileEntity.blockID : tileEntity.getDefaultDrop();
                    var drop = tileEntity.adjustDrop(new ItemStack(dropID, 1, 0));
...
function InventoryWindow(header, uiDescriptor) {
...
(function (WindSim) {
    ...
    Saver.addSavesScope("windSim", function read(scope) {
        WindSim.windStrength = scope ? scope.strength : randomInt(5, 25);
    }, function save() {
...
World.addGenerationCallback(BlockEngine.getMainGameVersion() == 11 ? "GenerateChunk" : "PreProcessChunk", function (chunkX, chunkZ, random) {
    ...
    if (random.nextDouble() < RubberTreeGenerator.getBiomeChance(biome)) {
        ...
        for (var i_14 = 0; i_14 < treeCount; i_14++) {
...
(function (OreGenerator) {
    ...
    function generateOre(chunkX, chunkZ, blockID, properties, random) {
        for (var i_17 = 0; i_17 < properties.count; i_17++) {
...
(function (Agriculture) {
    var CropTile = /** @class */ (function (_super) {
        ...
        CropTile.prototype.tick = function () {
            this.checkGround();
        ...
        CropTile.prototype.updateTerrainNutrients = function () {
            ...
            for (var i_18 = 2; i_18 < 5; ++i_18) {
                if (this.region.getBlockId(this.x, this.y - i_18, this.z) == 3)
                    nutrients++;
            }
        ...
        CropTile.prototype.performWeedWork = function () {
            ...
            var coords = relativeCropCoords[randomInt(0, 3)];
        ...
        CropTile.prototype.attemptCrossing = function () {
            ...
            this.data.statResistance = 0;
            this.data.statGain = 0;
            for (var i_19 in cropCoords) {
                var te2 = this.region.getTileEntity(cropCoords[i_19].x, cropCoords[i_19].y, cropCoords[i_19].z);
        ...
        CropTile.prototype.calculateRatioFor = function (newCrop, oldCrop) {
            ...
            for (var i_20 in propOld) {
                var delta = Math.abs(propOld[i_20] - propNew[i_20]);
                value += 2 - delta;
            }
        ...
        CropTile.prototype.performHarvest = function () {
            ...
            var ret = [];
            for (var i_21 = 0; i_21 < dropCount2; i_21++) {
                ret[i_21] = this.crop.getGain(this);
                if (ret[i_21] && randomInt(0, 100) <= this.data.statGain) {
                    ret[i_21] = ret[i_21].count++;
                }
            }
        ...
        CropTile.prototype.pick = function () {
            ...
            if (bonus) {
                if (Math.random() <= (firstchance + 1) * .8)
                    dropCount++;
                var chance_2 = this.crop.getSeedDropChance(this) + this.data.statGrowth / 100;
                chance_2 *= Math.pow(.95, this.data.statGain - 23);
                if (Math.random() <= chance_2)
                    dropCount++;
            }
        ...
        CropTile.prototype.isBlockBelow = function (reqBlockID) {
            ...
            for (var i_22 = 1; i_22 < rootsLength; i_22++) {
                var blockID = this.region.getBlockId(this.x, this.y - i_22, this.z);
                if (!blockID)
                    return false;
...
(function (Machine) {
    var WaterGenerator = /** @class */ (function (_super) {
        ...
        WaterGenerator.prototype.biomeCheck = function (x, z) {
        ...
        WaterGenerator.prototype.energyTick = function (type, src) {
            if (World.getThreadTime() % 20 == 0) {
                ...
                if (biome && this.y >= 32 && this.y < 64) {
                    ...
                    var tile = this.region.getBlockId(this.x - randomInt(-radius, radius), this.y - randomInt(-radius, radius), this.z - randomInt(-radius, radius));
                    if (tile == 8 || tile == 9) {
...
(function (Machine) {
    var RTGenerator = /** @class */ (function (_super) {
        ...
        RTGenerator.prototype.onTick = function () {
            var output = 0.5;
            for (var i_23 = 0; i_23 < 6; i_23++) {
                var slot = this.container.getSlot("slot" + i_23);
...
(function (Machine) {
    var StirlingGenerator = /** @class */ (function (_super) {
        ...
        StirlingGenerator.prototype.isWrenchable = function () {
            return true;
        };
...
(function (Machine) {
    var ElectricHeatGenerator = /** @class */ (function (_super) {
        ...
        ElectricHeatGenerator.prototype.calcOutput = function () {
            var maxOutput = 0;
            for (var i_24 = 0; i_24 < 10; i_24++) {
                var slot = this.container.getSlot("slot" + i_24);
            ...
            if (this.data.energy >= 1) {
                ...
                if (tile && tile.canReceiveHeat && tile.canReceiveHeat(side ^ 1)) {
                    ...
                    if (output > 0) {
                        ...
                        for (var i_25 = outputText.length; i_25 < 6; i_25++) {
                            outputText += " ";
                        }
        ...
        ElectricHeatGenerator.prototype.isWrenchable = function () {
            return true;
        };
...
(function (Machine) {
    var FluidHeatGenerator = /** @class */ (function (_super) {
        ...
        FluidHeatGenerator.prototype.setupContainer = function () {
            var liquidFuel = MachineRecipeRegistry.requireRecipesFor("fluidFuel");
            this.liquidTank = this.addLiquidTank("fluid", 10000, Object.keys(liquidFuel));
            StorageInterface.setSlotValidatePolicy(this.container, "slot1", function (name, id, count, data) {
                var empty = LiquidItemRegistry.getEmptyItem(id, data);
                if (!empty)
        ...
        FluidHeatGenerator.prototype.isWrenchable = function () {
            return true;
        };
...
(function (Machine) {
    var RTHeatGenerator = /** @class */ (function (_super) {
        ...
        RTHeatGenerator.prototype.calculateOutput = function () {
            var output = 1;
            for (var i_26 = 0; i_26 < 6; i_26++) {
                var slot = this.container.getSlot("slot" + i_26);
                if (slot.id == ItemID.rtgPellet) {
                    output *= 2;
                }
            }
        ...
        RTHeatGenerator.prototype.getOutputText = function (output) {
            var outputText = output.toString();
            for (var i_27 = outputText.length; i_27 < 6; i_27++) {
                outputText += " ";
            }
            return outputText;
        };
        ...
        RTHeatGenerator.prototype.isWrenchable = function () {
            return true;
        };
...
(function (Machine) {
    var SolidHeatGenerator = /** @class */ (function (_super) {
        ...
        SolidHeatGenerator.prototype.onTick = function () {
            ...
            for (var i_28 = outputText.length; i_28 < 6; i_28++) {
                outputText += " ";
            }
            this.container.setText("textInfo1", outputText + "/");
            this.container.setScale("burningScale", this.data.burn / this.data.burnMax || 0);
        ...
        SolidHeatGenerator.prototype.isWrenchable = function () {
            return true;
        };
...
Block.registerPlaceFunction(BlockID.nuclearReactor, function (coords, item, block, player, region) {
    ...
    for (var i_29 = 0; i_29 < 6; i_29++) {
        var c = World.getRelativeCoords(x, y, z, i_29);
...
for (var y = 0; y < 6; y++) {
    for (var x = 0; x < 9; x++) {
        var i_30 = x * 6 + y;
        reactorElements["slot" + i_30] = { type: "slot", x: 400 + 54 * x, y: 40 + 54 * y, size: 54 };
    }
}
var guiNuclearReactor = InventoryWindow("Nuclear Reactor", {
    drawing: [
...
(function (Machine) {
    var NuclearReactor = /** @class */ (function (_super) {
        ...
        NuclearReactor.prototype.onInit = function () {
            ...
            for (var i_31 = 0; i_31 < 6; i_31++) {
                var coords = StorageInterface.getRelativeCoords(this, i_31);
        ...
        NuclearReactor.prototype.explode = function () {
            ...
            var boomPower = 10;
            var boomMod = 1;
            for (var i_32 = 0; i_32 < this.getReactorSize() * 6; i_32++) {
                var slot = this.container.getSlot("slot" + i_32);
                var component = ReactorItem.getComponent(slot.id);
                if (component) {
                ...
                this.container.setSlot("slot" + i_32, 0, 0, 0);
            }
            if (explode) {
        ...
        NuclearReactor.prototype.calculateHeatEffects = function () {
            ...
            if (power >= 0.85 && Math.random() <= 0.2 * this.data.hem) {
                ...
                var block = this.region.getBlockId(coord);
                var material = ToolAPI.getBlockMaterialName(block);
                if (block == BlockID.nuclearReactor) {
                    var tileEntity = this.region.getTileEntity(coord);
                    if (tileEntity) {
                        tileEntity.explode();
                    }
                }
            ...
            if (power >= 0.4 && Math.random() <= this.data.hem) {
                ...
                if (block != 49 && (material == "wood" || material == "wool" || material == "fibre" || material == "plant")) {
                    for (var i_33 = 0; i_33 < 6; i_33++) {
                        var coord2 = World.getRelativeCoords(coord.x, coord.y, coord.z, i_33);
...
Block.registerPlaceFunction(BlockID.reactorChamber, function (coords, item, block, player, region) {
    ...
    for (var i_34 = 0; i_34 < 6; i_34++) {
        var c = World.getRelativeCoords(x, y, z, i_34);
    ...
    var ReactorChamber = /** @class */ (function (_super) {
        ...
        ReactorChamber.prototype.onInit = function () {
            if (this.data.corePos && this.region.getBlockId(this.data.corePos) == BlockID.nuclearReactor) {
                ...
                for (var i_35 = 0; i_35 < 6; i_35++) {
                    var coords = StorageInterface.getRelativeCoords(this, i_35);
...
(function (Machine) {
    var BatteryBlock = /** @class */ (function (_super) {
        ...
        BatteryBlock.prototype.isWrenchable = function () {
            return true;
        };
...
(function (Machine) {
    var Transformer = /** @class */ (function (_super) {
        ...
        Transformer.prototype.isWrenchable = function () {
            return true;
        };
        ...
        SolidCanner.prototype.setupContainer = function () {
            ...
            StorageInterface.setGlobalValidatePolicy(this.container, function (name, id, amount, data) {
                ...
                if (name == "slotCan") {
                    ...
                    for (var i_36 in recipes) {
                        if (recipes[i_36].can == id)
                            return true;
                    }
                    return false;
                }
    ...
    StorageInterface.createInterface(BlockID.solidCanner, {
        slots: {
            ...
            "slotCan": { input: true, isValid: function (item) {
                    var recipes = MachineRecipeRegistry.requireRecipesFor("solidCanner");
                    for (var i_37 in recipes) {
                        if (recipes[i_37].can == item.id)
                            return true;
                    }
                    return false;
                } },
...
var guiCanner = InventoryWindow("Fluid/Solid Canning Machine", {
    drawing: [
    ...
    var Canner = /** @class */ (function (_super) {
        ...
        Canner.prototype.isValidSourceItem = function (id, data) {
            ...
            if (this.data.mode == 3) {
                ...
                for (var i_38 in recipes) {
                    if (recipes[i_38].input[1].id == id)
                        return true;
                }
            }
            return false;
        ...
        Canner.prototype.isValidCan = function (id, data) {
            switch (this.data.mode) {
                case 0: {
                    var recipes = MachineRecipeRegistry.requireRecipesFor("solidCanner");
                    for (var i_39 in recipes) {
                        if (recipes[i_39].can == id)
                            return true;
                    }
                    return false;
                }
        ...
        Canner.prototype.onTick = function () {
            ...
            switch (this.data.mode) {
                ...
                case 3:
                    var recipes = MachineRecipeRegistry.requireRecipesFor("fluidCanner");
                    resetProgress = true;
                    for (var i_40 in recipes) {
                        var recipe_1 = recipes[i_40];
...
Callback.addCallback("PreLoaded", function () {
    function isToolboxEmpty(slot) {
        ...
        if (container) {
            for (var i_41 = 1; i_41 <= 10; i_41++) {
                if (container.getSlot("slot" + i_41).id != 0) {
                    return false;
                }
            }
        }
    ...
    Recipes.addShaped({ id: BlockID.metalFormer, count: 1, data: 0 }, [
        ...
        if (isToolboxEmpty(field[3]) && isToolboxEmpty(field[5])) {
            for (var i_42 = 0; i_42 < field.length; i_42++) {
                api.decreaseFieldSlot(i_42);
            }
        }
        else {
...
(function (Machine) {
    var OreWasher = /** @class */ (function (_super) {
        ...
        OreWasher.prototype.checkResult = function (result) {
            for (var i_43 = 1; i_43 < 4; i_43++) {
                var id = result[(i_43 - 1) * 2];
                if (!id)
                    return true;
                var count = result[(i_43 - 1) * 2 + 1];
                var resultSlot = this.container.getSlot("slotResult" + i_43);
        ...
        OreWasher.prototype.putResult = function (result) {
            ...
            for (var i_44 = 1; i_44 < 4; i_44++) {
                var id = result[(i_44 - 1) * 2];
                if (!id)
                    break;
                var count = result[(i_44 - 1) * 2 + 1];
                var resultSlot = this.container.getSlot("slotResult" + i_44);
...
(function (Machine) {
    var ThermalCentrifuge = /** @class */ (function (_super) {
        ...
        ThermalCentrifuge.prototype.checkResult = function (result) {
            for (var i_45 = 1; i_45 < 4; i_45++) {
                var id = result[(i_45 - 1) * 2];
                var count = result[(i_45 - 1) * 2 + 1];
                var resultSlot = this.container.getSlot("slotResult" + i_45);
        ...
        ThermalCentrifuge.prototype.putResult = function (result) {
            for (var i_46 = 1; i_46 < 4; i_46++) {
                var id = result[(i_46 - 1) * 2];
                var count = result[(i_46 - 1) * 2 + 1];
                var resultSlot = this.container.getSlot("slotResult" + i_46);
                if (id) {
                    resultSlot.setSlot(id, resultSlot.count + count, 0);
                }
            }
...
(function (Machine) {
    var BlastFurnace = /** @class */ (function (_super) {
        ...
        BlastFurnace.prototype.isWrenchable = function () {
            return true;
        };
        ...
        BlastFurnace.prototype.checkResult = function (result) {
            for (var i_47 = 1; i_47 < 3; i_47++) {
                var id = result[(i_47 - 1) * 2];
                if (!id)
                    break;
                var count = result[(i_47 - 1) * 2 + 1];
                var resultSlot = this.container.getSlot("slotResult" + i_47);
                if (resultSlot.id != 0 && (resultSlot.id != id || resultSlot.count + count > 64)) {
                    return false;
                }
            }
        ...
        BlastFurnace.prototype.putResult = function (result) {
            for (var i_48 = 1; i_48 < 3; i_48++) {
                var id = result[(i_48 - 1) * 2];
                if (!id)
                    break;
                var count = result[(i_48 - 1) * 2 + 1];
                var resultSlot = this.container.getSlot("slotResult" + i_48);
                resultSlot.setSlot(id, resultSlot.count + count, 0);
            }
        };
...
(function (Machine) {
    var Fermenter = /** @class */ (function (_super) {
        ...
        Fermenter.prototype.isWrenchable = function () {
            return true;
        };
...
(function (Machine) {
    var FluidDistributor = /** @class */ (function (_super) {
        ...
        FluidDistributor.prototype.isWrenchable = function () {
            return true;
        };
...
var guiMiner = InventoryWindow("Miner", {
    drawing: [
        { type: "bitmap", x: 550, y: 150, bitmap: "energy_small_background", scale: GUI_SCALE }
    ],
    elements: {
...
(function (Machine) {
    var Miner = /** @class */ (function (_super) {
        ...
        Miner.prototype.mineBlock = function (x, y, z, block, level) {
            var drop = ToolLib.getBlockDrop(new Vector3(x, y, z), block.id, block.data, level);
            var items = [];
            for (var _i = 0, drop_1 = drop; _i < drop_1.length; _i++) {
                var item = drop_1[_i];
                items.push(new ItemStack(item[0], item[1], item[2], item[3]));
            }
            var container = this.region.getContainer(x, y, z);
            if (container) {
                var slots = StorageInterface.getContainerSlots(container);
                for (var i_49 in slots) {
                    var slot = container.getSlot(slots[i_49]);
                    if (slot.id > 0) {
                        items.push(new ItemStack(slot));
                        container.setSlot(slots[i_49], 0, 0, 0);
                    }
                }
            }
                ...
                if (this.data.y < this.y && this.findOre(level)) {
                    ...
                    if (this.isEmptyBlock(block)) {
                        ...
                        if (this.data.progress >= params.time) {
                            ...
                            this.mineBlock(this.x, this.data.y - 1, this.z, block_1, level);
                            this.setPipe(this.data.y);
                        }
                    }
                }
...
var guiAdvancedMiner = InventoryWindow("Advanced Miner", {
    drawing: [
...
(function (Machine) {
    var AdvancedMiner = /** @class */ (function (_super) {
            ...
            if (data.isEnabled && this.y + data.y >= 0 && data.energy >= 512) {
                ...
                if (scanR > 0 && energyStored >= 64) {
                    ...
                    if (World.getThreadTime() % 20 == 0) {
                        ...
                        for (var i_50 = 0; i_50 < this.maxScanCount; i_50++) {
                            if (data.x > scanR) {
                                data.x = -scanR;
                                data.z++;
                            }
        ...
        AdvancedMiner.prototype.harvestBlock = function (x, y, z, block) {
            ...
            var drop = ToolLib.getBlockDrop(new Vector3(x, y, z), block.id, block.data, 100, enchant);
        ...
        AdvancedMiner.prototype.checkDrop = function (drop) {
            ...
            for (var _i = 0, drop_3 = drop; _i < drop_3.length; _i++) {
                var item = drop_3[_i];
                for (var i_51 = 0; i_51 < 16; i_51++) {
                    var slot = this.container.getSlot("slot" + i_51);
...
for (var i_52 = 0; i_52 < 15; i_52++) {
    var x = i_52 % 5;
    var y = Math.floor(i_52 / 5) + 1;
    cropHarvesterGuiElements["outSlot" + i_52] = { type: "slot", x: 520 + x * 60, y: 50 + y * 60 };
}
;
...
(function (Machine) {
    var CropHarvester = /** @class */ (function (_super) {
        ...
        CropHarvester.prototype.putItem = function (item) {
            for (var i_53 = 0; i_53 < 15; i_53++) {
                var slot = this.container.getSlot("outSlot" + i_53);
        ...
        CropHarvester.prototype.isInventoryFull = function () {
            for (var i_54 = 0; i_54 < 15; i_54++) {
                var slot = this.container.getSlot("outSlot" + i_54);
...
for (var i_55 = 1; i_55 < 7; i_55++) {
    сropMatronGuiElements["slotWeedEx" + i_55] = { type: "slot", x: 441 + 60 * i_55, y: 155 };
}
for (var i_56 = 1; i_56 < 7; i_56++) {
    сropMatronGuiElements["slotFertilizer" + i_56] = { type: "slot", x: 441 + 60 * i_56, y: 75 };
}
...
(function (Machine) {
    var CropMatron = /** @class */ (function (_super) {
        ...
        CropMatron.prototype.getSlot = function (type) {
            for (var i_57 = 0; i_57 < 7; i_57++) {
                var slot = this.container.getSlot(type + i_57);
                if (slot.id)
                    return slot;
            }
            return null;
...
(function (Machine) {
    var Nuke = /** @class */ (function (_super) {
                    ...
                    for (var dz = -radius; dz <= radius; dz++) {
                        if (Math.sqrt(dx * dx + dy * dy * 4 + dz * dz) <= radius) {
                            ...
                            if (block.id > 0 && Block.getExplosionResistance(block.id) < 10000) {
                                ...
                                if (Math.random() < 0.01) {
                                    var drop = ToolLib.getBlockDrop(new Vector3(xx, yy, zz), block.id, block.data, 100);
                                    for (var _a = 0, drop_4 = drop; _a < drop_4.length; _a++) {
                                        var item = drop_4[_a];
        ...
        Nuke.prototype.explodeAnimation = function (data) {
            ...
            for (var i_58 = 0; i_58 < count; i_58++) {
                var dx = randomInt(-radius, radius);
                var dy = randomInt(-radius / 2, radius / 2);
                var dz = randomInt(-radius, radius);
                if (Math.sqrt(dx * dx + dy * dy * 4 + dz * dz) <= radius) {
    ...
    var Teleporter = /** @class */ (function (_super) {
        ...
        Teleporter.prototype.onTick = function () {
            if (World.getThreadTime() % 11 == 0 && this.data.isActive && this.data.frequency) {
                ...
                for (var i_59 in storages) {
                    energyAvailable += storages[i_59].data.energy;
                }
                var receive = this.data.frequency;
                if (energyAvailable > receive.energy * 100) {
                    var entities = this.region.listEntitiesInAABB(this.x - 1, this.y, this.z - 1, this.x + 2, this.y + 3, this.z + 2);
                    for (var _i = 0, entities_5 = entities; _i < entities_5.length; _i++) {
                        ...
                        if (energyNeed <= energyAvailable) {
                            for (var i_60 in storages) {
                                var data = storages[i_60].data;
...
var ItemScrapBox = /** @class */ (function (_super) {
    ...
    ItemScrapBox.prototype.getDropItem = function () {
        var total = 0;
        for (var i_61 in SCRAP_BOX_RANDOM_DROP) {
            total += SCRAP_BOX_RANDOM_DROP[i_61].chance;
        }
        var random = Math.random() * total * 1.35;
        var current = 0;
        for (var i_62 in SCRAP_BOX_RANDOM_DROP) {
            var drop = SCRAP_BOX_RANDOM_DROP[i_62];
...
var ItemTerraWart = /** @class */ (function (_super) {
    ...
    function ItemTerraWart() {
        var _this = _super.call(this, "terraWart", "terra_wart", "terra_wart", 1) || this;
    ...
    ItemTerraWart.prototype.onFoodEaten = function (item, food, saturation, player) {
        ...
        for (var i_63 in negativePotions) {
            var potionID = negativePotions[i_63];
...
var CoffeeMug = {
    ...
    craftFunction: function (api, field, result) {
        for (var i_64 in field) {
            if (field[i_64].id == VanillaItemID.bucket) {
                if (field[i_64].count == 1) {
                    field[i_64].data = 0;
                }
                else {
                    api.decreaseFieldSlot(i_64);
...
var ItemBatteryCharging = /** @class */ (function (_super) {
    ...
    ItemBatteryCharging.checkCharging = function (playerUid) {
        var player = new PlayerEntity(playerUid);
        for (var i_65 = 0; i_65 < 36; i_65++) {
            var slot = player.getInventorySlot(i_65);
            ...
            if (itemInstance instanceof ItemBatteryCharging) {
                ...
                for (var index = 0; index < 9; index++) {
                    if (mode == 1 && player.getSelectedSlot() == index)
                        continue;
                    var stack = player.getInventorySlot(index);
                    if (!ChargeItemRegistry.isValidStorage(stack.id, "Eu", 5)) {
                        var energyAdd = ChargeItemRegistry.addEnergyTo(stack, "Eu", Math.min(energyStored, itemInstance.transferLimit * 20), itemInstance.tier, true);
                        if (energyAdd > 0) {
                ...
                player.setInventorySlot(i_65, slot);
            }
        }
    };
...
(function (ReactorItem) {
    var FuelRod = /** @class */ (function (_super) {
        ...
        FuelRod.prototype.processChamber = function (item, reactor, x, y, heatrun) {
            ...
            for (var i_66 = 0; i_66 < this.numberOfCells; i_66++) {
                var dheat = 0;
                var pulses = basePulses;
                if (!heatrun) {
                    for (var i_67 = 0; i_67 < pulses; i_67++) {
    ...
    var HeatExchanger = /** @class */ (function (_super) {
        ...
        HeatExchanger.prototype.processChamber = function (item, reactor, x, y, heatrun) {
            ...
            if (this.switchSide > 0) {
                ...
                this.checkHeatAcceptor(reactor, x, y - 1, heatAcceptors);
                this.checkHeatAcceptor(reactor, x, y + 1, heatAcceptors);
                for (var i_68 in heatAcceptors) {
                    var acceptor = heatAcceptors[i_68];
...
Recipes.addShapeless({ id: ItemID.rshCondensator, count: 1, data: 1 }, [{ id: ItemID.rshCondensator, data: -1 }, { id: 331, data: 0 }], function (api, field, result) {
    ...
    for (var i_69 = 0; i_69 < field.length; i_69++) {
        var slot = field[i_69];
...
Recipes.addShapeless({ id: ItemID.lzhCondensator, count: 1, data: 1 }, [{ id: ItemID.lzhCondensator, data: -1 }, { id: 331, data: 0 }], function (api, field, result) {
    var index = 0;
    var canBeRepaired = false;
    for (var i_70 = 0; i_70 < field.length; i_70++) {
        var slot = field[i_70];
...
Recipes.addShapeless({ id: ItemID.lzhCondensator, count: 1, data: 1 }, [{ id: ItemID.lzhCondensator, data: -1 }, IDConverter.getIDData("lapis_lazuli")], function (api, field, result) {
    var index = 0;
    var canBeRepaired = false;
    for (var i_71 = 0; i_71 < field.length; i_71++) {
        var slot = field[i_71];
        if (slot.id == ItemID.lzhCondensator) {
            if (slot.data <= 1)
                break;
...
var ArmorHazmat = /** @class */ (function (_super) {
    ...
    ArmorHazmat.prototype.onHurt = function (params, item, index, playerUid) {
        if (params.type == 9 && index == 0) {
            var player = new PlayerEntity(playerUid);
            for (var i_72 = 0; i_72 < 36; i_72++) {
                var stack = player.getInventorySlot(i_72);
                if (stack.id == ItemID.cellAir) {
                    Game.prevent();
                    Entity.addEffect(playerUid, PotionEffect.waterBreathing, 1, 60);
                    stack.decrease(1);
                    player.setInventorySlot(i_72, stack);
                    player.addItemToInventory(ItemID.cellEmpty, 1, 0);
                    break;
                }
            }
    ...
    function onTick(item, playerUid) {
        ...
        if (item.extra && item.extra.getBoolean("hover")) {
            ...
            if (energyStored < 8 || EntityHelper.isOnGround(playerUid)) {
                ...
                if (client)
                    client.sendMessage("§4" + Translation.translate("Hover mode disabled"));
                return item;
            }
            else if (vel.y < 0) {
...
var ArmorJetpackElectric = /** @class */ (function (_super) {
    ...
    ArmorJetpackElectric.prototype.getIcon = function (armorName) {
        return armorName;
    };
...
var ArmorBatpack = /** @class */ (function (_super) {
    ...
    ArmorBatpack.prototype.getIcon = function (armorName) {
        return armorName;
    };
...
var ArmorNanoSuit = /** @class */ (function (_super) {
    ...
    function ArmorNanoSuit(stringID, name, params) {
        var _this = _super.call(this, stringID, name, params, 1000000, 2048, 3) || this;
...
var ArmorQuantumSuit = /** @class */ (function (_super) {
    ...
    function ArmorQuantumSuit(stringID, name, params, isDischarged) {
        if (isDischarged === void 0) { isDischarged = false; }
        var _this = _super.call(this, stringID, name, params, 1e7, 12000, 4) || this;
        _this.setRarity(EnumRarity.RARE);
        RadiationAPI.registerHazmatArmor(_this.id);
        return _this;
    }
...
var ArmorQuantumHelmet = /** @class */ (function (_super) {
    ...
    ArmorQuantumHelmet.prototype.onTick = function (item, index, playerUid) {
        ...
        if (hunger < 20 && newEnergyStored >= 500) {
            var i_73 = World.getThreadTime() % 36;
            var stack = player.getInventorySlot(i_73);
            if (stack.id == ItemID.tinCanFull) {
                var count = Math.min(20 - hunger, stack.count);
                player.setHunger(hunger + count);
                stack.decrease(count);
                player.setInventorySlot(i_73, stack);
                player.addItemToInventory(ItemID.tinCanEmpty, count, 0);
                newEnergyStored -= 500;
            }
        }
...
var ArmorQuantumBoots = /** @class */ (function (_super) {
    ...
    function ArmorQuantumBoots(stringID, name, texture) {
        ...
        UIbuttons.setArmorButton(_this.id, "button_jump");
        return _this;
    }
...
Callback.addCallback("EntityHurt", function (attacker, victim, damage, type) {
    if (damage > 0 && Entity.getType(victim) == 1 && (type == 2 || type == 3 || type == 11)) {
        var defencePoints = 0;
        for (var i_74 = 0; i_74 < 4; i_74++) {
            var item = Entity.getArmorSlot(victim, i_74);
    ...
    ArmorSolarHelmet.prototype.onTick = function (item, index, playerUid) {
        ...
        if (World.getThreadTime() % 20 == 0 && time >= 23500 || time < 12550) {
            ...
            if (region.canSeeSky(pos) && (!World.getWeather().rain || region.getLightLevel(pos) > 14)) {
                for (var i_75 = 1; i_75 < 4; i_75++) {
                    var energy = 20;
                    var armor = Entity.getArmorSlot(playerUid, i_75);
                    var energyAdd = ChargeItemRegistry.addEnergyTo(armor, "Eu", energy, 4);
                    if (energyAdd > 0) {
                        energy -= energyAdd;
                        Entity.setArmorSlot(playerUid, i_75, armor.id, 1, armor.data, armor.extra);
                        if (energy <= 0)
                            break;
                    }
                }
...
var CropAnalyser = /** @class */ (function (_super) {
    ...
    CropAnalyser.prototype.showCropValues = function (tileEntity) {
    ...
    CropAnalyser.showAllValues = function (container, seedBagSlot) {
        ...
        switch (level) {
            ...
            case 3:
                ...
                for (var i_76 in attributes) {
                    container.setText("attributes" + (+i_76 % 3), attributes[i_76]);
                }
            case 2:
...
var DebugItem = /** @class */ (function (_super) {
    ...
    DebugItem.prototype.onCharge = function (item, amount, tier) {
        return amount;
    };
    DebugItem.prototype.onDischarge = function (item, amount, tier) {
        return amount;
    };
...
var EUMeterUpdatable = /** @class */ (function () {
    ...
    EUMeterUpdatable.prototype.displayValue = function (value, unit) {
...
Item.registerUseFunction("cutter", function (coords, item, block, playerUid) {
    ...
    if (cableData && cableData.insulation < cableData.maxInsulation) {
        var player = new PlayerEntity(playerUid);
        for (var i_77 = 9; i_77 < 45; i_77++) {
            var stack = player.getInventorySlot(i_77);
            if (stack.id == ItemID.rubber) {
                var blockID = Block.getNumericId(cableData.name + (cableData.insulation + 1));
                var region = BlockSource.getDefaultForActor(playerUid);
                region.setBlock(coords.x, coords.y, coords.z, blockID, 0);
                stack.decrease(1);
                player.setInventorySlot(i_77, stack);
                if (block.data > 0) {
...
var ToolWrench = /** @class */ (function (_super) {
    ...
    function ToolWrench(stringID, name, icon, dropChance) {
        var _this = _super.call(this, stringID, name, icon) || this;
        _this.setMaxStack(1);
        _this.setMaxDamage(161);
        _this.setCategory(ItemCategory.EQUIPMENT);
        _this.dropChance = dropChance;
...
var ElectricTool = /** @class */ (function (_super) {
    ...
    function ElectricTool(stringID, name, toolData, blockMaterials, maxCharge, transferLimit, tier) {
        var _this = _super.call(this, stringID, name, maxCharge, transferLimit, tier) || this;
        _this.damage = 0;
        _this.setHandEquipped(true);
        _this.energyPerUse = toolData.energyPerUse;
        var toolMaterial = {
            level: toolData.level,
            efficiency: toolData.efficiency,
            damage: toolData.damage,
            durability: Item.getMaxDamage(_this.id)
        };
        ToolAPI.registerTool(_this.id, toolMaterial, blockMaterials, _this);
        return _this;
    }
    ElectricTool.prototype.onBroke = function () { return true; };
...
var ElectricChainsaw = /** @class */ (function (_super) {
    ...
    function ElectricChainsaw(stringID, name, toolData, maxCharge, transferLimit, tier) {
        var _this = _super.call(this, stringID, name, toolData, ["wood", "wool", "fibre", "plant"], maxCharge, transferLimit, tier) || this;
        _this.damage = 4;
...
var ToolDrill = /** @class */ (function (_super) {
    ...
    function ToolDrill(stringID, name, toolData, maxCharge, transferLimit, tier) {
        return _super.call(this, stringID, name, toolData, ["stone", "dirt"], maxCharge, transferLimit, tier) || this;
    }
    ...
    ToolDrill.prototype.onItemUse = function (coords, item, block, playerUid) {
        ...
        var player = new PlayerEntity(playerUid);
        for (var i_78 = 9; i_78 < 45; i_78++) {
            var stack = player.getInventorySlot(i_78);
            if (stack.id != 50)
                continue;
            ...
            stack.decrease(1);
            player.setInventorySlot(i_78, stack);
            break;
        }
    };
...
var ToolDrillIridium = /** @class */ (function (_super) {
    ...
    function ToolDrillIridium() {
        ...
        _this.setGlint(true);
        _this.setRarity(EnumRarity.RARE);
        UIbuttons.setToolButton(_this.id, "button_switch");
        UIbuttons.registerSwitchFunction(_this.id, function (item, player) {
            _this.onModeSwitch(item, player);
        });
        return _this;
    }
                    ...
                    for (var yy = coords.y - rad.y; yy <= coords.y + rad.y; yy++)
                        for (var zz = coords.z - rad.z; zz <= coords.z + rad.z; zz++) {
                            ...
                            if (material_1 && (material_1.name == "dirt" || material_1.name == "stone")) {
                                destroyTime = Math.max(destroyTime, Block.getDestroyTime(blockID) / material_1.multiplier / 24);
                            }
                        }
            }
            return destroyTime;
...
var ItemNanoSaber = /** @class */ (function (_super) {
    ...
    function ItemNanoSaber() {
        var _this = _super.call(this, "nanoSaber", "nano_saber", { energyPerUse: 64, level: 0, damage: 16, efficiency: 4 }, [], 1000000, 2048, 3) || this;
        _this.damage = 4;
        _this.setRarity(EnumRarity.UNCOMMON);
        return _this;
    ...
    ItemNanoSaber.onTick = function (playerUid) {
        if (World.getThreadTime() % 20 == 0) {
            var player = new PlayerActor(playerUid);
            for (var i_79 = 0; i_79 < 36; i_79++) {
                var item = player.getInventorySlot(i_79);
                if (item.id == ItemID.nanoSaber && item.extra && item.extra.getBoolean("active")) {
                    ...
                    ChargeItemRegistry.setEnergyStored(item, energyStored);
                    player.setInventorySlot(i_79, item.id, 1, item.data, item.extra);
                }
            }
        }
    };
...
var ItemMiningLaser = /** @class */ (function (_super) {
    ...
    function ItemMiningLaser() {
        ...
        _this.setHandEquipped(true);
        _this.setRarity(EnumRarity.UNCOMMON);
        UIbuttons.setToolButton(_this.id, "button_switch", true);
        UIbuttons.registerSwitchFunction(_this.id, function (item, player) {
            _this.onModeSwitch(item, player);
        });
        return _this;
    ...
    ItemMiningLaser.prototype.getModeData = function (mode) {
        return this.modes[mode];
    };
    ItemMiningLaser.prototype.getModeInfo = function (mode) {
...
var ItemPainter = /** @class */ (function (_super) {
    ...
    function ItemPainter(colorIndex) {
        ...
        _this = _super.call(this, "icPainter" + itemIndex, "painter." + colorIndex, { name: "ic_painter", meta: itemIndex }) || this;
        _this.setMaxStack(1);
...
for (var i_80 = 0; i_80 < 16; i_80++) {
    var item = ItemRegistry.registerItem(new ItemPainter(i_80));
...
Callback.addCallback("PreLoaded", function () {
    ...
    for (var i_81 = 6; i_81 <= 15; i_81++) {
        Recipes.addShapeless({ id: ItemID["icPainter" + i_81], count: 1, data: 0 }, [{ id: ItemID.icPainter, data: 0 }, { id: 351, data: i_81 - 1 }]);
    }
