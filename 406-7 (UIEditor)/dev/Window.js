var Windows = {windows: {}, dismiss: function (name) {
    ...
    Widgets.run(function () {
        if (layout) {
            ...
            if (widget) {
                ...
                widget.layout.postDelayed(new java.lang.Runnable({run: function () {
                    Widgets.run(function () {
                        widget.name = name;
                ...
                var window = Widgets.window(scroll, Widgets.gravity.top | Widgets.gravity["left"], null, null, null, Widgets.size.match);
                Windows.windows[type + "Menu"] = {window: window, parent: scroll, layout: layout, name: name};
            }
        }
    });
    ...
    Widgets.run(function () {
        if (layout) {
            ...
            if (widget && !focus) {
                    ...
                    if (Settings.config.interfaceAnimated) {
                        ...
                        widget.layout.postDelayed(new java.lang.Runnable({run: function () {
                            Widgets.run(function () {
                                ...
                                widget.parent.addView(layout);
                                widget.layout = layout;
                                widget.window.update(view ? (type == "menu") ? view.getWidth() : (type == "right") ? view.getWidth() + layout.getWidth() : 0 : 0, view ? view.getY() : 0, Widgets.size.wrap, Widgets.size.wrap);
                ...
                var scroll = Widgets.scroll(layout);
                var window = Widgets.window(scroll, view ? Widgets.gravity.top | Widgets.gravity[view.getParent().getTag()] : Widgets.gravity.center, view ? (type == "menu") ? view.getWidth() : (type == "right") ? view.getWidth() + layout.getWidth() : 0 : 0, view ? view.getY() : 0, null, null, focus);
                if (!focus) {
                    Windows.windows[type + "Popup"] = {window: window, parent: scroll, layout: layout, name: name};
                }
            }
        }
    });
