var Widgets = {ctx: UI.getContext(), theme: 16974120, dialogTheme: 16974126, size: {wrap: android.widget.RelativeLayout.LayoutParams.WRAP_CONTENT, match: android.widget.RelativeLayout.LayoutParams.MATCH_PARENT}, display: {width: UI.getContext().getWindowManager().getDefaultDisplay().getWidth(), height: UI.getContext().getWindowManager().getDefaultDisplay().getHeight()}, gravity: {top: android.view.Gravity.TOP, bottom: android.view.Gravity.BOTTOM, left: android.view.Gravity.LEFT, right: android.view.Gravity.RIGHT, center: android.view.Gravity.CENTER}, visibility: {visible: android.view.View.VISIBLE, invisible: android.view.View.INVISIBLE, gone: android.view.View.GONE}, color: android.graphics.Color, font: android.graphics.Typeface.createFromFile(new java.io.File(FileTools.root + "games/com.mojang/innercore/mc-typeface.ttf")), orientate: {vertical: android.widget.LinearLayout.VERTICAL, horizontal: android.widget.LinearLayout.HORIZONTAL}, run: function (code) {
    ...
    return isWidth ? (value * 1 / 800 * this.display.width) : (value * 1 / 480 * this.display.height);
}, check: function (image, isPath) {
    if (!isPath) {
        return image && new java.io.File(__dir__ + "gui/UI/" + image + ".png").exists();
    } else {
        return image && new java.io.File(image).exists();
    }
}, bitmap: function (file, scale, isPath) {
    if (!this.check(file, isPath)) {
        return null;
    }
    var bitmap = "";
    if (isPath) {
        bitmap = FileTools.ReadImage(file);
    } else {
        bitmap = FileTools.ReadImage(__dir__ + "gui/UI/" + file + ".png");
    }
    ...
    var window = new android.widget.PopupWindow(view, this.resize(width) || this.size.wrap, this.resize(height) || this.size.wrap, focus || false);
    if (gravity == null) {
        gravity = this.gravity.center;
    }
    window.setBackgroundDrawable(this.bitmap("background"));
    window.showAtLocation(this.ctx.getWindow().getDecorView(), gravity, x || 0, y || 0);
    return window;
    ...
    if (size != null) {
        button.setTextSize(this.resize(size));
    }
    ...
    if (file) {
        image.setImageDrawable(this.bitmap(file, scale, isPath));
    }
    if (params) {
    ...
    if (size != null) {
        text.setTextSize(this.resize(size));
    }
    ...
    if (size != null) {
        edit.setTextSize(this.resize(size));
    }
    if (single) {
        edit.setSingleLine(single);
    }
    if (type) {
    ...
    return {adapter: adapter, view: list};
}};
