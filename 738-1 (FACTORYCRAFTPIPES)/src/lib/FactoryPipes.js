LIBRARY({name: "Pipe", version: 2, shared: true, api: "CoreEngine", dependencies: []});
var Pipe = {item: new GameObject("fc_item", {init: function () {
    ...
    this.target = {x: Math.floor(this.position.x) - this.direction.x, y: Math.floor(this.position.y) - this.direction.y, z: Math.floor(this.position.z) - this.direction.z};
    ...
    this.animation.describeItem({id: this.item.id, count: 1, data: this.item.data, size: 0.25, rotation: "x"}, {x: -OFFSET, y: -OFFSET, z: -OFFSET});
    this.animation.load();
}, moveAnimation: function () {
    var OFFSET = 0.5;
    if (this.animation) {
        this.animation.setPos(this.position.x + OFFSET, this.position.y + OFFSET, this.position.z + OFFSET);
    }
}, move: function () {
    var dvelocity = Math.min(0.5, Math.max(0.02, this.velocity - this.friction || 0));
...
}}), pipes: {}, nativeContainer: {54: true, 61: true, 62: true, 154: true}, registerTile: function (id, proto) {
    !proto ? proto = {} : null;
    ...
    if (!s && this.nativeContainer[id]) {
        return true;
    }
    if (!s && container) {
        if (container.tileEntity) {
            if (this.canUseTile(container.tileEntity)) {
                return true;
            }
        }
    }
    return false;
}, directions: [{x: 0, y: 0, z: -1}, {x: 0, y: 0, z: 1}, {x: -1, y: 0, z: 0}, {x: 1, y: 0, z: 0}, {x: 0, y: -1, z: 0}, {x: 0, y: 1, z: 0}], findDirections: function (x, y, z, s) {
    var possible = [];
    for (var i in this.directions) {
        var dir = this.directions[i];
        if (this.canConnectTo(x + dir.x, y + dir.y, z + dir.z, s)) {
            possible.push(dir);
        }
    }
    return possible;
    ...
    for (var i in this.directions) {
        var dir = this.directions[i];
        if (!this.isPipe(World.getBlockID(x + dir.x, y + dir.y, z + dir.z)) && this.canConnectTo(x + dir.x, y + dir.y, z + dir.z, s)) {
            possible.push(dir);
        }
    }
    return possible;
