...
;
   ...
   if(World.getBlockId(place.x,place.y-1,place.z)==BlockID.vic_tantros_grass){ 
        World.setBlock(place.x, place.y, place.z, BlockID.tantros_short_grass);  
...
;
    var place = coords.relative;
   if(World.getBlockId(place.x,place.y-1,place.z)==BlockID.vic_tantros_grass){ 
        World.setBlock(place.x, place.y, place.z, BlockID.tantros_grass);  
         Entity.setCarriedItem(player, item.id, item.count - 1, item.data);
    } 
});
...
;
    var place = coords.relative;
   if(World.getBlockId(place.x,place.y-1,place.z)==BlockID.vic_tantros_sand){ 
        World.setBlock(place.x, place.y, place.z, BlockID.tantros_cane);  
         Entity.setCarriedItem(player, item.id, item.count - 1, item.data);
    } 
});
...
Block.setRandomTickCallback(BlockID.tantros_cane, function(x, y, z, id, data) {
        World.setBlock(x, y+1, z, BlockID.tantros_cane,0);
});
//рост самого тростника
/*
Block.registerNeighbourChangeFunction(BlockID.tantros_cane, function(coords, block, changedCoords, World) {
...
;
    var place = coords.relative;
   if(World.getBlockId(place.x,place.y+1,place.z)==BlockID.vic_a2_turao_leaves){ 
        World.setBlock(place.x, place.y, place.z, BlockID.tantros_berry);  
         Entity.setCarriedItem(player, item.id, item.count - 1, item.data);
    } 
});
