    ...
    grimoire: {
		useItem: function(coords, item, block, player) {
			if (block.id == BlockID.arid_box) {
				let region = BlockSource.getDefaultForActor(player);
				region.setBlock(coords.x, coords.y, coords.z, BlockID.arid_box_released, 0);
				World.playSound(coords.x + .5, coords.y, coords.z + .5, "note.bell", 1, 0.8);
				ToolLib.breakCarriedTool(1, player);
			}
			if (block.id == BlockID.deep_box) {
				let region = BlockSource.getDefaultForActor(player);
				region.setBlock(coords.x, coords.y, coords.z, BlockID.deep_box_released, 0);
				World.playSound(coords.x + .5, coords.y, coords.z + .5, "note.bell", 1, 0.8);
				ToolLib.breakCarriedTool(1, player);
			}
			if (block.id == BlockID.possessed_box) {
				let region = BlockSource.getDefaultForActor(player);
				region.setBlock(coords.x, coords.y, coords.z, BlockID.possessed_box_released, 0);
				World.playSound(coords.x + .5, coords.y, coords.z + .5, "note.bell", 1, 0.8);
				ToolLib.breakCarriedTool(1, player);
			}
			if (block.id == BlockID.steep_box) {
				let region = BlockSource.getDefaultForActor(player);
				region.setBlock(coords.x, coords.y, coords.z, BlockID.steep_box_released, 0);
				World.playSound(coords.x + .5, coords.y, coords.z + .5, "note.bell", 1, 0.8);
				ToolLib.breakCarriedTool(1, player);
			}
			if (block.id == BlockID.underground_box) {
				let region = BlockSource.getDefaultForActor(player);
				region.setBlock(coords.x, coords.y, coords.z, BlockID.underground_box_released, 0);
				World.playSound(coords.x + .5, coords.y, coords.z + .5, "note.bell", 1, 0.8);
				ToolLib.breakCarriedTool(1, player);
			}
			if (block.id == BlockID.wild_box) {
				let region = BlockSource.getDefaultForActor(player);
				region.setBlock(coords.x, coords.y, coords.z, BlockID.wild_box_released, 0);
				World.playSound(coords.x + .5, coords.y, coords.z + .5, "note.bell", 1, 0.8);
				ToolLib.breakCarriedTool(1, player);
			}
		}
