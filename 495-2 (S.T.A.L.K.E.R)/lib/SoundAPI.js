LIBRARY({name: "SoundAPI", version: 3, api: "CoreEngine"});
var _soundUtils = {maxPlayer: 30, sounds: [], target: [], source: {NULL: null, PLAYER: 1, BLOCK: 2, ENTITY: 3}, getFreePlayer: function () {
    ...
    for (var i in this.sounds) {
        if (this.target[i] == this.source.NULL) {
            continue;
        }
        var s = this.sounds[i];
        if (s.source == this.source.PLAYER) {
            continue;
        }
        ...
        if (s.source == this.source.BLOCK) {
            ...
            if (s.source == this.source.ENTITY) {
        ...
        if (distance <= 2) {
            left_volume = (0.1 + (radVol * angle)) * s.volume, right_volume = (1 - (radVol * angle)) * s.volume;
        } else {
            left_volume = (0.1 + (radVol * angle)) * s.volume / (s.radius - 2) * _distance, right_volume = (1 - (radVol * angle)) * s.volume / (s.radius - 2) * _distance;
        }
...
var Sound = function (src, vol) {
    ...
    this.setInBlock = function (vector, y, z, r) {
        ...
        if (typeof (vector) == "number") {
            this.position = {x: vector + 0.5, y: y + 0.5, z: z + 0.5};
            this.radius = r;
        } else {
            vector.x += 0.5;
            vector.y += 0.5;
    ...
    this.setOnCompletion = function (f, a) {
        this.media.setOnCompletionListener(new android.media.MediaPlayer.OnCompletionListener() {onCompletion: function (mp) {
            f(a, mp);
        }});
    };
...
var gameVolume = parseFloat(FileTools.ReadKeyValueFile(settings_path)["audio_sound"]);
...
Callback.addCallback("NativeGuiChanged", function (screen) {
    ...
    if (prevScreen && !currentScreen) {
        gameVolume = parseFloat(FileTools.ReadKeyValueFile(settings_path)["audio_sound"]);
    }
