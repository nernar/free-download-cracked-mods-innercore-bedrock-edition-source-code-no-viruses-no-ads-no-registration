...
var bloodNetwork={
    ...
    setMaxBloodAmount:function(count){
		playerBloodMaxAmount=count;
	}
};
...
function addBloodParticle(x,y,z, count){
	for( var i = 0; i<=count; i++){
		Particles.addParticle(Native.ParticleType.redstone,x+Math.random(), y+Math.random()/2, z+Math.random(),0,0,0,0);
	}
}
...
function pullEntity(entity, r, speed, X, Y, Z) {
    var x = Entity.getPosition(entity).x - X;
    var y = Entity.getPosition(entity).y - Y;
    var z = Entity.getPosition(entity).z - Z;
    if (x * x + y * y + z * z < r * r) {
        Entity.setVelocity(entity, x * speed, y * speed, z * speed);
        return true;
    }
}
...
Item.createItem("weakBloodOrb", "Weak Blood Orb", {
    name: "Weak",
    meta: 0
}, {
    stack: 1
...
Translation.addTranslation("Weak Blood Orb", {
    ru: "Ослабленый Кровавый Шар"
});
...
Item.createItem("apprenticeBloodOrb", "Apprentice Blood Orb", {
    name: "Apprentice",
    meta: 0
}, {
    stack: 1
});
Translation.addTranslation("Apprentice Blood Orb", {
    ru: "Кровавый Шар Ученика"
});
...
Item.createItem("magiciansBloodOrb", "Magician's Blood Orb", {
    name: "ma",
    meta: 0
}, {
    stack: 1
});
Translation.addTranslation("Magician's Blood Orb", {
    ru: "Чародейский Кровавый Шар"
});
...
Item.createItem("masterBloodOrb", "Master Blood Orb", {
    name: "master_blood_orb",
    meta: 0
}, {
    stack: 1
});
Translation.addTranslation("Master Blood Orb", {
    ru: "Кровавый Шар Волшебника - Мастера"
});
...
Item.registerNoTargetUseFunction("weakBloodOrb", function (item) {
    bloodNetwork.addMaxBloodAmount(Math.max(5000, bloodNetwork.getMaxBloodAmount()));
});

Item.registerNoTargetUseFunction("apprenticeBloodOrb", function (item) {
    bloodNetwork.addMaxBloodAmount(Math.max(25000, bloodNetwork.getMaxBloodAmount()));
});

Item.registerNoTargetUseFunction("magiciansBloodOrb", function (item) {
    bloodNetwork.addMaxBloodAmount(Math.max(150000, bloodNetwork.getMaxBloodAmount()));
});

Item.registerNoTargetUseFunction("masterBloodOrb", function (item) {
    bloodNetwork.addMaxBloodAmount(Math.max(1000000, bloodNetwork.getMaxBloodAmount()));
});

var BloodOrbList = [ItemID.weakBloodShard, ItemID.apprenticeBloodOrb,ItemID.magiciansBloodOrb,ItemID.masterBloodOrb];
function isBloodOrb(id){
    return BloodOrbList.indexOf(id)!=-1;
}
...
// file: Mechanism/Scrible/scribleCore.js
...
// file: Mechanism/Scrible/animations.js
...
// file: Mechanism/Scrible/Scrible.js
...
bloodAnimationModel.createAnimation(1, function(frame) {
    ...
    bloodAnimationRender.setPart("head", partObj, {});
...
TileEntity.registerPrototype(BlockID.bloodAltar, {
	...
	tick: function(){
		...
		if(World.getThreadTime()%2==0){
			this.bloodAnimation.setPos(this.x+.5, this.y+((this.data.blood/this.getBloodStorage())*2)/16-5/6-0.26+0.0295, this.z+.5);
		}
		...
		this.animation.load();
                if(this.data.recipe!=null&&typeof(this.data.recipe.define)=="function")
		...
		if(this.recipe==null&&this.data.progress==0){
			if(isBloodOrb(this.data.id)){
				bloodNetwork.addBlood(Math.min(5, this.data.blood));
				this.data.blood= Math.max(this.data.blood-5,0);
				if(bloodNetwork.getMaxBloodAmount()>bloodNetwork.getAmountBlood()){
					if(Math.random()<0.05){
				Particles.addFarParticle(Native.ParticleType.redstone,this.x+Math.random(), this.y+Math.random()/10+0.5, this.z+Math.random(),0,0,0,0);
			}
				}
			   }
		}
		if(this.data.progres>0&&this.data.blood>0&&this.data.id==this.data.recipe.id&&this.data.data==this.data.recipe.data&&this.data.tier>=this.data.recipe.tier){
			...
			if(this.data.progres%10==0){
				Particles.addFarParticle(Native.ParticleType.redstone,this.x+.25+Math.random()/2, this.y+Math.random()/10+0.5, this.z+.25+Math.random()/2,0,0,0,0);
			}
		}
...
Item.createItem("voidSigil", "Void Sigil", {
    name: "void_sigil",
    meta: 0
}, {
    stack: 1
...
Item.createItem("airSigil", "Air Sigil", {
    name: "air_sigil",
    meta: 0
}, {
    stack: 1
...
Item.createItem("fastMiningSigilDeactive", "Fast Mining Sigil", {
    name: "mining_sigil",
    meta: 0
}, {
    stack: 1
...
Item.createItem("fastMiningSigilActive", "Fast Mining Sigil", {
    name: "mining_sigil",
    meta: 1
}, {
    stack: 1,
...
Translation.addTranslation("Void Sigil", {
    ru: "Сигил Пустоты"
});
Translation.addTranslation("Air Sigil", {
    ru: "Сигил Полёта"
});
Translation.addTranslation("Fast Mining Sigil", {
    ru: "Сигил Быстрого Шахтёра"
});
...
Recipes.addShaped({
    id: IDData.item.airSigil,
    count: 1,
    data: 0
}, ["gpg", "gsg", "gwg"], ["g", 288, 0, "s", IDData.item.reinforcedSlate, 0, "w", IDData.item.apprenticeBloodOrb, 0, "p", 370, 0], backWorkbench);
Recipes.addShaped({
    id: IDData.item.voidSigil,
    count: 1,
    data: 0
}, ["gpg", "gsg", "gwg"], ["g", 325, 0, "s", IDData.item.reinforcedSlate, 0, "w", IDData.item.apprenticeBloodOrb, 0, "p", 287, 0], backWorkbench);
Recipes.addShaped({
    id: IDData.item.fastMiningSigilDeactive,
    count: 1,
    data: 0
}, ["ghg", "osa", "gwg"], ["g", 1, 0, "s", IDData.item.reinforcedSlate, 0, "w", IDData.item.apprenticeBloodOrb, 0, "o", 256, 0, "h", 257, 0, "a", 258, 0], backWorkbench);

Item.setLiquidClip(IDData.item.voidSigil, true);//todo use that thing
Item.registerUseFunction("voidSigil", function (coords, item, block) {
    var id = World.getBlock(coords.relative.x, coords.relative.y, coords.relative.z).id;
    if (id == 9 || id == 11) {
        if (bloodNetwork.getBlood(50)) {
            World.setBlock(coords.relative.x, coords.relative.y, coords.relative.z, 0);
        }
    }
});
...
Block.createBlock("phantomBlock", [
	{name: "Phantom Block", texture: [["phantom_block",0]], inCreative: false}
...
Ritual.addWeakRitual(7,0, 5000, function(){
			Entity.addEffect(Player.get(), Native.PotionEffect.damageResistance, 3, 2400, true, true);
		});
		...
		Ritual.addWeakRitual(173,0, 5000, function(x,y,z){
			...
			var m =Entity.spawn(x, y+2, z, Native.EntityType.ZOMBIE);
    ...
    if (World.getThreadTime() % 2 == 0) {
        var mobs = Entity.getAll();
        for (var t = 0; t < mobs.length; t++) {
            if (Entity.getType(mobs[t]) < 60&&Entity.getType(mobs[t])>1) {
                if (bloodNetwork.getBlood(1)) {
                    var xx=Entity.getPosition(mobs[t]).x-x;
					var yy=Entity.getPosition(mobs[t]).y-y;
					var zz=Entity.getPosition(mobs[t]).z-z;
		    ...
		    if (World.getThreadTime() % 5 == 0) {
		        ...
		        for (var t = 0; t < mobs.length; t++) {
		                if (Math.floor(Entity.getPosition(mobs[t]).x) == x && Math.floor(Entity.getPosition(mobs[t]).z) == z) {
                            var delta = Entity.getPosition(mobs[t]).y-y;
		                    if (delta>0&&delta<3) {
		                        if (bloodNetwork.getBlood(10)) {
...
AddBloodRecipe(267, 0, 300, 3000, 2, IDData.item.daggerOfSacrifice, 0, function(){});
