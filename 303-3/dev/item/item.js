...
"b"
], [ 'b',b, 0],function(api, field, result){for(var i in field){
...
Recipes.addShaped({id: ItemID.coffeeworkshop$Chocolatebar, count: 1, data: 0}, [
"aa",
"c"
], [ 'a',ItemID.coffeeworkshop$Cocoabatter , 0, 'c',353,0],function(api, field, result){for(var i in field){
		api.decreaseFieldSlot(i);
	}
...
//DRegistry.genItemID("coffeeworkshop$Chopowder");
//Item.createItem("coffeeworkshop$Chopowder", "巧克力粉", {name:"coffeepowder"}, {inTech: true,stack: 64});
//再这么下去我会累死的，所以改下格式
...
var setfood1 = function ( item , cn ,stack,food) {
	IDRegistry.genItemID("coffeeworkshop$"+item);
    Item.createFoodItem("coffeeworkshop$"+item, cn, {name:item}, {inTech: true,stack: stack,food:food});
    Recipes.addShaped({id: ItemID["coffeeworkshop$cookie_" + item], count: 1, data: 0}, [
"xa"
], [ 'x', ItemID["coffeeworkshop$"+item], 0, 'a', 357, 0]);
};
...
Recipes.addShaped({id: ItemID.coffeeworkshop$cream_milk, count: 1, data: 0}, [
"aa",
"c"
], [ 'a', 325, 1, 'c',353,0],function(api, field, result){for(var i in field){
		api.decreaseFieldSlot(i);
	}
Player.addItemToInventory(325, 2);
//Player.addItemToInventory(ItemID.coffeeworkshop$mkhx, 1);
...
Recipes.addShaped({id: ItemID.coffeeworkshop$croissant_raw, count: 2, data: 0}, [
"xa",
"sr"
], [ 'x', ItemID.dough, 0, 'a', ItemID.butter, 0,'s',344,0,'r',353,0]);

setItem("croissant_chocolate_raw","巧克力可颂面团");

Recipes.addShaped({id: ItemID.coffeeworkshop$croissant_chocolate_raw, count: 2, data: 0}, [
"xa",
"srh"
], [ 'x', ItemID.dough, 0, 'a', ItemID.butter, 0,'s',344,0,'r',353,0,'h',ItemID.coffeeworkshop$Chocolatebar,0]);
...
"a",
"cc"
], [ 'a', 325, 8, 'c',ItemID.coffeeworkshop$flour,0],function(api, field, result){for(var i in field){
		api.decreaseFieldSlot(i);
	}
Player.addItemToInventory(325, 1);
//Player.addItemToInventory(ItemID.coffeeworkshop$mkhx, 1);
...
Recipes.addShaped({id: ItemID.coffeeworkshop$dough_baguette, count: 1, data: 0}, [
"xx"
], [ 'x', ItemID.dough, 0]);
...
Recipes.addShaped({id: ItemID.coffeeworkshop$dough_cookie, count: 1, data: 0}, [
"xa"
], [ 'x', ItemID.dough, 0, 'a', ItemID.coffeeworkshop$Chocolatechip,0]);
