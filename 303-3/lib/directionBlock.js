LIBRARY({name: "directionBlock", version: 2, shared: true, api: "CoreEngine"});
var directionBlockAPI = {creatBlock: function (blockid, name, bool) {
    var bqlmi = 0;
    for (bqlmi = 0; bqlmi < 4; bqlmi += 1) {
        IDRegistry.genBlockID(blockid + bqlmi);
        Block.createBlock(blockid + bqlmi, [{name: name, texture: [["anvil_base", 0]], inCreative: bool}]);
    }
        ...
        if (World.getBlockID(place.x, place.y, place.z) === 0) {
            if (angle > 0) {
                if (angle > 1.75 * Math.PI || angle < 0.25 * Math.PI) {
                    ...
                    if (angle >= 0.25 * Math.PI && angle < 0.75 * Math.PI) {
                        ...
                        if (angle >= 0.75 * Math.PI && angle < 1.25 * Math.PI) {
                            ...
                            World.setBlock(place.x, place.y, place.z, BlockID[blockid + "3"]);
                        }
                    }
                }
            } else {
                if (angleabs > 1.75 * Math.PI || angleabs < 0.25 * Math.PI) {
                    ...
                    if (angleabs >= 0.25 * Math.PI && angleabs < 0.75 * Math.PI) {
                        World.setBlock(place.x, place.y, place.z, BlockID[blockid + "2"]);
                    } else {
                        if (angleabs >= 0.75 * Math.PI && angleabs < 1.25 * Math.PI) {
                            World.setBlock(place.x, place.y, place.z, BlockID[blockid + "1"]);
                        } else {
                            World.setBlock(place.x, place.y, place.z, BlockID[blockid + "3"]);
                        }
