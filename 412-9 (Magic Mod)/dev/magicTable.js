...
Block.createBlock("magicTable", [{name: "Magic Table", texture: [["magic_table_bottom", 0], ["magic_table_top", 0], ["magic_table_side", 0]], inCreative: false}], Block.createSpecialType({base: 49}));
mod_tip(BlockID["magicTable"]);
IDRegistry.genItemID("magicTable_item");
Item.createItem("magicTable_item", "Magic Table", {name: "magic_table_item"}, {stack: 64});
...
var particle_table = Particles.registerParticleType({texture: "magicTableParticle", size: [0.5, 1], lifetime: [40, 40], render: 2});
const Crafts = [];
const onCraftStart = {};
const onCraftEnd = {};
const MagicTable = {addCraft: function (items, targetItem, result, recipeviewer_order) {
    if (!items) {
        return Logger.Log("items not listed", "MagicModAPI ERROR");
    }
    if (!targetItem) {
        return Logger.Log("targetItem not listed", "MagicModAPI ERROR");
    }
    if (!result) {
        return Logger.Log("result not listed", "MagicModAPI ERROR");
    }
    if (recipeviewer_order && recipeviewer_order.length != 16) {
        recipeviewer_order = null;
    }
    Crafts.push({items: items, centre: targetItem, result: result, rv: recipeviewer_order});
}, removeCraft: function (item) {
    if (!item) {
        return Logger.Log("item not listed", "MagicModAPI ERROR");
    }
    ...
    if (!func) {
        return Logger.Log("func not listed", "MagicModAPI ERROR");
    }
...
TileEntity.registerPrototype(BlockID.magicTable, {defaultValues: {anim: null, rotation: [0, 0, 0], coords: null, step: null, lastid: 0, lastdata: 0}, getTransportSlots: function () {
    return {input: ["slot"]};
}, created: function () {
    this.data.coords = [[this.x + 3, this.z], [this.x + 3, this.z + 1], [this.x + 2, this.z + 2], [this.x + 1, this.z + 3], [this.x, this.z + 3], [this.x - 1, this.z + 3], [this.x - 2, this.z + 2], [this.x - 3, this.z + 1], [this.x - 3, this.z], [this.x - 3, this.z - 1], [this.x - 2, this.z - 2], [this.x - 1, this.z - 3], [this.x, this.z - 3], [this.x + 1, this.z - 3], [this.x + 2, this.z - 2], [this.x + 3, this.z - 1]];
    ...
    if (id == ItemID.magicWand && Entity.getSneaking(Player.get())) {
        ...
        for (var i in this.data.coords) {
            if (World.getBlock(this.data.coords[i][0], this.y, this.data.coords[i][1]).id == BlockID.magicPedestal) {
                ...
                if (!tile) {
                    tile = World.addTileEntity(this.data.coords[i][0], this.y, this.data.coords[i][1]);
                }
                if (tile && tile.container.getSlot("slot").id != 0) {
                    pedestals.push(this.data.coords[i]);
                }
            }
        }
        if (this.container.getSlot("slot").id == 0 || pedestals.length == 0) {
            return;
        }
        var CraftingItems = {};
        for (var i in Crafts) {
        ...
        for (var l in CraftingItems) {
            ...
            for (var i in itemsInPedestals) {
                if (centreItem != CraftingItems[l].centre || JSONlength(itemsInPedestals) != JSONlength(CraftingItems[l]) - 1 || !CraftingItems[l][i] || itemsInPedestals[i] != CraftingItems[l][i]) {
                    continue;
                }
                result++;
        ...
        if (typeof (result) == "string") {
            if (onCraftStart[result] && onCraftStart[result]({x: this.x, y: this.y, z: this.z}) == "stop") {
                return;
            }
            for (var i in pedestals) {
            ...
            this.data.step = {pedestals: pedestals, result: result};
            ...
            asd(pedestals, 0, ths, function () {
                ...
                ths.data.step = null;
                if (onCraftEnd[result.toString()]) {
                    onCraftEnd[result.toString()]({x: ths.x, y: ths.y, z: ths.z});
                }
            });
        }
        return true;
    ...
    if ((!this.data.anim || (this.data.lastid != item.id || this.data.lastdata != item.data)) && item.id != 0) {
        if ((this.data.lastid != item.id || this.data.lastdata != item.data) && this.data.anim) {
        ...
        this.data.anim.describeItem({id: item.id, count: 1, data: item.data, size: 1 / (16 / 6), rotation: this.data.rotation, notRandomize: true});
        this.data.anim.load();
    } else {
...
function asd(pedestals, i, tileTable, endFunc) {
    ...
    if (i >= pedestals.length) {
        return endFunc();
    }
    var tile = World.getTileEntity(pedestals[i][0], tileTable.y, pedestals[i][1]);
    if (tile && tile.container.getSlot("slot").id != 0) {
        particles({x: tileTable.x, y: tileTable.y, z: tileTable.z}, {x: pedestals[i][0], y: tileTable.y, z: pedestals[i][1]}, tile, "slot", particle_table, 3, 40, function () {
