...
Item.createItem("magicWand", "Magic Wand", {name: "magic_wand"}, {stack: 1});
mod_tip(ItemID["magicWand"]);
var particle_air = Particles.registerParticleType({texture: "air", size: [1, 15], lifetime: [40, 120], render: 2, animators: {size: {period: 120, fadeIn: 1, fadeOut: 0, start: 0.2, end: 1}}});
var gI = 1;
var lens_selection_menu = new UI.Window({location: {width: 400, height: 400, x: 300, y: 50 + 20}, params: {}, drawing: [{type: "color", color: android.graphics.Color.TRANSPARENT}], elements: {"image": {type: "image", x: 0, y: 0, z: -1, bitmap: "frame1", scale: 1000 / 256}, "desc": {type: "text", x: 275, y: 275, text: "", font: {color: android.graphics.Color.WHITE, shadow: 0.5, size: 30}}}});
...
var drop_mob = false;
var mob_captured;
var tel_window = new UI.Window({location: {width: 60, height: 60 * 2, x: 0, y: 150}, drawing: [{type: "color", color: android.graphics.Color.TRANSPARENT}], elements: {"button_push": {x: 0, y: 0, type: "button", bitmap: "tel_push", scale: 50, clicker: {onClick: function (position, container, tileEntity, window, canvas, scale) {
    drop_mob = true;
    var vel = Entity.getLookVector(Player.get());
    Entity.addVelocity(mob_captured, vel.x * 5, vel.y * 5, vel.z * 5);
}}}, "button_drop": {x: 0, y: 1000, type: "button", bitmap: "tel_drop", scale: 50, clicker: {onClick: function (position, container, tileEntity, window, canvas, scale) {
    drop_mob = true;
}}}}});
var tel_container = new UI.Container();
tel_window.setAsGameOverlay(true);
var nature_window = new UI.Window({location: {width: 80, height: 80, x: 930, y: UI.getScreenHeight() / 2 - 50}, drawing: [{type: "color", color: android.graphics.Color.TRANSPARENT}], elements: {"subbutton": {x: -1000, y: -1000, type: "button", bitmap: "tel_push", scale: 150, onTouchEvent: function (a, event) {
    if ((event.localX <= 1 / 3 || event.localX >= 2 / 3) || (event.localY <= 1 / 3 || event.localY >= 2 / 3)) {
        nature_stop();
    }
}}, "button": {x: 0, y: 0, type: "button", bitmap2: "nature_button_pressed", bitmap: "nature_button", scale: 50, onTouchEvent: function (a, event) {
    if (event.type == "DOWN") {
        nature_start();
    }
...
var lenses = [{name: "Air", texture: "airLense", description: "The wind will push you up and when you attack the mobs, the wind will push them further away\n\nOn use no target: You fly up\nOn attack: The mob pushes further", onUseNoTarget: function (item) {
    sound_air.stop();
    ...
    sound_air.play();
}}, {name: "Fire", description: "Launches fireballs and sets fire to blocks\n\nOn use no target: Launch fareball\nOn use: Set fire", texture: "fireLense", onUseNoTarget: function (item) {
...
}}, {name: "Blood", description: "Restores health through target health\n\nWhen using on a mob: Inflicts 1hp to a mob and restore 1hp you\nOn use no target: If a diamond lies nearby, it replaces it with a bloody one and takes 19hp from you", texture: "bloodLense", onUseNoTarget: function (item) {
    ...
    if (items.length != 0) {
        ...
        if (!diamond || Entity.getHealth(Player.get()) < 20) {
            return;
        }
...
}}, {name: "Telekinesis", texture: "telLense", description: "Allows you to lift mobs into the air and throw them far\n\nWhen using on a mob: raise mob\nLeft top button using for throw mob\nLeft down button using for let off mob", onUseNoTarget: function (item) {
    var pointed = getPointed();
    if (pointed.entity == -1) {
        return;
    }
    ...
    var lastp = {x: 0, y: 0, z: 0};
    if (!tel_container.isOpened()) {
        tel_container.openAs(tel_window);
    }
    setInterval(function () {
        var item = Player.getCarriedItem();
        if (pointed.entity != mob_captured) {
            drop_mob = true;
        }
        if (item.id != ItemID.magicWand || item.data != 4) {
            drop_mob = true;
        }
        if (drop_mob) {
            drop_mob = false;
        ...
        var target_pos = {x: pos.x + vec.x * 5, y: pos.y + vec.y * 5, z: pos.z + vec.z * 5};
        Entity.moveToTarget(pointed.entity, target_pos, {speed: 10, denyY: false, jumpVel: 10});
    }, 1);
}}, {name: "Nature", description: "", texture: "natureLense", onSelected: function (item) {
    if (!nature_container.isOpened() && _LevelDisplayed) {
...
var lens = {add: function (data) {
    if (!data) {
        return Logger.Log("Data not listed", "MagicModAPI ERROR");
    }
    if (!data.name) {
        return Logger.Log("Name not listed", "MagicModAPI ERROR");
    }
    if (!data.texture) {
        return Logger.Log("Texture not listed", "MagicModAPI ERROR");
    }
    lenses.push(data);
...
Callback.addCallback("APILoaded", function () {
    for (var i in lenses) {
        ...
        Item.createItem("LensOf" + lenses[i].name, "Lens Of " + lenses[i].name, {name: lenses[i].texture}, {stack: 1});
...
function nature_start() {
    ...
    setInterval(function () {
        ...
        Game.tipMessage("\xa7a" + Translation.translate("Nature lens active"));
...
Item.registerNoTargetUseFunction("magicWand", function (item) {
    ...
    if (lenses[item.data - 1].onUseNoTarget) {
        lenses[item.data - 1].onUseNoTarget(item);
    }
});
Callback.addCallback("PlayerAttack", function (player, victim) {
    ...
    if (item.id == ItemID.magicWand) {
        ...
        if (lenses[item.data - 1].onPlayerAttack) {
            lenses[item.data - 1].onPlayerAttack(player, victim);
        }
    }
});
var _last_item = {};
Callback.addCallback("tick", function (player, victim) {
    var item = Player.getCarriedItem();
    if (JSON.stringify(item) == JSON.stringify(_last_item)) {
        return;
    }
    if (item.id == ItemID.magicWand) {
        if (item.data == 0) {
        ...
        if (lenses[item.data - 1].onSelected) {
            lenses[item.data - 1].onSelected(item);
        }
    }
    if (_last_item.id == ItemID.magicWand) {
        if (_last_item.data == 0) {
            return;
        }
        if (lenses[_last_item.data - 1].onDeSelected) {
            lenses[_last_item.data - 1].onDeSelected(_last_item);
        }
    }
    _last_item = item;
});
Item.registerNameOverrideFunction(ItemID.magicWand, function (item, name) {
    name = "\xa7b" + name;
    if (item.data == 0) {
        return name + "\n\xa77" + Translation.translate("Lens") + ": " + Translation.translate("Nope");
    }
    if (lenses[item.data - 1]) {
        return name + "\n\xa77" + Translation.translate("Lens") + ": " + Translation.translate(lenses[item.data - 1].name);
    } else {
        return name + "\n\xa77" + Translation.translate("Lens") + ": " + Translation.translate("Nope");
    }
});
Item.registerUseFunction("magicWand", function (coords, item, block) {
    if (block.id == BlockID.magicTable) {
        return;
    }
    if (Entity.getSneaking(Player.get())) {
        if (item.data == 0) {
            ...
            for (var i = 2 / items.length; i <= 2; i += 2 / items.length) {
                ...
                content.elements["slot" + k] = {slot: "slot" + k, type: "slot", x: Math.sin(Math.PI * i) * 385 + (500 - 100), y: Math.cos(Math.PI * i) * -1 * 385 + (500 - 100), size: 200, bitmap: "_default_slot_empty", visual: true, isTransparentBackground: true, clicker: {onClick: function (position, container, tileEntity, window, canvas, scale) {
                    ...
                    var data = 0;
                    for (var i in lenses) {
                        if (ItemID["LensOf" + lenses[i].name] == item.id) {
                            data = (+i) + 1;
                        }
                    }
                    ...
                    if (container.getSlot(this.slot).id == 0) {
                        return;
                    }
                    ...
                    for (var i = 1; i < splited.length; i++) {
                        cont.elements["desc" + i] = {type: "text", x: 275, y: 275 + y, text: splited[i], font: {color: android.graphics.Color.WHITE, shadow: 0.5, size: 30}};
                        y += 35;
                    }
                }}};
            }
    ...
    if (lenses[item.data - 1].onUse) {
        lenses[item.data - 1].onUse(coords, item, block);
    }
});
function stop_capture_on_death(entity) {
    if (entity == mob_captured) {
        drop_mob = true;
    }
}
