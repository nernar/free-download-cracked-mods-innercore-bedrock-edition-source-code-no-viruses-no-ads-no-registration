...
Block.createBlock("magicPedestal", [{name: "Magic Pedestal", texture: [["obsidian", 0]], inCreative: false}], Block.createSpecialType({base: 49}));
...
function particles_for_pedestal(x, y, z) {
    var bonus_coords = [[1 / 16 * 4, 1 / 16 * 4], [1 / 16 * 13, 1 / 16 * 4], [1 / 16 * 4, 1 / 16 * 13], [1 / 16 * 12, 1 / 16 * 12]];
...
(function () {
    ...
    var boxes = [[0, 0, 0, 1, 0.1, 1, BlockID.magicPedestal, 0], [0.15, 0.1, 0.15, 0.85, 0.2, 0.85, BlockID.magicPedestal, 0], [0.3, 0.2, 0.3, 0.7, 0.7, 0.7, BlockID.magicPedestal, 0], [0.2, 0.75, 0.2, 0.25, 0.8, 0.25, 89, 0], [0.75, 0.8, 0.25, 0.8, 0.75, 0.2, 89, 0], [0.25, 0.8, 0.75, 0.2, 0.75, 0.8, 89, 0], [0.75, 0.75, 0.75, 0.8, 0.8, 0.8, 89, 0]];
    for (var i in boxes) {
        var box = boxes[i];
    ...
    var model = BlockRenderer.createModel();
    model.addBox(0.2, 0.7, 0.2, 0.8, 0.75, 0.8, [["magic_pedestal_top", 0], ["magic_pedestal_top", 0], ["magic_pedestal_side", 0], ["magic_pedestal_side", 0], ["magic_pedestal_side", 0], ["magic_pedestal_side", 0]]);
    render.addEntry(model);
    BlockRenderer.setCustomCollisionShape(BlockID.magicPedestal, -1, Dmodel);
})();
var config_animation = __config__.getBool("animation");
TileEntity.registerPrototype(BlockID.magicPedestal, {defaultValues: {anim: null, lastid: 0, lastdata: 0, max_y: false, is_item: false}, getTransportSlots: function () {
    return {input: ["slot"]};
    ...
    if ((!this.data.anim || (this.data.lastid != item.id || this.data.lastdata != item.data)) && item.id != 0) {
        ...
        this.data.anim_coords = {x: this.x + 0.5 + 1 / 16 - 1 / 16 + bonus_coords.x, y: this.y + 0.75 + 0.125 + 1 / 16 / (1 / (1 / (16 / 6))) * 3.5 + bonus_coords.y, z: this.z + 0.5 - 1 / 16 + 1 / 16 + bonus_coords.z};
        this.data.anim = new Animation.Item(this.data.anim_coords.x, this.data.anim_coords.y, this.data.anim_coords.z);
        this.data.describe = {id: item.id, count: 1, data: item.data, size: 1 / (16 / 6), rotation: rotation, notRandomize: true};
        ...
        if (this.data.anim && item.id == 0) {
            ...
            if (this.data.anim && config_animation) {
                this.data.describe.rotation[1] += Math.sin(Math.PI / 180);
                if (this.data.describe.rotation[0] > 0) {
                    this.data.describe.rotation[0] -= Math.PI / 180;
                }
                if ((this.data.anim.coords.y <= this.data.anim_coords.y + (this.data.is_item ? 0.152 : 0) && this.data.max_y) || (this.data.anim.coords.y >= this.data.anim_coords.y + 0.3 + (this.data.is_item ? 0.152 : 0) && !this.data.max_y)) {
                    this.data.max_y = !this.data.max_y;
                }
                if (!this.data.max_y) {
