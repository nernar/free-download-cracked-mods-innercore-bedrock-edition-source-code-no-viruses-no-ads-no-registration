...
const jSetInterval = function (__fun, __mil) {
    var timer = new Timer();
    var task = new TimerTask({run: function () {
        if (__fun()) {
            timer.cancel();
        }
    }});
...
const setTimeout = function (func, _ticks) {
    var ticks = 0;
    return {id: onCallback("tick", function () {
        ticks++;
        if (ticks >= _ticks) {
            func();
            return "delete";
...
const setTimeoutLocal = function (func, _ticks) {
    var ticks = 0;
    return {id: onCallback("LocalTick", function () {
        ticks++;
        if (ticks >= _ticks) {
            func();
            return "delete";
...
function _find_block_y(coords) {
    for (var i = coords.y; i >= 0; i--) {
        if (World.getBlock(coords.x, i, coords.z).id != 0) {
            return i;
        }
    }
}
function rotateBitmap(__bitmap, __angle) {
    ...
    return Bitmap.createBitmap(__bitmap, 0, 0, __bitmap.getWidth(), __bitmap.getHeight(), matrix, true);
}
