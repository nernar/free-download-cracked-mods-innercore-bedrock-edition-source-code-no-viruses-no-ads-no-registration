LIBRARY({name: "Bow", version: 2, shared: true, api: "CoreEngine", dependencies: ["Timer", "SoundAPI", "Inventory"]});
IMPORT("Timer", "Timer");
IMPORT("SoundAPI", "Sound");
IMPORT("Inventory", "Inventory");
var McMath = {rtd(rotation, dgr) {
...
var Color = android.graphics.Color, LinearLayout = android.widget.LinearLayout, LayoutParams = android.widget.RelativeLayout.LayoutParams, Gravity = android.view.Gravity, BitmapFactory = android.graphics.BitmapFactory, View = android.view.View, t_bool = java.lang.Boolean.TYPE, t_int = java.lang.Integer.TYPE, t_double = java.lang.Double.TYPE, t_string = java.lang.String, ctx = UI.getContext(), ImageHelper = {decodeBmp(string) {
    string = android.util.Base64.decode(string, 0);
    return android.graphics.BitmapFactory.decodeByteArray(string, 0, string.length);
}, bmpToImg(string) {
    var decodedBmp = this.decodeBmp(string), bmp = android.graphics.Bitmap.createBitmap(35, 35, android.graphics.Bitmap.Config.ARGB_8888), canvas = new android.graphics.Canvas(bmp);
    canvas.drawBitmap(decodedBmp, 0, 0, null);
    return bmp;
}}, nativeItem = java.lang.Class.forName("zhekasmirnov.launcher.api.NativeItem", true, UI.getContext().getClass().getClassLoader()), overrideItemIcon = nativeItem.getMethod("overrideItemIcon", t_string, t_int), setItemRequiresIconOverride = nativeItem.getMethod("setItemRequiresIconOverride", t_int, t_bool);
Item.overrideItemIcon = function (name, index, id) {
    try {
        overrideItemIcon.invoke(null, name, new java.lang.Integer(index));
    }
    catch (e) {
    }
...
var arrows = [];
var arrow_inst_id = 0;
var Arrow = Particles.registerParticleType({texture: "arrow", size: [1, 1], lifetime: [1, 5], velocity: [0, 0, 0]});
...
function CustomArrow() {
    this.spawn = function (params) {
        ...
        var c = Entity.getPosition(Player.get()), v = McMath.lookDirection(lookAngle.yaw, lookAngle.pitch);
        entity = Entity.spawn(c.x + (v.x * 0.12), c.y + (v.y * 0.12), c.z + (v.z * 0.12), 80);
    ...
    this.animate = function () {
        for (let i = -0.99; i < 0.99; i += 0.33) {
            var c = {x: this.p.x - (this.v.x * i), y: this.p.y - (this.v.y * i), z: this.p.z - (this.v.z * i)};
...
var Bows = {aim: "", inGame: false, currentBow: null, aimShown: false, bowInfo: {}, dynamicProps: {bullets: [], hurt: []}, windowAim: null, addNewBow(info) {
    info.shooting = false;
    info.state = 0;
    Bows.bowInfo[info.id] = info;
    Item.setItemRequiresIconOverride(info.id, true);
    Item.registerNoTargetUseFunction(info.id, function () {
        Bows.setShoot(info.id, true);
    });
    ...
    newThread(function () {
        var layoutMain = new LinearLayout(ctx), params = new LinearLayout.LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT), aimIMG = new android.widget.ImageView(ctx);
    ...
    Bows.dynamicProps.bullets = Bows.dynamicProps.bullets.filter(function (bullet) {
        if (bullet.entity == p) {
            if (t.entity != -1) {
                Bows.addHurt({entity: t.entity, damage: bullet.damage});
